...
class AivBot:
    ...
    async def _onStartTask(self,task):
        ...
        try:
            ...
            errMsg = f'Bot: {self.botInfo.botName} > {apiName}() 运行错误: {e}'
    ...
    def _addFileToTaskOutParam(self,filePath, paramName= '', paramList = None):
        ''' 2024.6
            往参数列表添加文件
    ...
    def _addStringToTaskOutParam(self, text, paramName= '', paramList = None):
        ''' 2024.6
            往参数列表添加字符串
            默认是操作 self.task['paramOut'], 也可以指定一个空的 [] 操作,这样可以自定义一个参数列表,最后再赋值给 'paramIn' 或 'paramOut'
            @param text 文本
    ...
    def createParamIn(self,type, data, paramName= '', paramIn= []):
        ''' 2024.6
    ...
    def push(self,type,data):
        ...
        if type.lower() == 'string':
            return self._addStringToTaskOutParam(data)

        if type.lower() == 'file':
            return self._addFileToTaskOutParam(data)

        
        # logger.debug('当前输出的内容是: {}'.format(self.task['paramOut']))

    def setResult(self, code, msg, task= None):
        ''' 2024.6
            设置当前Bot任务的返回信息
    ...
    def getOutPath(self, extName= '', childPath= None):
        ''' 2024.3
            获取输出文件的目录 ( 或文件名)   
            @paran extName : 将输出的文件扩展名 (指定这个参数,则返回的结果是一个完整的文件名<唯一, 输出目录中还没创建>)        
            @param childPath : 需要创建的子目录 (像comfyUI,生成的图片很多,可以单独为它创建一个子目录,子目录AIV也会定时清理)          
            @return : 
                1、extName == '' :返回AIV的输出目录 (或子目录) (一般在 d:/aivc/data/out)
                2、extName != '' :返回一个输出目录中的唯一文件名 (目的是给后续的AI任务保存, 以便共享此文件)
        ...
        '''
        outPath = self.aivBotMmap.sysInfo['sys.outDir'] 
        ...
        if extName != '':
            # 如果用户设置了扩展名,则生成一个唯一的文件名 (输出目录还没存在的)
            ...
            count = 0
            while True:
                name = f'{timeStamp}{count}'
                fileName = os.path.join(outPath, name + extName) # 拼接路径生成文件名
    ...
    async def getPrompt(self, apiInfo = None):
        ...
        # from loguru import logger
        promptText = ''
        promptList = self.getParam('string', 'prompt')

        if len(promptList): #优先取 task['paramIn']的 'string' 中的 'prompt' 入参
            promptText = promptList[0]
        
        else: 
            # 如果没有文字入参, 则尝试查询是否有mp3或wav文件(前端可以使用语音输入指令),如果有则调用第三方的 Bot 获取语音识别文字
            # 用户也可以自定义其它语音识别的方法(比如用http调用百度语音、迅飞语音接口返回语音识别的文字) 2024.6
            fileList = self.getParam('file')
            if len(fileList)>0:
                ...
                logger.warning('无 prompt 提示词, 也没有设置语音文件, 获取prompt提示词失败!')
                return promptText
            ...
            slotTask = await self.startSlotTask(apiInfo,None, timeOut= 5)
            ...
            if slotTask['result']['code'] != 200:
                ...
                strList = self.getParam('string', None, 'paramOut',slotTask)            
                promptText = ''.join(strList)

        # logger.warning(f'AivAudio: Bot返回的结果是: {promptText}')
        return promptText
    

    def getParam(self, type, fieldName= None, paramStr = 'paramIn', task = None):
        ''' 2024.6
            获取任务参数
            @param type 要获取的参数类型, 可选 'file', 'string'
            @param fieldName 可以指定获取参数的名字, 如 'prompt' (必须与客户端传参匹配)
            ...
            for param in paramList:
                if param['type'].lower() == type.lower():
                    if fieldName is None:
                        ...
                        if param['name'].lower() == fieldName.lower(): # 比如获取 'prompt'参数的值:  getParam('string','prompt')
                            return param['data']  #读出所有行 (paramLs['data'] 是一个列表)
            return []
