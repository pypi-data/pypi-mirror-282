        ...
        if login(username, password):
            ...
            while True:
                ...
                if current_url == target_url:
                    print("URL has changed to the target URL:", target_url)
                else:
                    print("Current URL:", current_url)
                
                # Stop monitoring once correct password is found
                return
        
        else:
            print(f"Login failed with password: {password}")
            time.sleep(3)  # Sleep for 3 seconds before trying the next password
