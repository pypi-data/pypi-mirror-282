...
class ECSManager:
    ...
    def create_instance(self):
        try:
            ...
            available_categories = self.get_available_disk_categories(config.region, config.instance_type)
            print(f"Available disk categories for {config.instance_type}: {available_categories}")
            ...
            instance_name = f"sq-{time_str}"
            request.set_InstanceName(instance_name)
            request.set_InternetChargeType(config.internet_charge_type)

            request.set_IoOptimized('optimized')
            # request.set_SystemDiskCategory('cloud_ssd')
            request.set_SystemDiskCategory('cloud_efficiency')  # 使用高效云盘
            request.set_SystemDiskSize(20)  # 设置系统盘大小，单位为 GB

            request.set_InternetMaxBandwidthOut(60)  
            request.set_InternetMaxBandwidthIn(200)  
            response = self.client.do_action_with_exception(request)
            instance_details = json.loads(response)
            instance_id = instance_details['InstanceId']
            print(f'ECS Instance created with ID: {instance_id}')
            ...
            # 等待实例状态为 Running
            self.wait_instance_status(instance_id, 'Running')

            # 分配公网 IP
            self.allocate_public_ip(instance_id)

            # 执行shell脚本
            shell_script = 'wget https://get.vpnsetup.net -O vpn.sh && export YOUR_IPSEC_PSK=greatpsk; export YOUR_USERNAME=greatvpn; export YOUR_PASSWORD=greatpass'
            # shell_script = 'wget https://get.vpnsetup.net -O vpn.sh'
            
            # shell_script = "echo 'Hello from ECS'; date; uptime"
            # create_command_request.set_CommandContent('wget https://get.vpnsetup.net -O vpn.sh')
            invoke_id = self.run_command(instance_id, shell_script)
            print(f"Command invoked with ID: {invoke_id}")

            # 获取输出结果
            output = self.get_command_output(invoke_id)
            print("Command output:")
            print(output)

            # 执行shell脚本
            shell_script = 'sudo sh vpn.sh'
            invoke_id = self.run_command(instance_id, shell_script)
            print(f"Command invoked with ID: {invoke_id}")
