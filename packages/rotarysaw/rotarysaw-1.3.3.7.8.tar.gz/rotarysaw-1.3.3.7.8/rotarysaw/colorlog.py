...
BASE_COLOR_FORMAT = "[%(asctime)s][%(color_levelname)-18s] %(message)s"
...
class ColoredFormatter(logging.Formatter):
    ...
    def formatTime(self, record, datefmt=None ):
        return strftime("%Y%m%d %H%M%S", record)
    ...
    def format(self, record):
        ...
        truncated_level = record.levelname[:8]
...
class FileFormatter(logging.Formatter):
    def __init__(self):
        logging.Formatter.__init__(self, color_message(BASE_FORMAT))
    ...
    def format(self, record):
        levelname = record.levelname
        truncated_level = record.levelname[:8]
        levelname_color = truncated_level
        record.color_levelname = levelname_color
        return logging.Formatter.format(self, record)
