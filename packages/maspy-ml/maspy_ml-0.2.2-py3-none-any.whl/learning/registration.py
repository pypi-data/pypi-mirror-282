...
def register(
    ...
    vector_entry_point: VectorModelCreator | str | None = None,
    ...
    new_spec = ModelSpec(
        ...
        disable_model_checker=disable_env_checker,
        kwargs=kwargs,
...
def make(
    ...
    **kwargs: Any,
    ...
    if model_spec.entry_point is None:
        raise print(f"{model_spec.id} registered but entry_point is not specified")
    elif callable(model_spec.entry_point):
        env_creator = model_spec.entry_point
    else:
        # Assume it's a string
        env_creator = load_env_creator(model_spec.entry_point)
    ...
    model.unwrapped.spec = ModelSpec(
        id=model_spec.id,
        entry_point=model_spec.entry_point,
    ...
    num_prior_wrappers = len(model.spec.additional_wrappers)
    if (
        model_spec.additional_wrappers[:num_prior_wrappers]
        != model.spec.additional_wrappers
    ):
        for model_spec_wrapper_spec, recreated_wrapper_spec in zip(
            model_spec.additional_wrappers, model.spec.additional_wrappers
        ):
            raise ValueError(
    ...
    for wrapper_spec in model_spec.additional_wrappers[num_prior_wrappers:]:
        ...
        model = load_env_creator(wrapper_spec.entry_point)(env=model, **wrapper_spec.kwargs)
...
def pprint_registry(
    print_registry: dict[str, ModelSpec] = registry,
    *,
    num_cols: int = 3,
    ...
    for model_spec in print_registry.values():
        ns = model_spec.namespace

        if ns is None and isinstance(model_spec.entry_point, str):
            # Use regex to obtain namespace from entrypoints.
            env_entry_point = re.sub(r":\w+", "", model_spec.entry_point)
        ...
        namespace_envs[ns].append(model_spec.id)
        max_justify = max(max_justify, len(model_spec.name))

    # Iterate through each namespace and print environment alphabetically
    output: list[str] = []
    for ns, model_ids in namespace_envs.items():
        ...
        for count, model_id in enumerate(sorted(model_ids), 1):
            # Print column with justification.
            namespace_output += model_id.ljust(max_justify) + " "
