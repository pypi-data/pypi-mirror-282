...
class StrandMesh(Mesh):
    ...
    def evaluate_amplitude_dependent_meshing(self):
        ...
        if self.cacdm.mesh.filaments.amplitude_dependent_scaling and self.cacdm.solve.source_parameters.sine.current_amplitude == 0 and not self.cacdm.geometry.io_settings.load.load_from_yaml:
            ...
            if (number_of_elements_penetrated_by_field < desired_elements_in_field_penetration_region) and field_penetration_distance < filament_radius:
                print(f"The magnetic field is expected to penetrate over approximately {round(number_of_elements_penetrated_by_field, 3)} elements in the filaments. To ensure that the field penetrates over at least {desired_elements_in_field_penetration_region} elements, the mesh in the filaments has been adjusted.")
    ...
    def air_field(self):
        ...
        air_outer_boundary_curves = self.geometry_class.air.boundary_curves
        air_inner_boundary_curves = sum(self.geometry_class.air.inner_boundary_curves, [])
    ...
    def generate_cuts(self):
        ...
        # a) Cut for the total current intensity, associated with the strand boundary
        strand_bnd = self.geometry_class.air.physical_inner_boundary_tags[0]
        gmsh.model.mesh.addHomologyRequest("Homology", domainTags=[strand_bnd],dims=[1])
        air = self.geometry_class.air.physical_surface_tag
        gmsh.model.mesh.addHomologyRequest("Cohomology", domainTags=[air],dims=[1])
        cuts = gmsh.model.mesh.computeHomology()
        gmsh.model.mesh.clearHomologyRequests()
        gmsh.plugin.setString("HomologyPostProcessing", "PhysicalGroupsOfOperatedChains", str(cuts[0][1]))
        gmsh.plugin.setString("HomologyPostProcessing", "PhysicalGroupsOfOperatedChains2", str(cuts[1][1]))
        gmsh.plugin.run("HomologyPostProcessing")

        self.geometry_class.air.strand_bnd_cut_tag = cuts[1][1]+1 # The cut tag for the strand boundary	
    ...
    def generate_regions_file(self):
        ...
        regions_model.induced["Matrix"].surf_out.numbers.append(self.geometry_class.air.physical_inner_boundary_tags[0])
        regions_model.induced["Matrix"].surf_out.names.append(self.geometry_class.air.physical_inner_boundary_names[0])

        regions_model.induced["Matrix"].cochain.numbers.append(self.geometry_class.air.strand_bnd_cut_tag)
        regions_model.induced["Matrix"].cochain.names.append("Strand boundary cut tag")

        
        regions_model.air.vol.number = self.geometry_class.air.physical_surface_tag
        regions_model.air.vol.name = self.geometry_class.air.physical_surface_name

        regions_model.air.surf.number = self.geometry_class.air.physical_boundary_tag
        regions_model.air.surf.name = self.geometry_class.air.physical_boundary_name
        
        # Add physical point at matrix boundary to fix phi=0
        regions_model.air.point.names = ["Point at strand boundary"]
        regions_model.air.point.numbers = [self.geometry_class.air.strand_bnd_physicalEdgePointTag]
