...
class BaseStrategy(ABC):
    ...
    def run(self):
        ...
        self.indices_to_trade = initialize_indices(
            self, self._indices, self.allowed_dte
        )
...
def get_n_dte_indices(*indices: Index, dte: int) -> list[Index] | list[None]:
    ...
    if dte == 0:
        eligible_indices = list(indices_less_than_0_dte)
    else:
        indices_less_than_n_dte = set(
            [
                index
                for index, time_left_to_expiry in zip(indices, times_to_expiries)
                if time_left_to_expiry < (dte + 1) and index.name in safe_indices
            ]
        )
        eligible_indices = indices_less_than_n_dte.union(indices_less_than_0_dte)
    return list(eligible_indices)


def initialize_indices(strategy, indices: list[str], dte: int) -> list[Index]:
    indices = [Index(index) for index in indices]
    indices: list[Index] | [] = get_n_dte_indices(*indices, dte=dte)
