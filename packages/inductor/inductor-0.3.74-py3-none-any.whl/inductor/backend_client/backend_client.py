...
async def get_playground_websocket_iterator(
    ...
    On connection close, reconnect and continue yielding websocket client
    connections. This function creates its own auth session instead of
    requiring an auth access token, since the websocket connection could be
    long-lived and the access token could expire.
