...
class IDSUser:
    ...
    def is_device_registered(self: Self) -> bool:
        ...
        return (
            self.registration_certificate is not None
            and datetime.now().astimezone() < self.registration_certificate.not_valid_after_utc.astimezone()
        )

    def __init__(
        self: Self,
        apple_id: str,
        anisette_provider: MachineDataComponent | ProvidesAnisetteHeaders | None = None,
        apns_credentials: APNsCredentialsComponent | ProvidesAPNsCredentials | None = None,
    ) -> None:
        ...
        self.registration_certificate: Certificate | None = None
        ...
        self._gsa_client = GSAClient(self.anisette_provider)
    ...
    def _generate_payload(
        ...
        return (
            ...
            + _to_length_value(PUSH_TOKEN_TRANSFORMER.deserialize(self.apns_credentials.push_token)),
        )
    ...
    def _generate_auth_headers(
        ...
        push_nonce, push_sig = self._generate_signed_payload(
            self.apns_credentials.push_key, bag_key=bag_key, payload=payload,
        )
        ...
        return {
            ...
            "x-push-cert": b64encoded_der(self.apns_credentials.push_cert),
            "x-push-token": PUSH_TOKEN_TRANSFORMER.deserialize(self.apns_credentials.push_token),
    ...
    async def fetch_handles(self: Self) -> None:
        ...
        if self.apns_credentials.push_token is None:
    ...
    async def register_device(
        ...
        self.registration_certificate = load_der_x509_certificate(certificate_data)
        self.on_device_registered_event.set()

        self.log(logging.DEBUG, "Successfully obtained device certificate.")
        self.log(
            logging.DEBUG, f"Certificate valid until {self.registration_certificate.not_valid_after_utc.astimezone()}.",
        )

    def write(self: Self, path: PathLike) -> None:
        """Write the user's credentials to a file."""
        path = path if isinstance(path, Path) else Path(path)
        path.mkdir(parents=True, exist_ok=True)

        (path / "user_info.json").write_text(
            json.dumps(
                {
        ...
        if self.certificate:
            (path / "user.crt").write_bytes(self.certificate.public_bytes(Encoding.PEM))

        if self.encryption_key:
            (path / "encryption.key").write_bytes(
        ...
        if self.registration_certificate:
            (path / "registration.crt").write_bytes(self.registration_certificate.public_bytes(encoding=Encoding.PEM))
    ...
    def read(cls, path: PathLike) -> Self:
        ...
        if not path.is_dir():
            msg = f"User directory not found: {path}"
            raise FileNotFoundError(msg)

        user_info = json.loads((path / "user_info.json").read_text())

        private_key = None
        if (path / "user.key").is_file():
            private_key = load_pem_private_key((path / "user.key").read_bytes(), None)

        certificate = None
        if (path / "user.crt").is_file():
            certificate = load_pem_x509_certificate((path / "user.crt").read_bytes())

        encryption_key = None
        if (path / "encryption.key").is_file():
            encryption_key = load_pem_private_key((path / "encryption.key").read_bytes(), None)

        signing_key = None
        if (path / "signing.key").is_file():
            signing_key = load_pem_private_key((path / "signing.key").read_bytes(), None)

        registration_certificate = None
        if (path / "registration.crt").is_file():
            registration_certificate = load_pem_x509_certificate((path / "registration.crt").read_bytes())

        user = cls(user_info["apple_id"], None, None)
        user.profile_id = user_info["profile_id"]
        user.auth_token = user_info["auth_token"]
        user.private_key = private_key
        user.certificate = certificate
        user.encryption_key = encryption_key
        user.signing_key = signing_key
        user.registration_certificate = registration_certificate
