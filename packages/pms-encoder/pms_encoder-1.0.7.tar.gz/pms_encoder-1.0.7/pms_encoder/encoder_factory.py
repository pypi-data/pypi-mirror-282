...
class RayEncoderFactory:
    def __init__(
        ...
        if model_key in [SleepAndPassProcessor.__name__]:
            assert model_name == "", f"ERROR, Processor[{model_key}]'s model_name must be emptry string."
            assert model_alias == "", f"ERROR, Processor[{model_key}]'s model_alias must be emptry string."
        else:
