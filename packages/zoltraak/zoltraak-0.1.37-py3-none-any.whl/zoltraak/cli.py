...
def main():
    ...
    parser.add_argument("-r", "--requirements-dir", help="要件定義書の出力ディレクトリ", default="requirements")  # 追加: 要件定義書出力ディレクトリ指定オプション
...
def process_markdown_file(args):
    ...
    requirements_dir = args.requirements_dir                                 # 要件定義書ディレクトリを取得

    # デバッグ用のプリント文を追加
    print("デバッグ: requirements_dir =", requirements_dir)
    print("デバッグ: args.requirements_dir =", args.requirements_dir)
    ...
    mtp = MarkdownToPythonConverter(md_file_path, py_file_path,
                                    prompt, compiler_path, formatter_path, language, requirements_dir)
...
def process_text_input(args):
    text = args.input
    md_file_path = generate_md_file_name(text, args.requirements_dir)
    # print(f"新しい要件定義書 '{md_file_path}' が生成されました。")
    prompt = f"{text}"

    if args.custom_compiler:
        os.system(f"zoltraak {md_file_path} -p \"{prompt}\" -cc {args.custom_compiler} -f {args.formatter} -l {args.language} -r {args.requirements_dir}")
    else:
        os.system(f"zoltraak {md_file_path} -p \"{prompt}\" -c {args.compiler} -f {args.formatter} -l {args.language} -r {args.requirements_dir}")

def generate_md_file_name(prompt, requirements_dir):
