...
==============================================================
    ...
    save_md_content(md_content, target_file_path, requirements_dir)     # 生成された要件定義書の内容をファイルに保存
    print_generation_result(target_file_path, compiler_path, open_file, requirements_dir)                # 生成結果を出力し、open_fileフラグに応じてファイルを開く
...
def save_md_content(md_content, target_file_path, requirements_dir):
    """
...
def print_generation_result(target_file_path, compiler_path, open_file=True, requirements_dir="requirements"):
    """
    要件定義書の生成結果を表示する関数
    ...
    Args:
        ...
        open_file (bool): ファイルを開くかどうかのフラグ（デフォルトはTrue）
    """
    target_file_path = f"{requirements_dir}/{target_file_path}"
    print("")
