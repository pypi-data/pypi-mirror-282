...
def run_latex(tex_file, no_latex_mk=False, quiet=False):
    ...
    if not no_latex_mk and latex_mk:
        return run_exe("latexmk", args=[os.path.basename(tex_file), "-xelatex"], out_file=out_file, cwd=os.path.dirname(os.path.abspath(tex_file)))
    else:
        if not no_latex_mk:
            logger.warn("No LaTeX make system found. Compiling only once.")
        return run_exe("xelatex", args=[os.path.basename(tex_file), "-interaction=nonstopmode"], out_file=out_file, cwd=os.path.dirname(tex_file))
