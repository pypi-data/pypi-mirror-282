...
def job_to_dict(job):
    ...
    if not job.pending:
        ...
        data['next_run_time'] = job.next_run_time.strftime(dt_str_formatter) if isinstance(
            job.next_run_time, datetime) else None
    data["start_date"] = data.get("start_date").strftime(dt_str_formatter) if isinstance(
        data.get("start_date"), datetime) else None
    data["end_date"] = data.get("end_date").strftime(dt_str_formatter) if isinstance(
        data.get("end_date"), datetime) else None
    data["run_date"] = data.get("run_date").strftime(dt_str_formatter) if isinstance(
        data.get("run_date"), datetime) else None
    return data
