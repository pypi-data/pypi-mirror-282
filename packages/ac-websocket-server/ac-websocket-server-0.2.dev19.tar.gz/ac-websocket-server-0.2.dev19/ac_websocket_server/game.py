...
class GameServer(ChildServer):
    ...
    async def __set_laps(self, *args):
        '''Set the number of laps in the server config'''
        laps = args[0][0]
        ...
        try:
            self.cfg.set('RACE', 'LAPS', laps)
            with open(self.server_cfg_file_name, 'w') as configfile:
                self.cfg.write(configfile, space_around_delimiters=False)
            await self.put(Protocol.success({'msg': f'Updated {self.server_cfg_file_name} with LAPS={laps}'}))
        except IOError:
            await self.put(Protocol.error({'msg': 'Failed to set laps'}))
