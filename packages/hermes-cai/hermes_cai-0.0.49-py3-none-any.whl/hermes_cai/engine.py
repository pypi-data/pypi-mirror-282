...
DEFAULT_TEMPLATE_NAME = "production.yml.j2"
...
def build_structured_prefix(
    ...
    if template_name and "/" not in template_name:
        structured_prefix.hermes_generation_template_name = (
            f"./{DEFAULT_TEMPLATE_DIR}/{template_name}"
        )
    ...
    tokenized_context = _build_tokenized_context(prompt)
...
def build_prompt(
    ...
    if not template_name:
        template_name = f"./{DEFAULT_TEMPLATE_DIR}/{DEFAULT_TEMPLATE_NAME}"
    prompt = Prompt(
