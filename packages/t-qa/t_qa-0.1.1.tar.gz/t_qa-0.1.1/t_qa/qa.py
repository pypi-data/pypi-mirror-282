...
class QA(metaclass=SingletonMeta):
    ...
    def __init__(self):
        ...
        self.test_cases: list[TestCase] = None
        self.run_status: str = None
    ...
    def dump(self):
        ...
        try:
            run_data = RunData(
                ...
                empower_env=VARIABLES["environment"],
                run_link=METADATA["processRunUrl"],
    ...
    def __get_run_result(self):
        if self.run_status:
            run_result = self.run_status
        else:
            run_result = report_builder.status.value
        return run_result
