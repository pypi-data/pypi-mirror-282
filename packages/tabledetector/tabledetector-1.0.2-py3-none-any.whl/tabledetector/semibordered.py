...
class SemiborderedTableDetector:
    ...
    def cropTable(self):
        ...
        try:
            contoursL, bboxL = self.sortContours(cnts)
            contours, bbox = self.sortContours(contoursL, method="top-to-bottom")
        except:
            contours = []
            pass

        try:
            contours1, bbox1 = self.sortContours(cnts1, method="top-to-bottom")
        except:
            contours1 = []
            pass
        ...
        if len(contours1) > 1:
            for c1 in contours1[:1] + contours1[-1:]:
                x1, y1, w1, h1 = cv2.boundingRect(c1)
                horBoxes.append([x1, y1, w1, h1])

            X = min(horBoxes[0][0], horBoxes[1][0])
            Y = min(horBoxes[0][1], horBoxes[1][1])
            W = max(horBoxes[0][2], horBoxes[1][2])

            if W < (res.shape[1] * 40) / 100:
                W = round((res.shape[1] * 90) / 100)
                X = round((res.shape[1] * 10) / 100)
            H = int(abs(horBoxes[1][1] + horBoxes[1][3] - horBoxes[0][1]))

            return res[Y-50:Y+H+50,X-50:X+W+50], True
        
        elif len(contours) > 1:
            for c in contours[:1] + contours[-1:]:
                x, y, w, h = cv2.boundingRect(c)
                horBoxes.append([x, y, w, h])

            X = min(horBoxes[0][0], horBoxes[1][0])
            Y = min(horBoxes[0][1], horBoxes[1][1])
            W = int(abs(horBoxes[1][0] + horBoxes[1][2] - horBoxes[0][0]))
            H = int(abs(horBoxes[1][1] + horBoxes[1][3] - horBoxes[0][1]))
    ...
    def imageProcess(self, img, flag):
        cvImg = img
    ...
    def semiborderedMain(self):
        try:
            croppedTable, flag = self.cropTable()
            unborderedPath = self.imageProcess(croppedTable, flag)
