    ...
    for structure in structures:
        ...
        for index in indices:
            if index.lower() == 'mean':
                ...
                mean_dose = total_dose / total_volume
                structure_results[index] = mean_dose
            elif index.lower() == 'min':
                vv_dose = np.array(v_dose)
                dose_bin_centers = (vv_dose[:-1] + vv_dose[1:]) / 2
                structure_results[index] = np.min(dose_bin_centers) if dose_bin_centers.size > 0 else 0
            elif index.lower() == 'max':
                vv_dose = np.array(v_dose)
                dose_bin_centers = (vv_dose[:-1] + vv_dose[1:]) / 2
                structure_results[index] = np.max(dose_bin_centers) if dose_bin_centers.size > 0 else 0
                ...
                try:
                    ...
                    structure_results[index] = interpolated_dose
                ...
                try:
                    ...
                    structure_results[index] = interpolated_volume
                except Exception as e:
    ...
    for structure_name, dvh in dvh_data.items():
        ...
        with open(filename, 'w', newline='') as csvfile:
            ...
            for dose, volume in zip(dvh['dose'], dvh['volume']):
                writer.writerow({'dose': round(dose,2), 'volume': round(volume,2)})
