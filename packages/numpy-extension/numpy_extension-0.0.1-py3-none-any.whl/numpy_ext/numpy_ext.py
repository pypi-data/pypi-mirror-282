...
def apply_from_axis(func, arr: np.ndarray, axis = 0, otypes: Iterable[DTypeLike] = None):
    '''
...
def map_range(
    ...
    min_value: np.ndarray = np.min(arr, axis, keepdims=True)
    max_value: np.ndarray = np.max(arr, axis, keepdims=True)
    max_min_difference = max_value - min_value
    max_min_equal_mask = max_min_difference == 0
    max_min_difference[max_min_equal_mask] = 1
    arr = (arr - min_value)[max_min_equal_mask] = np.asarray(scalar_default_value(scalar_default, eps)).astype(arr.dtype)
    return (arr / max_min_difference * (interval[1] - interval[0]) + interval[0]).astype(dtype)
...
def map_ranges(
    ...
    max_min_equal_mask = max_min_difference == 0
    max_min_difference[max_min_equal_mask] = 1
    normed = (arr - min_value) / max_min_difference
    results = np.empty(len(intervals), dtype=np.ndarray)
    for i, interval in enumerate(intervals):
        normed[max_min_equal_mask] = np.asarray(scalar_default_value(scalar_default, eps)).astype(arr.dtype)
        results[i] = (normed * (interval[1] - interval[0]) + interval[0]).astype(dtype)
    return results
...
np.map_range = map_range
np.map_ranges = map_ranges
np.linspace_at = linspace_at
np.linspace_cumprod_at = linspace_cumprod_at