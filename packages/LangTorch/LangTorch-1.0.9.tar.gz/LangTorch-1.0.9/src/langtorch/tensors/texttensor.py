...
class TextTensor(torch.Tensor, metaclass=_ParameterMeta):
    ...
    def __new__(cls, content="", parse=True, metadata=None,
        ...
        tensor.metadata["_is_gradient"] = is_gradient
    ...
    def add_key(self, keys, inplace=False) -> 'TextTensor':
        ...
        # Apply set_key to each entry
        for index, text_entry in np.ndenumerate(result.content):
            result.content[index].add_key_(reshaped_keys[index].item())
        return result
    ...
    def backward(
        ...
        if grad_tensors is None:
            grad_tensors = tuple(torch.ones_like(t) for t in tensors)
        elif isinstance(grad_tensors, (str, torch.Tensor)):
            grad_tensors = (TextTensor(grad_tensors) if isinstance(grad_tensors, str) else grad_tensors,)
        else:
            grad_tensors = tuple(TextTensor(gt) if isinstance(gt, str) else gt for gt in grad_tensors)
