...
def _make_task_request(queue_name: str, task: Task, host: str, port: int):
    ...
    if task.app_engine_http_request.relative_uri:
        ...
        headers.update({
            ...
            'X-AppEngine-TaskETA': task.schedule_time.ToSeconds() if task.schedule_time else "",
        })
        ...
        headers.update({
            ...
            'X-CloudTasks-TaskETA': task.schedule_time.ToSeconds() if task.schedule_time else "",
        })
    else:
...
class QueueState(object):
    ...
    def submit_task(self, task_name: str, force_run: bool = False) -> Task:
        ...
        def now() -> timestamp_pb2.Timestamp:
            return timestamp_pb2.Timestamp().FromDatetime(datetime.utcnow())
        schedule_time = now()
        dispatch_time = None
        response_time = None
