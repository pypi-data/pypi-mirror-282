...
class POC_matching:
    ...
    def best_val(group_val:list):
        ...
        temp_in = None
        temp_sur = None
        for ind, sur in enumerate(group_val):
            if temp_sur is None:
                temp_in = ind
                temp_sur = sur
            else:
                if len(re.split(r'\/|\-',temp_sur)) < len(re.split(r'\/|\-',sur)):
                    continue
                elif len(re.split(r'\/|\-',temp_sur)) > len(re.split(r'\/|\-',sur)):
                    temp_in = ind
                    temp_sur = sur
                else:
                    old_sur = [ord(i.upper()) for i in temp_sur]
                    new_sur = [ord(i.upper()) for i in sur]
                    if (old_sur <= new_sur)&(len(old_sur) <= len(new_sur)):
                        continue
                    elif (old_sur > new_sur)|(len(old_sur) != len(new_sur)):
                        split_ol = re.split(r'\/|\-',temp_sur)[1]
                        split_new = re.split(r'\/|\-',sur)[1]
                        if (split_ol.isnumeric())&(split_new.isnumeric()):
                            padding = max(len(split_ol),len(split_new))
                            split_ol = split_ol.rjust(padding,'0')
                            split_new = split_new.rjust(padding,'0')
                            old_sur_up = [ord(i.upper()) for i in split_ol]
                            new_sur_up = [ord(i.upper()) for i in split_new]
                            if old_sur_up <= new_sur_up:
                                continue
                            else:
                                temp_in = ind
                                temp_sur = sur
