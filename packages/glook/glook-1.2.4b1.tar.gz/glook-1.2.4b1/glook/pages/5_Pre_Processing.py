...
def apply_winsorization_for_full_df(df, capping_method, tail, fold):
	winsor = None
	try:
...
if sup_unsup == 'Unsupervised Learning':
	...
	preprocessing_action = st.sidebar.radio('Select preprocessing action', [
		...
		'Treat Missing from full DF', 'Change Data Type', 'Drop Duplicates',
	...
	try:
		if preprocessing_action == 'Select Pre-Processing Step⬇️':
			...
			if st.button('Apply', on_click=callback
				...
				pipeline, modified_df = apply_winsorization_for_full_df(modified_df, capping_method, tail, fold)
				st.write('Modified DataFrame:')
				print(344)
				st.write(modified_df)
				print(345)
		...
		elif preprocessing_action == 'Treat Missing from full DF':
			print(409)
	...
	try:
		...
		'Treat Missing from full DF', 'Change Data Type',
