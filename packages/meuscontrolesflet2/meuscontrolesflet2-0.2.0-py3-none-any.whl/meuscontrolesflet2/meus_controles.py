...
class Logs():
    def __init__(self, arquivo_txt):
        ...
        # Custom logging format to handle Brazilian date format
        class CustomFormatter(logging.Formatter):
            def formatTime(self, record, datefmt=None):
                # Format the date to DD-MM-YYYY HH:MM:SS
                return datetime.fromtimestamp(record.created).strftime('%d-%m-%Y %H:%M:%S')

        # Configurar o logger
        file_handler = logging.FileHandler(arquivo_txt)
        ...
        return logger.error("Ocorreu um erro", exc_info=True)
...
class Gestos(ft.Stack):
    ...
    def Add_control(self,nome, control ):
        if isinstance(self.nome_json, dict):
            self.arquiv = self.nome_json
        elif isinstance(self.nome_json, str):
    ...
    def on_pan_update(self, e: ft.DragUpdateEvent):
        ...
        if isinstance(self.nome_json, str):
            self.escrever_json(self.arquiv,self.nome_json)
        e.control.update()
