...
class SGP(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        This automatically infers the `x_api_key` argument from the `SGP_API_KEY` environment variable if it is not provided.
        """
        if x_api_key is None:
            x_api_key = os.environ.get("SGP_API_KEY")
        if x_api_key is None:
            raise SGPError(
                "The x_api_key client option must be set either by passing x_api_key to the client or by setting the SGP_API_KEY environment variable"
            )
