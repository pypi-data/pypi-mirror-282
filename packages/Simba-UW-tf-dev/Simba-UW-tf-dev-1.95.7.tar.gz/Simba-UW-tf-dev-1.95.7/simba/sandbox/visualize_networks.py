...
from simba.utils.checks import check_instance, check_valid_tuple, check_if_dir_exists, check_float, check_int, check_valid_hex_color
...
def visualize(graph: Union[nx.Graph, nx.MultiGraph],
    ...
    if not multi_graph:
        ...
        for node_cnt, node_name in enumerate(graph):
            ...
            if isinstance(palette, dict): node_clr = palette[node_name]
            else: node_clr = clrs[node_cnt]
...
graph = NetworkMixin.create_graph(data={('Animal_1', 'Animal_2'): 1.0, ('Animal_1', 'Animal_3'): 0.2, ('Animal_2', 'Animal_3'): 0.5})
graph_pg = visualize(graph=graph, node_size={'Animal_1': 10, 'Animal_2': 26, 'Animal_3': 50})