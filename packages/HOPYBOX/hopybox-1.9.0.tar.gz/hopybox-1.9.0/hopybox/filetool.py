...
class Editingtool:
  ...
  def edit_new(self,filename):
    global running
    support_lexer = {'.py':PythonLexer,'.sql':SqlLexer,'.html':HtmlLexer}
    if os.path.splitext(filename)[1] in support_lexer:
      session = PromptSession(lexer=PygmentsLexer(support_lexer[os.path.splitext(filename)[1]]))
      text = session.prompt('1 ',multiline=True,key_bindings=bindings,prompt_continuation=self.prompt_continuation)
    else:
      text = prompt('1 ',multiline=True,key_bindings=bindings,prompt_continuation=self.prompt_continuation)
    if text != None:
      with open(filename,'x') as f:
        f.write(text)
      tip_tick(f'Successfully saved the text in {os.path.abspath(filename)}')
    if running == 1:
      running = 0
      if os.path.splitext(filename)[1] == '.py':
        print('\033[95m[Python Code Debug]\033[0m')
        exec(text)
      else:
        if os.path.splitext(filename)[1] == '.html':
          ...
          print("\033[95mServing on port 8000 …\033[0m")
          try:
            httpd.serve_forever()
          except:
            print("\033[95mService has stopped")
