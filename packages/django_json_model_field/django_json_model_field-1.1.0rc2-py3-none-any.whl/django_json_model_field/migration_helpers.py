...
def get_json_model_class(
    apps: Apps,
    model_or_app_label: Type[Model] | Type[JSONModel] | str,
    ...
    *,
    ...
    model, field, selector_value = _get_field_from_args(
        apps=apps,
        model_or_app_label=model_or_app_label,
...
def _get_field_from_args(
    *,
    ...
    if isclass(model_or_app_label):
        if not issubclass(model_or_app_label, (Model, JSONModel)):
            raise TypeError("model must be a Django Model or JSONModel class")
        model = model_or_app_label
