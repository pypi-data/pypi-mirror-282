...
class PanoptoClient:
    ...
    def __init__(
        ...
        self._authentication_controller = PanoptoAuthenticator(
            self._api_config, self._canvas_client
        )
    ...
    def _add_authentication_detector_hook(self, client: httpx.AsyncClient) -> None:
        client.event_hooks["response"].append(
            PanoptoUnauthenticatedDetectorHook(self._api_config.host)
        )

    def get_viewer_url(self, video_id: str) -> URL:
        return self._api_config.get_endpoint(f"Panopto/Pages/Viewer.aspx?id={video_id}")
