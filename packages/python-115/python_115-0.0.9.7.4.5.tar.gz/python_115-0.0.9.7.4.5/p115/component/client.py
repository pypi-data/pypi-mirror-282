        ...
        **request_kwargs, 
        ...
        :param use_anxia_api: 是否使用 https://v.anxia.com 的接口
            ...
            - 如果为 True，使用 "https://v.anxia.com/webapi/files/video"
                - 使用这个接口前，请先请求一次 f"https://v.anxia.com/?pickcode={pickcode}&share_id=0"，否则报错 403
        :param async_: 是否异步
        ...
        **request_kwargs, 
        ...
        :param use_anxia_api: 是否使用 https://v.anxia.com 的接口
            - 如果为 False（默认），使用 f"http://115.com/api/video/m3u8/{pickcode}.m3u8?definition={definition}"
                - 使用这个接口得到的 m3u8 文件，其中包含几个不同分辨率的视频的 m3u8 文件链接
                - 每个不同分辨率的 m3u8 文件，里面各自包含一系列的 ts 视频文件的链接（需要和请求 m3u8 文件时的 User-Agent 一致），ts 文件的链接省略了域名（一般是 "https://cpats01.115.com"，请根据具体情况进行拼接）
            - 如果为 True，使用 f"https://v.anxia.com/site/api/video/m3u8/{pickcode}.m3u8?definition={definition}"
                - 使用这个接口前，请先请求一次 f"https://v.anxia.com/?pickcode={pickcode}&share_id=0"，否则报错 403
                - 这个接口不要求 User-Agent 一致
        :param async_: 是否异步
        :param request_kwargs: 其它请求参数

        :return: 接口返回值

        # 另外还有个接口也可以获取不检查 User-Agent 的链接，并且还不限设备（上面的接口都需要 web 的 cookies），但需要破解里面一个 rsa 请求参数的生成方法
