...
class OpenAPISpec:
    ...
    @staticmethod
    def _patch(swagger_scheme: dict):
        """
    ...
    def _get_spec_by_url(self) -> Union[dict, None]:
        client = httpx.Client()
        try:
            response = client.get(self.spec_path, timeout=5)
    ...
    def _process_method(self, path: str, method: str, details: dict) -> None:
        ...
        self.api_tags.extend([tag for tag in tags if tag not in self.api_tags])
