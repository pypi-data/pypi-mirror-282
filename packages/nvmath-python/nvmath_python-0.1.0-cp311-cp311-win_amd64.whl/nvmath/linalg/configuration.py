...
__all__ = ['Epilogue', 'MatmulInnerShape', 'MatmulOptions', 'MatmulPreferences', 'matrix_qualifiers_dtype', 'NumericalImplFlags']
...
Epilogue = cublaslt.Epilogue
MatmulInnerShape = cublaslt.MatmulInnerShape

# TODO: some Matmul options that are executor (GPU, CPU) dependent will need to be moved to the executor later.
...
class NumericalImplFlags(IntEnum):
    """
...
@dataclasses.dataclass
class MatmulPreferences:
    """A data class for providing options to the :meth:`Matmul.plan` method and the wrapper function :func:`matmul`.

    Attributes:
        reduction_scheme_mask (cublaslt.ReductionScheme) : Enumerators from cublaslt.ReductionScheme combined with |. The
            default is all reduction schemes.
        ...
        numerical_impl_mask (linalg.NumericalImplFlags) : Enumerators from linalg.NumericalImplFlags combined with |. The
            default is all numerical implementation flag choices.
    ...
    numerical_impl_mask : Optional[NumericalImplFlags] = (1 << 64) - 1
    limit : int = 8

    #TODO: Validate inputs in post_init
    def __post_init__(self):
...
# TODO: pretty print algorithm capabilities with docstring.
