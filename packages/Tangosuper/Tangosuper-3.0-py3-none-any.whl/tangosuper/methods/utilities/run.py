...
import tangosuper
from tangosuper.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~tangosuper.Client.start`, :meth:`~tangosuper.idle` and :meth:`~tangosuper.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that Tangosuper's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`tangosuper.compose`.
