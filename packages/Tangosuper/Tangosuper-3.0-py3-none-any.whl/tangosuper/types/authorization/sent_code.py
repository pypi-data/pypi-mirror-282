...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~tangosuper.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~tangosuper.Client.sign_in` or :meth:`~tangosuper.Client.sign_up`).

        next_type (:obj:`~tangosuper.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~tangosuper.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~tangosuper.Client.resend_code`.
    """
