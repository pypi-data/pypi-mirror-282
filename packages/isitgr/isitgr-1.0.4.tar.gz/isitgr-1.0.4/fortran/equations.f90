    ...
    dgpi_3wplus1,dgpi_3wplus2,dgpi_3wplus1plusbetak) !CGQ
	!< ISiTGR MOD END
    implicit none
    type(EvolutionVars) EV
    ...
    !functional form expression
        ...
        if (CP%ISiTGR_growth_index) then
            omegam_t = ((CP%ombh2+CP%omch2+CP%omnuh2)/(CP%H0/100)**2) * a**(-3.d0)  / ( ((CP%ombh2+CP%omch2+CP%omnuh2)/(CP%H0/100)**2) &
            * a**(-3.d0) + (State%Omega_de))
            !((CP%ombh2+CP%omch2+CP%omnuh2)/(CP%H0/100)**2) * a**(-3.d0) * ((CP%H0*1000.d0/c)/(adotoa/a))**2.d0
    ...
    real(dl) :: mudot_MG
    real(dl) :: s1_k, s2_k, s1_k_dot, s2_k_dot, omegav, omegam_t, omegam_dot_t
    ...
    !functional form expression
        ...
        if (CP%ISiTGR_growth_index) then
            omegam_t = ((CP%ombh2+CP%omch2+CP%omnuh2)/(CP%H0/100)**2) * a**(-3.d0) / ( ((CP%ombh2+CP%omch2+CP%omnuh2)/(CP%H0/100)**2) &
            * a**(-3.d0) + (State%Omega_de))
            !* ((CP%H0*1000.d0/c)/(adotoa/a))**2.d0
    
            omegam_dot_t=-3.d0*omegam_t**2.d0*a**3.d0*adotoa*(1-((CP%ombh2+CP%omch2+CP%omnuh2)/(CP%H0/100)**2)) &
            & /(((CP%ombh2+CP%omch2+CP%omnuh2)/(CP%H0/100)**2))
    
            mudot_MG = (2.d0/3.d0*omegam_t**(CP%gamma_L-1.d0)*&
            (omegam_t**CP%gamma_L+2.d0-3.d0*CP%gamma_L+3.d0*(CP%gamma_L-0.5d0)*omegam_t))/omegam_t * &
            (CP%gamma_L-1.d0)*omegam_dot_t+2.d0/3.d0*omegam_t**(CP%gamma_L-1.d0)*omegam_dot_t*&
            (CP%gamma_L*omegam_t**(CP%gamma_L-1.d0)+3.d0*(CP%gamma_L-0.5d0))
            !mudot_MG = 2.d0*(omegam_t-1.d0)*omegam_t**(CP%gamma_L-1.d0)*adotoa* ( (2.d0*CP%gamma_L-1.d0)*omegam_t**CP%gamma_L + &
            !3.d0*omegam_t*CP%gamma_L*(CP%gamma_L-0.5d0) + (CP%gamma_L-1.d0)*(2.d0-3.d0*CP%gamma_L) )
        else
              !adding an extra factor for scale dependence
              s2_k = (CP%lambda*(adotoa/a)/k)**2.d0
              s1_k = (1.d0+CP%c1*s2_k)/(1.d0+s2_k)
    ...
    if (CP%GR/=1) then
        ...
        ! 3) Get sigmadot and Weyl potential
    ...
    if (associated(EV%OutputTransfer) .or. associated(EV%OutputSources)) then
        ...
        if (associated(EV%OutputTransfer)) then
            ...
            EV%OutputTransfer(Transfer_Weyl) = k2*phi ! CGQ: no phi is (Phi+Psi)/2 in the MG case 
            !< ISiTGR MOD END
