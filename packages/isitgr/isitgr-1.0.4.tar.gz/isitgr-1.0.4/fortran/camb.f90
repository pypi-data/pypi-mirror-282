	...
	logical :: Binning_Method, Hybrid_Binning, scale_bins
	!< ISiTGR MOD END
    ...
    scale_bins = Ini%Read_Logical('scale_bins', .false.)
    ...
    if (Binning_Method) then
        ...
        !whether to bin in redshift-only or redshift-scale
        if (scale_bins) then
		    P%ISiTGR_BIN_scale_bins = .true.
		    write(*,*) 'Binning in redshift and scale'
	    else
		    P%ISiTGR_BIN_scale_bins = .false.
		    write(*,*) 'Binning in redshift-only'
	    end if

    	!whether to use hybrid binning or traditional binning
	    if (Hybrid_Binning) then
		    P%trad_bin = .false.
		    write(*,*) 'Hybrid method'
	    else
		    P%trad_bin = .true.
        ...
        !for MG parameters mu, eta, with eta=1 and using mu in terms of the growth index
        else if (Parameterization == 5) then
            P%ISiTGR_mueta = .true.
            P%ISiTGR_growth_index = .true.
	    	write(*,*) '        growth index parametrization'
		    P%gamma_L = Ini%Read_Double('gamma_L')
		    write(*,*) 'growth index =', P%gamma_L
