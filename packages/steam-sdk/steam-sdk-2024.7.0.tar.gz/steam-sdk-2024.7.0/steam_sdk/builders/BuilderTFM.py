...
class BuilderTFM:
    ...
    def calculate_Wedge(self, T: float):
        ...
        P_tot, I_tot, tau_W, frequency, tau_index = self.interpolate(rho=rho_W, case='Wedge')
        ...
        M_W = np.transpose(np.ones(M_W.shape).transpose() * M_W[tau_index])
    ...
    def calculate_CB(self, T: float):
        ...
        P_tot, I_tot, tau_CB, frequency, tau_index = self.interpolate(rho=rho_CB, case='CB')
        w = 2 * np.pi * frequency
        P_tot = P_tot * self.General.magnet_length * 2
    ...
    def interpolate(self, rho: np.ndarray, case: str) -> np.ndarray:
        ...
        return P_tot, I_tot, tau, frequency, tau_index
    ...
    def calculate_Mutual_Coupling(self, Mutual_dict: dict):
        ...
        key_valid = ['M_ISCC_Wedge', 'M_ED_Wedge', 'M_IFCC_Wedge', 'M_PC_Wedge']
...
def calculate_tau_index(P_tot: np.ndarray, frequency: np.ndarray) -> int:
    '''
