...
class JiraCloudManifestAdapter(ManifestAdapter):
    ...
    def get_issues_count(self) -> int:
        ...
        pull_from = (
            '0' if self.config.full_redownload else self.config.earliest_issue_dt.strftime('%Y-%m-%d')
        )
    ...
    def get_issues_count_for_project(self, project_id: int) -> int:
        ...
        return self._get_jql_search(
            jql_search=f"project = {project_id} AND updatedDate > {pull_from}", max_results=0,
        )['total']
    ...
    def get_last_updated_for_project(self, project_id: int) -> datetime:
        try:
            most_recent_issue_from_api: dict = self._get_jql_search(
                jql_search=f"project = {project_id} ORDER BY updated", max_results=1,
            )['issues'][0]
