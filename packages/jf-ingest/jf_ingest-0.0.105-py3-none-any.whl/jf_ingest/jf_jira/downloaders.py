...
def fetch_id_to_key_for_all_existing(
    ...
    for proj_id in project_id_to_issues.keys():
        ...
        jql_expression = f"project = {proj_id} ORDER BY id ASC"
...
def get_issue_list_to_download(
    ...
    * deletes since last pull:
    ...
    valid_project_ids = {int(project_id) for project_id in project_key_to_id.values()}
    project_to_issue_map = {key: set() for key in valid_project_ids}
    for issue in jellyfish_issue_metadata:
        project_id_from_issue = int(issue.project_id)
        if project_id_from_issue not in valid_project_ids:
            continue
    ...
    id_new = id_on_remote.difference(id_on_local)
    id_to_key_new = {str(jira_id): id_to_key_on_remote[jira_id] for jira_id in id_new}
    # these are considered updated, and would have shown up in jql query with `updated > {last_pull_dt}`
    id_to_key_updated.update(id_to_key_new)
    logger.info(f"{len(id_to_key_new)} issues found on remote not found on local. These will be downloaded.")
