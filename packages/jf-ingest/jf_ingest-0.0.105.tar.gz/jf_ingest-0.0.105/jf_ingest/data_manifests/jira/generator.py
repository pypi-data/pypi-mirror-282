...
from jf_ingest.data_manifests.jira.manifest_base import IJiraDataManifest, IJiraProjectManifest
...
def create_manifest(manifest_adapter: ManifestAdapter,
                    classifications_name_lookup,
                    reset_current_tenant: callable,
                    create_jira_data_manifest: callable,
                    create_jira_project_manifest: callable) -> IJiraDataManifest:
    ...
    with tqdm(total=total_threads) as pbar:
        with ThreadPoolExecutor(max_workers=config.issue_download_concurrent_threads) as executor:
            ...
            generate_base_manifest_future = executor.submit(
                process_global_jira_data, manifest_adapter, create_jira_data_manifest, reset_current_tenant
            )
            # Add callback
            ...
            for future in project_future_to_project_key.keys():
                ...
                if future.exception():
                    project_keys_to_errors[
                        project_future_to_project_key[future]
                    ] = future.exception()
                else:
...
def generate_project_manifest(
    ...
    try:
        ...
        return create_jira_project_manifest(
            ...
            excluded=excluded,
            pull_from=manifest_adapter.config.earliest_issue_dt,
...
def process_global_jira_data(manifest_adapter: ManifestAdapter,
                             create_jira_data_manifest: callable,
                             reset_current_tenant: callable) -> IJiraDataManifest:
    # Need to reset tenancy when spinning up threads
    reset_current_tenant()
