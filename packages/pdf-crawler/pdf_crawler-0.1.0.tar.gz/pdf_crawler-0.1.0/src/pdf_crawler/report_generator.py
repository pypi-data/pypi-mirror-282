...
class ReportGenerator:
    def __init__(self, report_pdf, published_evidence_folder, witness_evidence_folder, console_text):
        ...
        self.font_path_roman = "fonts/FrutigerLTStd-Roman.ttf"
        self.font_path_bold = "fonts/FrutigerLTStd-Bold.ttf"
        self.font_path_italic = "fonts/FrutigerLTStd-Italic.ttf"
    ...
    def contains_images(self, page):
        ...
        Returns:
            bool: True if the page contains images, otherwise False.
        """
    ...
    def concatenate_pdfs(self, pdf_list, output_path, published_evidence=False, witness_evidence=False):
        """
        Concatenates multiple PDF files into a single PDF with optional features for tracking question numbers and start pages.

        Args:
            pdf_list (list): List of paths to the PDF files to be concatenated.
            output_path (str): Path where the concatenated PDF will be saved.
            published_evidence (bool): Flag to indicate if published evidence processing is required. Default is False.
            witness_evidence (bool): Flag to indicate if witness evidence processing is required. Default is False.

        Returns:
            dict or tuple or None: Depending on the flags, returns different structures:
                - If only published_evidence is True: dict mapping PDF paths to their start pages.
                - If only witness_evidence is True: tuple containing question ranges, start pages, and question numbers.
                - If both flags are False: None.
        """
        writer = PdfWriter()
        page_indices = []
        ...
        for pdf in pdf_list:
            with open(pdf, "rb") as input_pdf:
                ...
                for page_number, page in enumerate(reader.pages):
                    ...
                    contains_text = bool(page.extract_text().strip())
                    contains_images = self.contains_images(page)

                    if contains_text or contains_images:
                        ...
                        if page.get('/RotateProcessed') is None:
                            ...
                            if page_width > page_height and rotation_angle == 0:
                                print("Page needs rotation to portrait orientation")
        ...
        with fitz.open(input_pdf) as pdf_file:
            for page_index in range(len(pdf_file)):
                ...
                if rotation == 0:
                    ...
                    print("page is rotated 270 deg adjusting EV position")
