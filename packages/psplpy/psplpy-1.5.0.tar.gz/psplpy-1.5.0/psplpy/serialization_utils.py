...
class Compressor:
    ...
    def compress(self, data: bytes, lib: ModuleType = None) -> bytes:
        lib = lib or self.lib
        compressed_data = lib.compress(data)
        return compressed_data

    def decompress(self, compressed_data: bytes, lib: ModuleType = None) -> bytes:
        lib = lib or self.lib
        data = lib.decompress(compressed_data)
        return data
...
class CompressSerializer:
    ...
    def __init__(self, compress_lib: ModuleType = Compressor.ZLIB, threshold: int = 1024 * 128):
