...
rotation_model_path = os.path.join(models_dir, 'rotation_model.onnx')
table_detect_model_path = os.path.join(models_dir, 'table_detect_model.pt')
cell_detect_model_path = os.path.join(models_dir, 'cell_detect_model.pt')

rotation_model = ort.InferenceSession(rotation_model_path)
...
table_detector = YOLO(table_detect_model_path)
cell_detector = YOLO(cell_detect_model_path)
...
def is_text_selectable(pdf_path):
    ...
    try:
        ...
        text_content_threshold = 100

        for page_num in range(len(pdf_document)):
            page = pdf_document.load_page(page_num)
            text = page.get_text("text")
            if len(text.strip()) > text_content_threshold:
                common_signature_phrases = ["Digitally signed", "Signature", "Certified by"]
