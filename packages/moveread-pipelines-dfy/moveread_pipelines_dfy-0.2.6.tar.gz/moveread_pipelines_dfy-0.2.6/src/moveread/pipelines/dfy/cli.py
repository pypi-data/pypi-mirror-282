...
def main():
  ...
  parser.add_argument('--blobs', **env('BLOBS_CONN_STR', required=True), type=str, help='Connection string for blobs storage') 
  ...
  parser.add_argument('--api-workers', default=1, type=int, help='Number of API workers')
  ...
  blobs = KV.of(args.blobs)
  images_path = blobs.base_path if isinstance(blobs, FilesystemKV) else None
  if images_path:
    os.makedirs(images_path, exist_ok=True)
  params = DFYPipeline.Params(logger=logger, token=args.token, tfserving=tfparams, blobs=blobs, images_path=images_path, **storage)
  ...
  artifs.api.mount('/blobs', kv.rest.api(blobs))
  ...
  ps['api'] = Process(target=uvicorn.run, args=(artifs.api,), kwargs={'host': args.host, 'port': args.port})
...
if __name__ == '__main__':
  ...
  args = '-b demo --port 8324 --logs demo/logs --blobs file://demo/images --api-workers 2'
