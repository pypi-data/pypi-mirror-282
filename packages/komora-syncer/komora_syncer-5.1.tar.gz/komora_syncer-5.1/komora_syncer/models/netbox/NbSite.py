...
class NbSite(NetboxBase):
    def __init__(self, komora_obj):
        ...
        try:
            ...
            if komora_org_id:
                self.tenant = self.nb.tenancy.tenants.get(
    ...
    def find(self):
        ...
        if self.komora_id is not None:
            ...
            try:
                if self.komora_id in site_cache:
                    netbox_site = site_cache[self.komora_id]
                else:
                    netbox_site = self.nb.dcim.sites.get(cf_komora_id=self.komora_id)
        ...
        try:
            netbox_site = self.nb.dcim.sites.get(
                name__ie=self.name)
            if netbox_site:
                logger.warning(
