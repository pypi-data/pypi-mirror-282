...
class NbDevice(NetboxBase):
    ...
    def find(self):
        ...
        try:
            ...
            logger.exception(
                f"Unable to get Device by name: {self.name}. Error: {e}")
    ...
    def update(self, nb_device):
        try:
            ...
            logger.exception(
                f"Unable to update device {self.name}. Error: {e}")
    ...
    def get_nb_devices_data():
        ...
        filter_device_roles = list(nb.dcim.device_roles.filter(
            name=["Passive component", "unknown", "Server"]))
        filter_device_role_ids = [device.id for device in filter_device_roles]

        devices = list(nb.dcim.devices.filter(status="active",
                       name__empty=False, role_id__n=filter_device_role_ids))
        ...
        unique_tenant_ids = {
            device.tenant.id for device in devices if device.tenant}
        tenants = list(nb.tenancy.tenants.filter(
            tenant_id__in=list(unique_tenant_ids)))
                ...
                "device_list": [
                    {
                        ...
                        "location": NbDevice.get_info_from_cache(location_cache, device.location.id) if device.location else None,
                        "tenant": NbDevice.get_info_from_cache(tenant_cache, device.tenant.id) if device.tenant else None,
                        "site": NbDevice.get_info_from_cache(site_cache, device.site.id, include_tenant=True) if device.site else None,
