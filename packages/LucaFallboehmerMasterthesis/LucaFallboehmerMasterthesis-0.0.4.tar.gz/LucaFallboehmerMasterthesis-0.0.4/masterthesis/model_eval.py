...
def eval_by_threshold(model, threshold_list, X_test, y_test, device="cpu"):
    ...
    for count, threshold in enumerate(threshold_list):
...
def plot_eval_by_threshold(score_by_threshold):
    # get at which threshold metric is max
    max_acc, thr_max_acc = (
    ...
    fig, ax = plt.subplots(3, 2, figsize=(20, 20), dpi=200)
    ax[0][0].plot(score_by_threshold["threshold_roc"], score_by_threshold["accuracy"])
    ax[0][0].axvline(
        x=thr_max_acc,
        color="purple",
    ...
    ax[0][0].set_title("accuracy by threshold")

    ax[0][1].plot(score_by_threshold["threshold_roc"], score_by_threshold["f1"])
    ax[0][1].axvline(
        x=thr_max_f1,
        color="purple",
    ...
    ax[0][1].set_title("f1 by threshold")

    ax[1][0].plot(score_by_threshold["threshold_roc"], score_by_threshold["recall"])
    ...
    ax[1][0].set_title("recall (sensitivity) by threshold")

    ax[1][1].plot(score_by_threshold["threshold_roc"], score_by_threshold["fall-out"])
    ax[1][1].scatter(
        score_by_threshold["threshold_roc"], score_by_threshold["fall-out"]
    )
    ax[1][1].set_xlabel("threshold")
    ax[1][1].set_ylabel("fallout (false positive rate)")
    ax[1][1].set_title("fallout by threshold")

    ax[2][0].plot(score_by_threshold["fall-out"], score_by_threshold["recall"])
    ax[2][0].scatter(score_by_threshold["fall-out"], score_by_threshold["recall"])
    ax[2][0].set_xlabel("fallout (false positive rate)")
    ax[2][0].set_ylabel("recall (sensitivity)")
    ax[2][0].set_title("ROC Curve")

    ax[2][1].plot(score_by_threshold["recall"], score_by_threshold["precision"])
    ax[2][1].scatter(score_by_threshold["recall"], score_by_threshold["precision"])
