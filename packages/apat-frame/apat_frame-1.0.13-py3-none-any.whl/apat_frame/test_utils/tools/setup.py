...
from apat_frame.test_utils.tools.commonpath import commonpath

def setupforuse():
    """
    设置环境，用于确保Playwright及其依赖的浏览器（Chromium、Firefox、WebKit）已安装并准备就绪。
    同时解压缩特定的Appium和Allure插件。
    """
    """
    ...
    """
    # 尝试安装Playwright及其支持的浏览器（Chromium、Firefox、WebKit）
    try:
    ...
    # 尝试解压缩Appium和Allure相关文件
    try:
        filepath = commonpath()

        # 初始化PublicZip对象，用于解压缩Appium和Allure相关文件
        setzip_appium = PublicZip(filepath.get_appium_path(),filepath.get_appium_7z_path(),filepath.get_plugins_path())
        setzip_allure = PublicZip(filepath.get_allure_path(),filepath.get_allure_7z_path(),filepath.get_plugins_path())

        # 执行解压缩操作
        setzip_appium.extract()
        setzip_allure.extract()
        print("相关项解压成功。")
...
# 确保当脚本直接被执行时（而非作为模块导入时）才运行 setupforuse 函数
