...
output_file = cp.get_appium_7z_path()  # 压缩文件路径
...
if check_file_exists(input_dir):
    ...
    print(f"解压文件路径: {output_file}")
    if check_file_exists(output_file):
        create_directory(output_file)  # 创建输出目录
        clear_directory(output_file)   # 清空输出目录
        extract_7z(seven_zip_path, output_file)
        print("解压完成")
    else:
