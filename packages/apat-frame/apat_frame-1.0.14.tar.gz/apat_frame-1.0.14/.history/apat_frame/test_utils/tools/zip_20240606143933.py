...
class PublicZip:
    def __init__(self,input_file,output_dir,freepath):
        ...
        self.output_dir = output_file  #压缩文件路径
    ...
    def extract(self):    
        if PublicZip.check_file_exists(self.input_dir):
            print(f"文件 '{self.input_dir}' 存在。")
        else:
            print(f"文件 '{self.input_dir}' 不存在。")
            print(f"解压文件路径: {self.output_dir}")
            PublicZip.extract_7z(self.output_dir, self.freepath)
            print("解压完成")
