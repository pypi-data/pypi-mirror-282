...
class DeviceContext(InstanceContext):
    ...
    def read_property_historical(self, property_name, query):
        device_id = self._solution['id']
        return self._version.fetch(HTTP_POST, f'data/devices/{device_id}/properties/{property_name}/query', data=query)

    def read_events(self, page_size, page_no, admin_auth):
        device_id = self._solution['id']
        return self._version.fetch(HTTP_GET, f'sys/data/devices/{device_id}?pageSize={page_size}&pageNo={page_no}', auth=admin_auth)
