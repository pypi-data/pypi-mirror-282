...
class DeviceList(ListResource):
    ...
    def page(self, friendly_name=values.unset, profile_id=values.unset, page_number=values.unset,
        ...
        return DevicePage(self._version, response, self._solution, options=params)
...
class DevicePage(Page):
    ...
    def __init__(self, version, response, solution, options=None):
        """
        ...
        super(DevicePage, self).__init__(version, response, options)
    ...
    def mk_page_url_path(self):
        ...
        :return: 
        """
        url = '{}/accounts/{}/devices?pageNo={}&pageSize={}'
        if self.options is not None and 'is_a' in self.options:
            url += '&is_a={}'.format(self.options['is_a'])
        return url
...
class DeviceContext(InstanceContext):
    ...
    def __init__(self, version, account_id, id):
        ...
        self._data_uri = '/accounts/{account_id}/data/devices/{id}'.format(**self._solution)
        self._apikey = None
    ...
    def insert_property_value(self, payload, use_device_key=False):
        ...
        if(not use_device_key):
            return self._version.fetch(HTTP_POST, self._data_uri, data=payload)

        return self._version.fetch(HTTP_POST, self._data_uri, data=payload, auth=(self.apikey.id, self.apikey.secret))

    def call_method(self, payload, method_name):
        device_id = self._solution[self.ID_KEY]
        return self._version.fetch(HTTP_POST, f'{self._data_uri}/methods/{method_name}', data=payload)

    def read_property_historical(self, property_name, query):
        device_id = self._solution['id']
        return self._version.fetch(HTTP_POST, f'{self._data_uri}/properties/{property_name}/query', data=query)
    ...
    def state(self):
        ...
        """
        device_id = self._solution[self.ID_KEY]
        return self._version.fetch(HTTP_GET, self._data_uri)
