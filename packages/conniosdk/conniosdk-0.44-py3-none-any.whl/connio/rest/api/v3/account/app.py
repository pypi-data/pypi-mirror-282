...
class AppContext(InstanceContext):
    ...
    def __init__(self, version, account_id, id):
        ...
        self._data_uri = '/accounts/{account_id}/data/apps/{id}'.format(**self._solution)
    ...
    def state(self):
        ...
        return self._version.fetch('GET', self._data_uri)

    def insert_property_value(self, payload, auth_key=None):
        app_id = self._solution['id']
        if auth_key is None:
            return self._version.fetch('POST', self._data_uri, data=payload)
        return self._version.fetch('POST', self._data_uri, data=payload, auth=auth_key)
        
    def call_method(self, payload, method_name):
        app_id = self._solution['id']
        return self._version.fetch('POST', f'{self._data_uri}/methods/{method_name}', data=payload)

    def read_property_historical(self, property_name, query):
        app_id = self._solution['id']
        return self._version.fetch('POST', f'{self._data_uri}/properties/{property_name}/query', data=query)
