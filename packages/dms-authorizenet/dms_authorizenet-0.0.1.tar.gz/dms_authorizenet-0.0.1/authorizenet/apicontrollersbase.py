...
class APIOperationBase(APIOperationBaseInterface):
    ...
    def buildrequest(self):
        ...
        xmlRequest = self._request.toxml(
            encoding=constants.xml_encoding, element_name=self.getrequesttype()
        )
    ...
    def execute(self):
        ...
        proxyDictionary = {
            "http": utility.helper.getproperty("http_proxy"),
        ...
        try:
            ...
            anetLogger.error(
                "Error retrieving http response from: %s for request: %s",
                self.endpoint,
                self.getprettyxmlrequest(),
            )
            anetLogger.error(
                "Exception: %s, %s", type(httpException), httpException.args
            )
        ...
        if self._httpResponse:
            ...
            try:
                ...
                # objectify code
                xmlResponse = self._response.toxml(
                    encoding=constants.xml_encoding, element_name=self.getrequesttype()
                )
                ...
                anetLogger.error(
                    "Create Document Exception: %s, %s",
                    type(objectifyexception),
                    objectifyexception.args,
                )
                ...
                if type(self.getresponseclass()) != type(self._mainObject):
                    ...
                    domResponse = xml.dom.minidom.parseString(
                        self._httpResponse.encode("utf-8")
                    )
                    anetLogger.debug(
                        "Received response: %s"
                        % domResponse.toprettyxml(encoding="utf-8")
                    )
                else:
                    # Need to handle ErrorResponse
                    anetLogger.debug(
                        "Error retrieving response for request: %s" % self._request
                    )
        else:
