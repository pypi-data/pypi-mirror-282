...
QDial, QDialog, QGridLayout, QGroupBox, QHBoxLayout, QLabel, QLineEdit, QFileDialog,
...
import json, subprocess, threading, time, os, sys, py_compile, pkgutil
...
def updateAnalysis():
    ...
    hasSelection, hasObservable = False, False
        ...
        while True:
            ...
            if hasattr(self.runModule, 'process'):
                ...
                if self.status:
                    ...
                    if os.path.isfile(hdir+'/result.json'):
...
class RunModule(threading.Thread):
    def __init__(self, evt, dpi):
        ...
        self.target = 'tt' if not settings else settings['Target']
    ...
    def run(self):
        ...
        ' --energy=\"'+str(self.energy)+'\" --detector=\"'+str(self.detector)+'\" --target=\"'+str(self.target)+'\" --data=\"'+str(self.data)+'\" --dpi=\"'+str(self.dpi)+'\"']
...
class WidgetGallery(QDialog):
    ...
    def createCodeWindow(self):
        ...
        codeSelection.setFixedHeight(170)
        ...
        codeAnalysis.setFixedHeight(170)
