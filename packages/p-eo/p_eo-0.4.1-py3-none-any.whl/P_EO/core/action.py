...
class Action(metaclass=abc.ABCMeta):
    ...
    def click(self, wait=Default.ACTION_WAIT, check_displayed=True, check_enabled=True):
        ...
        if check_displayed and not self.displayed:
            ...
            raise ElementNotDisplayedError(desc=self.describe, loc=self.loc, method=self.method)
        ...
        if check_enabled and not self.enabled:
            ...
            raise ElementNotEnableError(desc=self.describe, loc=self.loc, method=self.method)
    ...
    def wait(self, timeout=Default.TIMEOUT, interval=Default.INTERVAL):
        ...
        raise ElementFindTimeoutError(desc=self.describe, loc=self.loc, method=self.method)
