...
!    Healy tranformation routines by expanding a field in the space domain,
!    transforming this to spherical harmonics, and comparing the relative error
!    of the coefficients.
!
!    Copyright (c) 2006, SHTOOLS
!    All rights reserved.
!
!------------------------------------------------------------------------------
    use SHTOOLS
    ...
    do
        ...
        if (sampling == 1) then
            call MakeGridDH(griddh(1:n, 1:n), n, &
                            cilm(1:2,1:lmax+1, 1:lmax+1), lmax, &
                            sampling=sampling)
        else
            call MakeGridDH(griddh(1:n, 1:2*n), n, &
                            cilm(1:2,1:lmax+1, 1:lmax+1), lmax, &
                            sampling=sampling)
        end if
        ...
        if (sampling == 1) then
            call SHExpandDH(griddh(1:n, 1:n), n, &
                            cilm2(1:2, 1:lmax+1, 1:lmax+1), lmaxout, &
                            sampling=sampling)
        else
            call SHExpandDH(griddh(1:n, 1:2*n), n, &
                            cilm2(1:2, 1:lmax+1, 1:lmax+1), lmaxout, &
                            sampling=sampling)
        end if
