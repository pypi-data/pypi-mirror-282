subroutine CilmPlus(cilm, gridin, lmax, nmax, mass, d, rho, gridtype, w, zero,&
                    plx, n, dref, exitstatus)
!------------------------------------------------------------------------------
!
...
!                       gridtype 3 and 4, this must be less than or equal to
!                       N/2 - 1.
!           nmax        Order of expansion.
!           mass        Mass of planet.
!           rho         density of the relief.
!           gridtype    1 = Gauss-Legendre quadrature grid corresponding to
!                       LMAX.
...
!------------------------------------------------------------------------------
    ...
    if (gridtype == 4) then
        ...
        if (present(w) .and. present(zero) .and. present(plx)) then
            ...
            print*, "Error --- CilmPlus"
            print*, "For GLQ grids, either W and ZERO or W " // &
                    "and PLX must be present"
            if (present(exitstatus)) then
                exitstatus = 5
                return
            else
        ...
        if (present(w) .or. present(zero) .or. present(plx)) then
            print*, "Error --- CilmPlus"
            print*, "W, ZERO and PLX can not be present " // &
                    "for Driscoll-Healy grids."
            if (present(exitstatus)) then
                exitstatus = 5
                return
            else
    ...
    if (size(gridin(1,:)) < nlong .or. size(gridin(:,1)) < nlat) then
        print*, "Error --- CilmPlus"
        if (gridtype == 2) then
            print*, "GRIDIN must be dimensioned as " // &
