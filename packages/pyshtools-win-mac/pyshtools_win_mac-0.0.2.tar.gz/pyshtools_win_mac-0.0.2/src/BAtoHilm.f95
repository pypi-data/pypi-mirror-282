subroutine BAtoHilm(cilm, ba, grid, lmax, nmax, mass, r0, rho, gridtype, w, &
...
!                       corresponding to LMAX. 3 = N by 2N Driscoll and
!                       Healy grid corresponding to LMAX.
!
!       OUT
...
!                       (for GRIDTYPE=1, determined from a call to SHGLQ).
!           zero        Array of dimension lmax+1 that contains the latitudinal
!                       gridpoints used in the Gauss-Legendre quadrature
!                       integration scheme (For GRIDTYPE=1). Only needed if
!                       plx is not included.
!           plx         Input array of Associated Legendre Polnomials computed
!                       at the Gauss points (for GRIDTYPE=1, determined from
!                       a call to SHGLQ). If this is not included, then the
...
!                       A value of (0) corresponds to none, (1) corresponds
!                       to the minimum amplitude filter in Wieczorek and
!                       Phillips (1998), whereas as value of (2) corresponds
...
!------------------------------------------------------------------------------
    ...
    if (present(w)) then
        ...
        if (.not. present(plx) .and. .not.present(zero)) then
            print*, "Error --- BAtoHilm"
            print*, "If the optional parameter W is present, then one of " // &
                    "PLX or ZERO must also be present."
            if (present(exitstatus)) then
                exitstatus = 5
                return
            else
    ...
    if (present(filter_type)) then
        if (filter_type /= 0 .and. filter_type /= 1 .and. filter_type /= 2) then
            print*, "Error --- BAtoHilm"
            print*, "FILTER_TYPE must be either 0 (none), 1 " // &
                    "(minimum amplitude), or 2 (minimum curvature)"
    ...
    do n = 2, nmax
        ...
        grid2(1:nlat,1:nlong) = ((grid(1:nlat,1:nlong) - d) / d)**n

        if (gridtype == 1) then
            if (present(plx)) then
                if (present(exitstatus)) then
                    call SHExpandGLQ(cilmn, lmax, grid2(1:nlat,1:nlong), &
                                     w(1:lmax+1), plx = plx(1:lmax+1, &
                                     1:(lmax+1)*(lmax+2)/2), norm = 1, &
                                     csphase = 1, lmax_calc = lmax_calc2, &
                                     exitstatus = exitstatus)
                    if (exitstatus /= 0) return
                else
                    call SHExpandGLQ(cilmn, lmax, grid2(1:nlat,1:nlong), &
