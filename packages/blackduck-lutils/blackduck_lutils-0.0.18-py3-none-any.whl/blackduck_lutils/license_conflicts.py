...
def generate_license_conflict_report(**kwargs):
    ...
    if 'NOASSERTION' in unique_license_list:
        components_without_license = [(c['name'], c['versionInfo']) for c in sbom_data['packages'] if c['licenseConcluded'] == 'NOASSERTION']
    ...
    if not output_file:
        ...
        if xls and not output_file.lower().endswith('.xlsx'):
            output_file += ".xlsx"
