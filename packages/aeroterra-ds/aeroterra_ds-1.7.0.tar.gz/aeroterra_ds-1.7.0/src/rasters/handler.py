...
def clip_tiff(tiff_file, land, save_file=None):
    """
...
def grid_raster(tiff_file, size, save_folder, overlap=0, extremes = Borders.IGNORE, extent=None, extent_crs=None):
    ...
    with rasterio.open(tiff_file) as src:
        ...
        if extent:
            ...
            row_stop, col_stop = (width+1, height+1)
        extent_bbox = (row_start, col_start, row_stop, col_stop)
        while x_pos * size - overlap * x_pos < width:
            while y_pos * size - overlap * y_pos < height:
                ...
                if point_in_bbox((start_x, start_y), extent_bbox):
