...
class ModelTracer():
    def __init__(self, model):
        ...
        self.grads = {n:p.grad.mean() for n, p in self.params.items() if p.grad != None}
        print(">>> Attributes")
        print(".params: parameters-dict / .keys: params' name-list / .no_grad_keys: no-grad-keys-list / .grad_keys: yes-grad-keys-list / .grads: yes-grad-dict")
    ...
    def confess(self, mode=None, grad=None): # mode: None, name, grad
        ...
        if grad == None:
            ...
            if "grad" == mode:
                return {n:p.grad.mean() for n, p in model.named_parameters() if p.grad != None}
        if grad == True:
            if "None" == mode:
...
class GradTracer():
    ...
    def gradgap(self, model, mode="grad", grad=True):
        if self.prev_state == self.now_state: print("GradTracer: same-grad")
        else: 
