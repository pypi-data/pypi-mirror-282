...
_Session = async_sessionmaker(class_=AsyncSession, expire_on_commit=False)
...
async def lifespan(_) -> AsyncGenerator[None, None]:
    ...
    Base.registry.dispose(cascade=True)
    Base.metadata.clear()
...
def new_pagination(
    ...
    def default_dependency(
        ...
        limit=Query(min_page_size, ge=1, le=max_page_size),
    ) -> PaginateType[T]:
