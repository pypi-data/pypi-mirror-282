...
class MLModel:
    def __init__(self, override_config=None, overriden_config_node=None):
        ...
        Args:
            override_config (dict, optional): Configuration to override the default one.
            overriden_config_node (str, optional): Node to override the default one.
        """
    ...
    def register_model(self, props: RegisterModelProps):
        ...
        Args:
            props (RegisterModelProps): Properties of the model to register.
        """
    ...
    def get_active_model_by_group(self, group_id):
        """
        Get the active model by group.

        This method queries the GraphQL API to get the active model for a given group.
        The active model is determined by the 'is_active' flag in the 'mlModelVersionsApis' node.

        Args:
            group_id (str): The ID of the group for which to get the active model.

        Returns:
            dict: The response from the GraphQL API query. This includes details of the active model
            such as the model ID, model location, run ID, version, and associated metrics.
        """
        ml_code = self._config_manager.get_ml_code()
        ml_model_api = self._config_manager.get_ml_model_api()
        query = f"""
    ...
    def get_current_active_model(self):
        ...
        This method retrieves the current group ID from the configuration manager and then calls the
        'get_active_model_by_group' method with this group ID to get the active model.

        Returns:
            dict: The active model for the current group.
        """
        group_id = self._config_manager.get_current_group_id()
    ...
    def create_new_ml_model(self, props: RegisterModelProps):
        ...
        This method constructs a GraphQL mutation query to create a new ML model with the provided properties.
        It then sends this query to the GraphQL API using the web client. The response from the API is parsed
        to get the IDs of the newly created model and its version, which are then used to update the version
        of the model.

        Args:
            props (RegisterModelProps): The properties of the model to create. This includes the run through,
            training features, target columns, additional parameters, description, hyperparameters,
            active status, model location, and metrics.
        """
        ml_model_api = self._config_manager.get_ml_model_api()
        ml_code = self._config_manager.get_ml_code()
    ...
    def update_version(self, ml_model_id, ml_model_versions_id):
        """
        Update the version of the ML model.

        This method constructs a GraphQL query to get the latest version of the ML model and then constructs a GraphQL mutation query to update the version of the model. Both queries are sent to the GraphQL API using the web client.

        Args:
            ml_model_id (str): The ID of the ML model to update.
            ml_model_versions_id (str): The ID of the ML model versions to update.
        """
    ...
    def update_ml_model(self, ml_model_id, props: RegisterModelProps):
        ...
        This method constructs a GraphQL mutation query to update the ML model with the provided properties. It then sends this query to the GraphQL API using the web client. The response from the API is parsed to get the ID of the updated model version, which is then used to update the version of the model.

        Args:
            ml_model_id (str): The ID of the ML model to update.
            props (RegisterModelProps): The properties of the model to update. This includes the run through, training features, target columns, additional parameters, description, hyperparameters, active status, model location, and metrics.
        """
        ml_model_api = self._config_manager.get_ml_model_api()
        run_through = props.run_through
        run_id = self._config_manager.get_current_run_id()
