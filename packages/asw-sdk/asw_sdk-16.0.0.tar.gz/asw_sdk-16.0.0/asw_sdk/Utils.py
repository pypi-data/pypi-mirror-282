...
class Utils:
    ...
    def print(self, message, level=LogLevel.INFO):
        """
        Prints a message at a specified log level.

        This method first generates a log request body. If the message is a dictionary, it is converted to a JSON string.
        The message is then parsed to be JSON compatible. If there is no operations key in the log request body, the message
        is printed and the method returns. Otherwise, a filter log request body string is created and printed. This string
        is then converted to a JSON object and printed. Finally, a POST request is made to the filter progress log endpoint
        with the filter log request body as JSON data.

        Args:
            message (str or dict): The message to be printed. If it is a dictionary, it is converted to a JSON string.
            level (LogLevel, optional): The log level at which to print the message. Defaults to LogLevel.INFO. Available options are INFO, FAILED, and SUCCESS.
