...
```python
util.mint_token(usdc_address, user, expand_number(10, 18))
```
...
- `amount` type int, token amount, with token's decimal, eg: 1 dai is 1000000000000000000
...
- `execution_fee` type int,  execution_fee >= position.min_execution_fee, setting on arbitrum sepolia is 0.0001 eth
