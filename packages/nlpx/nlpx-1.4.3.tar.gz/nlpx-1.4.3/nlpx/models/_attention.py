...
class CNNRNNAttention(nn.Module):
	...
	def __init__(self, embed_dim: int, seq_length: int = 16, kernel_sizes=(2, 3, 4), activation=nn.ReLU(inplace=True),
	             hidden_size: int = 64, num_layers: int = 1, num_heads: int = 1,
	             out_features: int = 2, rnn=nn.GRU, drop_out: float = 0.0):
		"""
		:param embed_dim: RNN的input_size，word embedding维度
		:param hidden_size: RNN的hidden_size, RNN隐藏层维度
		...
		super().__init__()
		self.cnn = CNNLayer(embed_dim, seq_length, kernel_sizes, activation)
		self.attn = RNNAttention(embed_dim, hidden_size, num_layers, num_heads, out_features, rnn, drop_out)
