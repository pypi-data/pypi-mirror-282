...
class EmbeddingClassifier(nn.Module):
	...
	def __init__(self, classifier, embedding=None, vocab_size: int = None, embed_dim: int = None):
		"""
...
class TextCNNClassifier(EmbeddingClassifier):
	...
	def __init__(self, embed_dim: int, num_classes: int, embedding=None, vocab_size: int = None,
...
class CNNRNNAttentionClassifier(nn.Module):
	...
	def __init__(self, embed_dim: int, seq_length: int = 16, kernel_sizes=(2, 3, 4), activation=nn.ReLU(inplace=True),
				 num_classes: int = 2, embedding=None, vocab_size: int = None, hidden_size: int = 64,
				 num_layers: int = 1,num_heads: int = 2, rnn=nn.GRU, drop_out: float = 0.0):
		"""
		:param embed_dim: word embedding维度
		...
		"""
		super().__init__()
		self.embedding = CNNEmbedding(embed_dim, seq_length, kernel_sizes, activation, embedding, vocab_size)
		self.rnn_attn = RNNAttention(embed_dim, hidden_size, num_layers, num_heads, num_classes, rnn, drop_out)
