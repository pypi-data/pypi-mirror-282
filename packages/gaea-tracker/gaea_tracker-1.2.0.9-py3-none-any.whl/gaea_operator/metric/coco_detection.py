...
from gaea_operator.utils import is_list_of
...
class DistCocoEval(COCOeval):
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouThrs=None, areaRng=None, maxDets=None, iouType='segm',
...
class COCOMetric(BaseMetric):
    """
    Calculates the coco metric.
    Args:
        iou_thrs: IoU threshold to compute AP and AR. If not specified, IoUs from 0.5 to 0.95 will be used.
        classwise: Whether to return the computed  results of each class.
        proposal_nums: Numbers of proposals to be evaluated.
    ...
    def gather(self):
        ...
        self._results['bbox'] = self.coco_eval_bbox.eval_imgs_all
        self._results['segm'] = self.coco_eval_segm.eval_imgs_all
        ...
        for key in self._results:
            ...
            for k in range(K):
                for a in range(A):
                    ...
                    # used for debug
                    # print("________len gather res is {}, while len dataset is {}________".format(len(gather_res_list), self.dataset_length))
    ...
    def compute(self):
        ...
        eval_results = {'bbox iou 0.5-0.05-0.95': -1,
                        'bbox iou 0.5': -1, 'bbox iou 0.75': -1}
        if 'bbox' in self._results:
            # used for debug
            # if len(self._results['bbox'][0][0]) > 0 and self._results['bbox'][0][0][-1] is not None:
            #     print("_____len images is {}, last image id is {}_______".format(len(self._results['bbox'][0][0]), self._results['bbox'][0][0][-1]['image_id']))

            self.coco_eval_bbox.eval_imgs_all = self._results['bbox']
            self.coco_eval_bbox.accumulate_rank()
            self.coco_eval_bbox.summarize()

        if self.segm:
            self.coco_eval_segm.eval_imgs_all = self._results['segm']
        ...
        if 'bbox' in self._results:
            ...
            eval_results = {
                'bbox iou 0.5-0.05-0.95': stats[0], 'bbox iou 0.5': stats[1], 'bbox iou 0.75': stats[2]}
        if self.segm:
            stats = self.coco_eval_bbox.stats.tolist()
            eval_results['segm iou 0.5-0.05-0.95'] = stats[0]
            eval_results['segm iou 0.5'] = stats[1]
            eval_results['segm iou 0.75'] = stats[2]
        self.coco_eval_bbox.reset()
        self.coco_eval_segm.reset()
    ...
    def _compute_per_cat_metrics(self, params, precisions, recalls,
        ...
        for idx, category in enumerate(self.categories):
            ...
            self.results_per_category.append(
                dict(categoryName=cate_name, averagePrecision=ap, averageRecall=ar, metrics=[]))

    def save(self, dirname: str) -> None:
        """
        """
        if idist.get_rank() == 0:
            map = self.coco_eval_bbox.stats[0]
            metrics = {"boundingBoxMeanAveragePrecision": map,
                       "boundingBoxMetrics": self.results_per_category,
