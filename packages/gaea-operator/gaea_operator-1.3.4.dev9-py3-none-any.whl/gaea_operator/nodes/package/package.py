...
from gaea_operator.utils import write_file, ModelTemplate
...
def create_model(windmill_client: WindmillClient,
    ...
    if scene is None or len(scene) == 0:
        if response.category["category"] == "Image/Preprocess":
            local_name = format_name(transform_model_name, "pre", start_index=-36)
            display_name = format_name(transform_model_display_name, "预处理", start_index=-80)
        if response.category["category"] == "Image/Postprocess":
            local_name = format_name(transform_model_name, "post", start_index=-36)
            display_name = format_name(transform_model_display_name, "后处理", start_index=-80)
