...
from FactualSceneGraph.src.factual_scene_graph.parser.scene_graph_parser import SceneGraphParser
from FactualSceneGraph.src.factual_scene_graph.evaluation.soft_spice_evaluation import encode_phrases
...
class CAPTUREEvaluator:
    ...
    def eval_pred_list(self, pred_list, prev_gt_parsed=None, prev_cand_parsed=None):
        gts = [(sample_index, gt_answer) for sample_index, sample in enumerate(pred_list) for gt_answer in sample['gt_answers']]
        gt_nums = [len(sample['gt_answers']) for sample_index, sample in enumerate(pred_list)]
        cands = [(sample_index, sample['pred_answer']) for sample_index, sample in enumerate(pred_list)]
        ...
        for gt_num in gt_nums:
            grouped_gt_parsed.append(gt_parsed[cur: cur + gt_num])
            cur += gt_num

        scores = []
        parse_results = []
        for sample_idx in tqdm.trange(len(cand_parsed), desc="computing score"):
            sample_gt_parsed, sample_cand_parsed = grouped_gt_parsed[sample_idx], cand_parsed[sample_idx]
            results = [
...
if __name__ == '__main__':
    ...
    pred_list = [
        {'gt_answers': refs[i], 'pred_answer': preds[i]} for i in range(len(refs))
    ]
