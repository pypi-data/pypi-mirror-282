...
async def wallet_dialog(message: Message, config: BotConfig):
    ...
    if len(message.text.split()) > 2:
        return await message.reply(f"{message.from_user.username}you are doing something wrong!")
    if len(message.text.split()[1]) > 12:
        return await message.reply(f"{message.from_user.username}you entered an invalid wallet!")
    if len(message.text.split()) == 2:
        ...
        if player:
            ...
            user = await User.create(name=tg_username, tg_id=user_id, wallet=address, last_claim = twenty_four_hours_ago.timestamp())
            ...
            await welcome_user(
                bot=config.bot, 
                message_text=f"A new user joined the Melting Pot Movement! Welcome @{user.name}",
                keyword="mp_guild",
                config = config)