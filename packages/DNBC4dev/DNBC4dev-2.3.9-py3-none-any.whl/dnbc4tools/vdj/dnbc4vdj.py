...
from dnbc4tools.vdj.__init__ import _pipe
...
def pipeline_package(pipe: str) -> Any:
    package_name = f"dnbc4tools.rna.{pipe}"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        description=textwrap.dedent(help_text('vdj', 'dnbc4vdj'))
    )
    parser.add_argument(
        '-v',
        '--version',
    ...
    for pipe in _pipe:
        ...
        parser_step = subparsers.add_parser(
            pipe,
            description=textwrap.dedent(help_sub_text('vdj', 'dnbc4vdj', pipe)),
            formatter_class=argparse.RawDescriptionHelpFormatter
        )
