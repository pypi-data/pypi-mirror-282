        ...
        if x[0] == 'E':
            ...
            if tmp[1] == "Participant":
                ...
                ann = {'ann_type': 'TextBound', 'tag_id': tmp[0], 'attribute': tmp[1].split(
                    ':')[0], 'tag_ref': tmp[2], 'value': ' '.join(tmp[4:])}
        elif x[0] == 'R':
...
def get_all_entities(ann_dict):
    ...
    for x in ann_dict:
        if x['ann_type'] == 'TextBound' and 'attribute' in x:
            ...
            # # TODO CHECK IF AFFECTS OTHERS, WROTE THIS BECAUSE OF FILE 102 (T37 AND T83)
            # else:
            #     entities_dict[x['tag_id']] = {'attribute': x['attribute'], 'value': x['value'],
            #                                   'tag_ref_beginning': x['tag_ref']}
            # else:
...
def assemble_extended_scenes(semantic_roles_dict_with_dups, participants, events):
    ...
    for k, v in double_check.items():
        ...
        if (is_event or is_participant) and scene_id != '':
            if v['tag_ref2'] in participants or (
                ...
                results[scene_id]['relations'].update({k: v})
                results[scene_id]['participants'] += ';' + v['tag_ref2']
        # elif v['rel_type'] == 'SRLINK_result': # Caso não encaixe em nenhuma scena anterior, um result pode ser a própria cena
        #    results[k] = v
    ...
    # Sorting so SRLINK_agent comes first
    # for k, r in results.items():
    #     has_agent = None
    #     before_agent = []
    #     after_agent = []
    #     for relation_key, relation_value in r['relations'].items(): # assuming agent and patient are adjacent and first
    #         if relation_value['rel_type'] == 'SRLINK_agent':
    #             has_agent = (relation_key, relation_value)
    #         if has_agent:
    #             after_agent.append((relation_key, relation_value))
    #         else:
    #             before_agent.append((relation_key, relation_value))
    #     if has_agent is not None:
    #         sorted_relations = {}
    #         sorted_relations[has_agent[0]] = has_agent[1]
    #         for (relation_key, relation_value) in before_agent:
    #             sorted_relations[relation_key] = relation_value
    #         for (relation_key, relation_value) in after_agent:
    #             sorted_relations[relation_key] = relation_value
    #         r['relations'] = sorted_relations
...
def build_titles(extended_scenes):
    for scene_key, scene_value in extended_scenes.items():
        ...
        for i, v in enumerate(relations_list):
            # First relation
            if i == 0:
                if v['tag_ref1'] in scene_value['events']:  # verbo -> deve ser escrito em segundo
                ...
                elif v['tag_ref1'] in scene_value['participants']:
                    if v['tag_ref2'] in scene_value['events']:
...
def brat2json(f, json_dir):
    ...
    # file_name = f.split('/')[-1]
    # shutil.copyfile(f, NEWS_VIZ_DATA_DIR + 'brat/' + file_name)
    # shutil.copyfile(f.replace('ann', 'txt'), NEWS_VIZ_DATA_DIR + 'text/' + file_name.replace('ann', 'txt'))
