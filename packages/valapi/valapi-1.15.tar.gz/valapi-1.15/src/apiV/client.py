...
class Client:
    ...
    def fetch_match_history(
        ...
        start_index: int = 0,
        end_index: int = 15,
        queue_id: t.Optional[t.Text] = "null",
    ) -> dict:
        """
        Match History
    ...
    def fetch_match_details(self, match_id: t.Text) -> t.Mapping[str, t.Any]:
        """
        Match Details
        Get the details of any match after it ends
        """
        data = self.fetch(
    ...
    def fetch_penalties(self) -> t.Mapping[str, t.Any]:
        """
    ...
    def name_service(self, body: t.Text) -> t.Mapping[str, t.Any]:
        """
        Name Service
    ...
    def party_set_custom_game_settings(self, settings: t.Mapping) -> t.Mapping[str, t.Any]:
        ...
        settings:
        {
            ...
            "UseBots": true,
            "GamePod": "aresriot.aws-rclusterprod-use1-1.na-gp-ashburn-awsedge-1",
            "GameRules": null 
        }
        """
        party_id = self.__get_current_party_id()
    ...
    def party_leave(self) -> t.Mapping[str, t.Any]:
        """
        Party_PlayerLeave
        Leave a party
        """
    ...
    def party_disable_code(self) -> t.Mapping[str, t.Any]:
        """
        Party Disable Code
        Disable the party invite code
        """
        party_id = self.__get_current_party_id()
        data = self.delete(endpoint=f"/parties/v1/parties/{party_id}/invitecode", endpoint_type="glz")
        return data

    def party_generate_code(self) -> t.Mapping[str, t.Any]:
        """
        Party Generate Code
        Generate a party invite code
        """
        party_id = self.__get_current_party_id()
        data = self.post(endpoint=f"/parties/v1/parties/{party_id}/invitecode", endpoint_type="glz")
        return data

    def party_join_by_code(self, code: t.Text) -> t.Mapping[str, t.Any]:
        """
        Party Join By Code
        Join a party using an invite code
        """
        data = self.post(endpoint=f"/parties/v1/players/joinbycode/{code}", endpoint_type="glz")
        return data
    ...
    def fetch_contracts(self) -> t.Mapping[str, t.Any]:
        """
        Contracts_Fetch
    ...
    def activate_contracts(self, contract_id: t.Text) -> t.Mapping[str, t.Any]:
        """
        Contracts_Activate
    ...
    def add_friend_requests(self) -> t.Mapping[str, t.Any]:
        """
    ...
    def remove_friend_requests(self, puuid: t.Text) -> t.Mapping[str, t.Any]:

        data = self.delete(
            endpoint="/chat/v4/friendrequests",
            endpoint_type="local",
            json_data=puuid,
        )
        return data
    ...
    def fetch_party_info(self) -> t.Mapping[str, t.Any]:
        """
        Party Chat Info
        Get information about the party chat
        """
        data=self.fetch(endpoint="/chat/v6/conversations/ares-parties", endpoint_type="local")
        return data
    
    def pregame_chat_info(self) -> t.Mapping[str, t.Any]:
        """
        Pre-Game Chat Info
        Get information about the pre-game chat
        """
        data=self.fetch(endpoint="/chat/v6/conversations/ares-pregame", endpoint_type="local")
        return data
    
    def current_game_chat(self) -> t.Mapping[str, t.Any]:
        """
        Current Game Chat
        Get information about the current game chat
        """
        data=self.fetch(endpoint="/chat/v6/conversations/ares-coregame", endpoint_type="local")
        return data
    
    def all_chat_info(self) -> t.Mapping[str, t.Any]:
        """"
        All Chat Info
    ...
    def chat_participants(self, cid: t.Text) -> t.Mapping[str, t.Any]:
        """
        Get information about the participants of all active conversations or a specific conversation if a cid is provided
        """
        data=self.fetch(endpoint=f"/chat/v5/participants?cid={cid}", endpoint_type="local")
        return data
    
    def send_chat(self, cid: t.Text, message: t.Text, type: t.Text) -> t.Mapping[str, t.Any]:
        """
        Send Chat
    ...
    def chat_history(self, cid: t.Optional[t.Text] = None   ) -> t.Mapping[str, t.Any]:
        """
        Chat History
        Get chat history for all conversations or a specific conversation if the cid is provided
        """
        data=self.fetch(endpoint=f"/chat/v6/messages?cid={cid}", endpoint_type="local")
        return data
