...
def convert_TableFile_to_dataset(tableTxtFilePath, prefix=''):
    ...
    with HiddenPrints():
        try:
            ...
            numpyDataNeg = np.flip(np.hstack((-np.expand_dims(np.char.replace(numpyData[negativeIndex,0], prefix + '_', '').astype(np.float32), axis=-1), numpyData[negativeIndex,1:])), axis=0)
            numpyDataPos = np.hstack((np.expand_dims(numpyData[positiveIndex,0], axis=-1), numpyData[positiveIndex,1:]))
