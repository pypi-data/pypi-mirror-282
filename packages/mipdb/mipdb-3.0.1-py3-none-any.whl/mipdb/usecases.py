...
class ImportCSV(UseCase):
    ...
    def execute(
        ...
        relative_csv_path = self._get_relative_csv_path(csv_path)
    ...
    def _get_relative_csv_path(self, csv_path):
        # In case the DATA_PATH is empty it will return the whole path.
        credentials = credentials_from_config()
        if credentials["DATA_PATH"]:
            return csv_path.split(credentials["DATA_PATH"])[-1]
        return csv_path
