...
class MeasurementResult:
    ...
    * `targets`: (t, v)-positions of any targets to draw when highlighted
    """
    def __init__(self, value, vlines=None, hlines=None, targets=None):
        self.value = value
...
class LeftMeasurement:
    ...
    def measure(cls, channel, times, values):
        ...
        return MeasurementResult(f, targets=targets)
...
class DeltaMeasurement:
    ...
    def measure(cls, channel, times, values):
        ...
        if times is values:
            targets = hlines = []
        f = channel.formatted_value(value=v)
        return MeasurementResult(f, hlines=hlines, targets=targets)
...
class MeanMeasurement:
    ...
    def measure(cls, channel, times, values):
        ...
        v = np.nanmean(values)
        f = channel.formatted_value(value=v)
        return MeasurementResult(f, hlines=[v])
...
class StdMeasurement:
    ...
    def measure(cls, channel, times, values):
        ...
        if np.isfinite(v) and np.isfinite(u):
            ...
            hlines = []
        f = channel.formatted_value(value=v)
        return MeasurementResult(f, hlines=hlines)
...
class PeriodMeasurement:
    ...
    def measure(cls, channel, times, values):
        ...
        return MeasurementResult(f"{period:.04g}", vlines=vlines, targets=targets)
...
class FrequencyMeasurement(PeriodMeasurement):
    ...
    def measure(cls, channel, times, values):
        ...
        period, vlines, targets = result
        freq = 1.0 / period
        return MeasurementResult(f"{freq:.04g}", vlines=vlines, targets=targets)
