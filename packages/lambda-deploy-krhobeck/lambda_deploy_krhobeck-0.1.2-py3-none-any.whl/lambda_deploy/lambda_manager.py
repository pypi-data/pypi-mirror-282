...
from decorators import lambda_functions
...
def upload_lambda_code(session: Session, lambda_function_data):
    """Uploads lambda function code to an S3 bucket."""
    s3: S3Client = session.client("s3")
    s3.upload_fileobj(
        Fileobj=open(lambda_function_data["file_path"], "rb"),
        Bucket=lambda_function_data["s3_bucket"],
        Key=lambda_function_data["s3_key"],
    )
...
def setup_lambdas(session: Session):
    """Setup Lambda functions based on metadata from decorators."""
    for lambda_function_data in lambda_functions:
        if "file_path" in lambda_function_data:
            upload_lambda_code(session, lambda_function_data)
