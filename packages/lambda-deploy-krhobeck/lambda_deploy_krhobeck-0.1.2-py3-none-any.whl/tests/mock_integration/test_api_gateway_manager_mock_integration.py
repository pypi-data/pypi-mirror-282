...
from api_gateway_manager import (
    create_rest_api,
    deploy_api,
    create_resource,
    setup_method,
...
from pydantic_models import AuthorizerConfig
...
class MockApiGatewayManagerMockIntegrationTest(TestCase):
    ...
    def test_setup_method(self):
        ...
        lambda_function_data = {
            ...
            "my-function",
            "authorizer": AuthorizerConfig(**{"authorizer_id": "abc123"}),
        }
        setup_method(
    ...
    def test_setup_api_gateway(self):
        """Test the complete setup of API Gateway."""
        with patch(
            "api_gateway_manager.lambda_functions",
            new_callable=lambda: [
                {
                    "name": "TestFunction",
                    "endpoint": "test",
                    "http_method": "GET",
                    "lambda_arn": "arn:aws:lambda:us-east-1:123456789012:"
            ...
            setup_api_gateway(self.aws_session)
