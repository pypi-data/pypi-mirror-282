    ...
    def _compute_is_deposit_sale_order(self):
        for record in self:
            ...
            first_rule = self.env['stock.rule'].search([
                ('location_src_id', '=', constants.LOCATION_ID_STOCK),
                ('location_id', '=', constants.LOCATION_ID_DEPOSITO)
            ], limit=1)

            second_rule = self.env['stock.rule'].search([
                ('location_src_id', '=', constants.LOCATION_ID_DEPOSITO),
                ('location_id', '=', constants.LOCATION_ID_CUSTOMERS)
            ], limit=1)
