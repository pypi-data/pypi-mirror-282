...
class SimpleActiveLearner:
    ...
    def __init__(self, data_pack:HyperparamDataSetup, fail_predictor=None, classifier_predict_uncertainty_func: Callable[[Any, np.ndarray], Tuple[np.ndarray, np.ndarray]]=classifier_predict_simple_uncertainty, redundancy_regressor=None, regressor_accuracy:Callable[[Any, np.ndarray, np.ndarray],float]=get_regressor_accuracy, skip_redundancy = False, get_valid_func:Callable[[np.ndarray],np.ndarray]=get_valid, X_train: np.ndarray=None, Y_train_success: np.ndarray=None, Y_train_perfs: np.ndarray=None, use_absolute_query_strategy=False, DESIGN_SPACE_DENSITY:int=100000, UNCERTAINTY_THRESHOLD:float=0.05, DROPOUT_RATE:float=0.2):
    ...
    def query(self, batchNum:int, proximity_weight:float=0.5) -> np.ndarray:
        ...
        print("Estimated Mape: ", avg_resids)
