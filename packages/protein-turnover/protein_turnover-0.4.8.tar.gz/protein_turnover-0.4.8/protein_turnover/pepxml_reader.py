...
def rowgetters(items: dict[str, Any], paranoid: bool = False):  # noqa C901:
    def mkget(keys, check):
        *rel, attr = keys
...
def peptide_maker(  # noqa C901:
    ...
    *,
    ...
    def search_hit(node: etree.Element) -> dict[str, Any]:
        ...
        def doPP(ret: dict[str, float | str], p, cdict: Converters) -> None:
        ...
        HIT: Converters = {
            ...
            "massdiff": lambda v: 0.0
            if v.startswith("+-")
            else float(v),  # as abs of absolute_delta
        }
...
def okdf(df1: pd.DataFrame, df2: pd.DataFrame) -> None:  # pepxml dataframes
    ...
    def okarr(s: pd.Series, c: pd.Series) -> bool:
        a1 = s[c]
        a2 = s[c + "_o"]
        if len(a1) != len(a2):
            return False
...
def error_data(node: etree.Element) -> list[ErrorData]:
    ret: list[ErrorData] = []
