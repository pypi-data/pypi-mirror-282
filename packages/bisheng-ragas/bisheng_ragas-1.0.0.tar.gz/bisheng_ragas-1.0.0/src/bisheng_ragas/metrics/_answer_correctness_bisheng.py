...
class AnswerCorrectnessBisheng(MetricWithLLM):
    ...
    def _score_batch(
        ...
        for prediction in outputs:
            ...
            if prediction:
                prediction = [item.get(key_map[k], np.nan) for item in prediction for k in key_map.keys()]
                ...
                statements_overlap.append(tp)
                score = tp / (tp + 0.5 * (fp + fn))
                precision_score.append(tp / (tp + fp))
                recall_score.append(tp / (tp + fn))
            else:
                statements_num_gt_only.append(np.nan)
                statements_num_answer_only.append(np.nan)
                statements_overlap.append(np.nan)
                score = np.nan
