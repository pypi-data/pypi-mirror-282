# -*- coding:utf-8 -*-
##############################################################
# Created Date: Tuesday, July 11th 2023
...
class Loga:
    ...
    def __init__(
        ...
        *,  # Reject positional arguments
        ...
        if do_write:
            ...
            pathlib.Path(os.path.dirname(logfile)).mkdir(
        ...
        def full_decoration(*args: Any, **kwargs: Any) -> Any:
            ...
            if bound is None:
                self.warning(
                    ...
                    extra={"callable_name": getattr(
                        function, "__qualname__", "unknown_callable")},
                )
    ...
    def _string_params(self, non_private_params: Mapping, use_repr: bool = True) -> dict[str, str]:
        ...
        for key, val in non_private_params.items():
            ...
            safe_val = self._force_string_and_truncate(
                val, truncation, use_repr=use_repr)
    ...
    def _truncate(string_to_truncate: str, max_len: int | None) -> str:
        ...
        if max_len < len(truncation_suffix):
            raise ValueError(
                f"Can't truncate to less than {len(truncation_suffix)} characters")
