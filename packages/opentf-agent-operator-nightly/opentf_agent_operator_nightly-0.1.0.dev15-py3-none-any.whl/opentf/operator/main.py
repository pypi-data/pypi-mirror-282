...
POOLS_KIND = 'pools'
AGENTS_URL_TMPL = 'http://{pod}/agents'
AGENT_ID_URL_TMPL = 'http://{pod}/agents/{agent_id}'
...
def _as_list(item: Union[str, List[str]]) -> List[str]:
...
def _get_token(spec: Dict[str, Any], logger) -> Optional[str]:
    ...
    try:
        ...
        raise NotFound(f'Cannot retrieve orchestrator token: {str(err)}')
...
def _register_agent(
    ...
    logger.debug(
        f'Trying to register agent. Url: {url}, tags: {REGISTRATION["spec"]["tags"]}.'
    )
...
def _deregister_agent(
    ...
    logger.debug(f'Trying to de-register agent on {url}.')
    response = requests.delete(url, headers=headers, timeout=REQUEST_TIMEOUT)

    if response.status_code != 200:
        raise kopf.PermanentError(
            f'Cannot de-register agent: {response.json()["message"]}'
        )
    logger.debug(response.json()['message'])
...
def _get_agents(
    ...
    logger.debug(f'Agents list retrieved from {url}')
...
def relaunch_agents(spec, status, name, logger, patch, **kwargs):
    try:
        ...
        if kube_agents is None:
            logger.debug(f'No agents found, will create some.')
        ...
        if len(kube_agents) > len(otf_agents):
            ...
            return
        # if len(otf_agents) > len(kube_agents) ??? Do we need to handle it?
    except Exception as err:
...
AGENTS_POLLING_DELAY = 10
BUSY_AGENTS = set()


def _maybe_get_agent_command(agent_id: str, spec: Dict[str, Any], logger):
...
def _create_exec_pod(agent_id: str, api_instance, logger):
    pod_name = agent_id + '-pod'
    ...
    if existing_pods and pod_name in [pod.metadata.name for pod in existing_pods.items]:
        logger.info(f'Pod {pod_name} already exists, ignoring.')
        return
    pod = client.V1Pod(
        api_version='v1',
        kind='Pod',
    ...
    try:
        response = api_instance.create_namespaced_pod(namespace='default', body=pod)
        logger.info(f'Pod {pod_name} created.')
    except Exception as err:
        logger.error(f'Cannot create pod: {str(err)}')


def _delete_exec_pod(agent_id: str, api_instance, logger):
    try:
        response = api_instance.delete_namespaced_pod(
            name=agent_id + '-pod', namespace='default'
        )
        logger.info(f'Pod {agent_id}-pod deleted.')
    except Exception as err:
        logger.error(f'Cannot delete pod: {str(err)}')


def _process_command_mock(
    agent_url: str, headers: Optional[Dict[str, Any]], command: Dict[str, Any]
):
    kind = command['kind']
    if kind == 'put':
        requests.get(agent_url + '/files/' + command['file_id'], headers=headers)
    elif kind == 'exec':
        requests.post(
            agent_url,
            json={'stderr': [], 'stdout': [], 'exit_status': 0},
            headers=headers,
        )


@kopf.daemon(POOLS_API_GROUP, POOLS_API_VERSION, POOLS_KIND, initial_delay=10, cancellation_backoff=1.0)  # type: ignore
def handle_agent_command(stopped, spec, logger, **kwargs):
    # TODO: handle possible spec changes
    ...
    while not stopped:
        ...
        try:
            ...
            if command['kind'] == 'exec' and ('-2.sh' in command['command']):
                _process_command_mock(url, headers, command)
                time.sleep(1)
                busy = _maybe_get_agent_command(agent_id, spec, logger)
                if not busy:
                    try:
                        BUSY_AGENTS.remove(agent_id)
                    except KeyError:
                        pass
                    _delete_exec_pod(agent_id, api_instance, logger)
                    continue
            if agent_id not in BUSY_AGENTS:
                BUSY_AGENTS.add(agent_id)
                _create_exec_pod(agent_id, api_instance, logger)
            _process_command_mock(url, headers, command)
        except Empty:
            continue
        except Exception as err:
            logger.error(f'Command handling error: {str(err)}.')
    logger.info('Stopping agent commands handling daemon...')


@kopf.daemon(POOLS_API_GROUP, POOLS_API_VERSION, POOLS_KIND, initial_delay=10, cancellation_backoff=1.0)  # type: ignore
def monitor_agents(stopped, spec, status, logger, **kwargs):
    logger.info('Starting agents daemon...')
    while not stopped:
        try:
            if not (agents := status.get('create_agents', {}).get('agents')):
            ...
            for agent in agents:
                ...
                if not command:
                    logger.warning(f'Agent {agent} is idle, will retry.')
                    continue
    ...
    logger.info('Stopping agents daemon...')
