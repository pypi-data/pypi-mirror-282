...
RTC_STATE_PATH = settings.SONA_STREAM_RTC_STATE_PATH
...
def create_peer(ice_servers):
    ...
    if peers_lens < INFERENCER_PROCESS_LIMIT:
        peer = RTCPeerConnection(configuration=RTCConfiguration(ice_servers))
        peers.add(peer)
        return peer
    raise HTTPException(
        status_code=429,
        detail=f"Too many inference sessions (sessions: {peers_lens}, limit: {INFERENCER_PROCESS_LIMIT})",
    )


def add_routes(app):
    if RTC_STATE_PATH:
        Path(RTC_STATE_PATH).mkdir(parents=True, exist_ok=True)
        with open(str(Path(RTC_STATE_PATH) / "__metadata"), "w") as f:
