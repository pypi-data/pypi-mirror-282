    ...
    **kwargs
    ...
    if(is_pandas):
        ...
        if(method == 'clip'):
            ...
            for _col in vr:
                ...
                if(gr is None):
                    ...
                    if('bounds' in kwargs):
                        ...
                        _lower_bound_val = numpy.quantile(df[_col], 
                                                      q = kwargs['quantiles'][0])
                        _upper_bound_val = numpy.quantile(df[_col], 
                    ...
                    # clip
                    df[_col] = numpy.where(_upper_condition, 
                                           _upper_bound_val, 
                                           df[_col])
                    df[_col] = numpy.where(_lower_condition, 
                                           _lower_bound_val, 
                                           df[_col])
                    ...
                    _dfs_to_concat = []
                    for _, group in df.groupby(by = gr):

                        _lower_bound_val = numpy.quantile(group[_col], 
                                                          q = _qtiles[0])
                        _upper_bound_val = numpy.quantile(group[_col], 
                                                          q = _qtiles[1])
            ...
            if(gr is None):
                df[vr] = df[vr].apply(_func_to_apply)
            else: 
                ...
                _tmp = df.groupby(by = gr)[vr].apply(_func_to_apply)
