...
class DialogWorkflowLocal(GenericCRUD, metaclass=MetaLogger, object=DIALOG_WORKFLOW_CODE_LOGGER_OBJECT):
    def __init__(self, *, user_context: UserContext = UserContext(), is_test_data: bool = False) -> None:
    ...
    def _save_compound_message_answer(self, incoming_message: str) -> None:
        ...
        for compound_message_answer in compound_message_answers:
            data_dict = {
                "user_id": compound_message_answer.get("user_id") or self.user_context.get_effective_user_id(),
                "profile_id": compound_message_answer.get("profile_id") or self.user_context.get_effective_profile_id(),
            }
