...
class Action(metaclass=MetaLogger, object=DIALOG_WORKFLOW_CODE_LOGGER_OBJECT):
    def __init__(self, *, incoming_message: str, profile_id: int,
        ...
        self.record: DialogWorkflowRecord or None = None
    ...
    def send_rest_api_action() -> dict:
        ...
        # insert_profile_variable_value(self.profile_id, self.variable.get_variable_id("Post Result"), incoming_message_string, self.profile_curr_state_id)
    ...
    def multi_choice_poll(self) -> dict:
        ...
        if not self.got_response:
            self.accumulated_message = self.accumulated_message + self.record.parameter1 + seperator + \
                                       f"Please select your desired choices, You may select any of the numbers between 1-{len(child_nodes)} with a comma seperator between each choice:{seperator}"
        is_state_changed, next_state_id, outgoing_message = self.generic_user_choice_action(
            record=self.record, accumulated_message=self.accumulated_message, child_nodes=child_nodes,
    ...
    def present_groups_with_certain_text(self) -> dict:
        ...
        if not self.got_response:
            self.accumulated_message += "Please choose your desired interests. You may select more than one choice with a comma seperator." + seperator
    ...
    def present_and_choose_script(self) -> dict:
        ...
        cursor = connection.cursor(dictionary=True, buffered=True)
        cursor.execute(
            """SELECT d.start_state_id, dml.title FROM dialog_workflow_script_view AS d JOIN dialog_workflow_script_ml_view AS dml on dml.dialog_workflow_script_id=d.dialog_workflow_script_id WHERE dml.lang_code = %s OR dml.lang_code IS NULL""",
    ...
    def present_form_action(self) -> dict:
        ...
        save_additional_parameters = {"source_subsystem_id": 2,  # TODO do not use hard coded
                                      "channel_id": 14,
                                      "dialog_workflow_state_id": self.record.curr_state_id, }
    ...
    def generic_user_choice_action(self, *, record: DialogWorkflowRecord, accumulated_message: str, child_nodes: list,
                                   ...
                                   profile_context: ProfileContext) -> (bool, int, str):  # TODO: return dict
