...
def generate_diesel_schema(graph: Graph) -> str:
    ...
    cw.emit("// @generated automatically by stone.")
    cw.emit()
    ...
    with cw.block("pub mod graph", delim=delim):
        ...
        with cw.block("pub mod sql_types", delim=delim):
            ...
            for enum in graph.enums.values():
                ...
                cw.emit(
                    f'#[diesel(postgres_type(name = "{fmt_underscores(enum.name)}", schema = "{SCHEMA_NAME}"))]'
                )
            ...
            for ctype in graph.ctypes.values():
                ...
                cw.emit("#[derive(diesel::sql_types::SqlType, QueryId)]")
                cw.emit(
                    f'#[diesel(postgres_type(name = "{fmt_underscores(ctype.name)}", schema = "{SCHEMA_NAME}"))]'
                )
        ...
        for vertex in graph.vertices.values():
            ...
            with cw.block("diesel::table!", delim=delim):
                ...
                cw.emit()
                with cw.block(
                    f"{SCHEMA_NAME}.{fmt_underscores(vertex.name)} (gid)", delim=delim
                ):
        ...
        for edge in graph.edges.values():
            ...
            with cw.block("diesel::table!", delim=delim):
                ...
                with cw.block(
                    f'{SCHEMA_NAME}.{fmt_underscores(edge.name)} ({", ".join(pks)})',
                    delim=delim,
                ):
            ...
            for counter in edge.counters:
                with cw.block("diesel::table!", delim=delim):
                    ...
                    cw.emit_list(
                        items=items,
                        bracket=delim,
                        before=f'{SCHEMA_NAME}.{counter_name} (src, {", ".join(counter_col_names)})',
                    )
                cw.emit()
...
// @generated automatically by stone.

use crate::schema::graph::sql_types as graph_sql_types;
...
#[diesel(sql_type = crate::schema::graph::sql_types::PgGid)]
pub struct Gid {
    pub id: i64,
    pub vshard: i16,
}
