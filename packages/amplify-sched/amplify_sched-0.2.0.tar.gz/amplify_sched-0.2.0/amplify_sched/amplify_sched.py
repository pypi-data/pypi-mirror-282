...
class DatetimeUnit:
    ...
    def __init__(
        ...
        self._dtime_start: datetime = (
            datetime.combine(self._today, self._start)
            if isinstance(self._start, time)
            else self._start
        )
        self._used = False
    ...
    def to_int_datetime(self, dtime: Union[datetime, time]) -> int:
        ...
        if isinstance(self.start, time) and isinstance(dtime, datetime):
            raise ValueError(
                "Argument type must be 'datetime.datetime' since 'start' type is 'datetime.time'"
            )
        ...
        if dtime < self._dtime_start:
            raise ValueError(
                f"The argument date or time {dtime} must be later than 'start' time {self._dtime_start}"
            )
    ...
    def from_int_datetime(self, int_datetime: int) -> Union[datetime, timedelta]:
        ...
        if isinstance(self.start, time):
            return (
                self._dtime_start
                + int_datetime * self.unit
                - datetime.combine(self._today, time())
            )
...
def _to_int_datetime(
    dtime_unit: Optional[DatetimeUnit], dtime: Union[int, datetime, time]
) -> int:
...
def _to_int_timedelta(
    dtime_unit: Optional[DatetimeUnit], delta: Union[int, timedelta]
) -> int:
...
class Schedule:
    ...
    def __init__(self, result: list[dict[str, int]], status: str, model: "Model"):
        ...
        # conversion
        self._table["Machine"] = self._table["Machine"].apply(
            lambda x: self._model.machines._names()[x]
        )
        self._table["Job"] = self._table["Job"].apply(
            lambda x: self._model.jobs._names()[x]
        )
        self._time_type: type
        if self._model._datetime_unit._used:
            self._time_type = type(self._model._datetime_unit.start)
            self._table["Start"] = self._table["Start"].apply(
        ...
        self._table = self._table.reindex(
            columns=["Job", "Process", "Machine", "Start", "Finish"]
        )
    ...
    def timeline(
        ...
        def __process_dataframe_timeline_for_int(
            args: dict[str, Any],
        ) -> dict[str, Any]:
            """
        ...
        if self._time_type == int:
            ...
            df["Start"] = df["Start"].apply(
                lambda x: datetime.combine(date.today(), time()) + x
            )
    ...
    def resource(
        ...
        def get_required_resources(x):
            required_resources = self._model.jobs[x["Job"]][
                x["Process"]
            ].required_resources
            ret = []
                    ...
                    pd.DataFrame(
                        {
                            ...
                            "requirement": [
                                0 for _ in range(len(self._model.resources))
                            ],
                        }
                    ),
                ]
                    ...
                    pd.DataFrame(
                        {
                            ...
                            "time": [
                                self._model.start
                                for _ in range(len(self._model.resources))
                            ],
                            "requirement": [
                                0 for _ in range(len(self._model.resources))
                            ],
        ...
        df_resource["requirement"] = df_resource.groupby("Resource")[
            "requirement"
        ].cumsum()
        df_resource["available"] = [
            self._model.resources[row["Resource"]].capacity - row["requirement"]
...
class TimeRange(tuple):  # type: ignore
    ...
    def __new__(
        cls, start: Union[datetime, time, int], end: Union[datetime, time, int]
    ) -> TimeRange:
    ...
    def _create_request_data(
        self, dtime_unit: Optional[DatetimeUnit] = None
    ) -> tuple[int, int]:
        if isinstance(self.start, int) or isinstance(self.end, int):
            return self.start, self.end  # type: ignore
        if dtime_unit is None:
            raise ValueError("start time or time unit is not given")
        return dtime_unit.to_int_datetime(self.start), dtime_unit.to_int_datetime(
            self.end
        )
...
class ProcessingTimes:
    ...
    def __setitem__(
        self, key: Union["Machine", str], value: Union[int, timedelta]
    ) -> None:
    ...
    def _create_request_data(
        ...
        return [
            [_to_int_timedelta(dtime_unit, v), k.index] for k, v in self._times.items()
        ]
    ...
    def get(
        self, key: Union["Machine", str], default: Any = None
    ) -> Union[int, timedelta]:
        if isinstance(key, str):
            model = _get_model(self)
            if model is None:
...
class SetupTimes:
    ...
    def _check_element(
        ...
        tuple[
            Union[int, timedelta], Union["Job", "Task", str], Union["Job", "Task", str]
        ],
        tuple[Union[int, timedelta], Union["Job", "Task", str]],
        int,
        timedelta,
        ...
        return [
            (
                ...
                if isinstance(t, int) or isinstance(t, timedelta)
                    ...
                    _to_int_timedelta(dtime_unit, v) if i == 0 else __f(v)
    ...
    def _tolist(
        ...
        tuple[Union[int, timedelta], list[Union[str, int]], list[Union[str, int]]]
    ]:
...
class Resources:
    ...
    def add(self, resource: Union[str, Resource]) -> None:
        ...
        if not resource.name:
            raise ValueError(
                'resource is no name, use `resource["name"] = value` instead'
            )
...
class RequiredResources:
    ...
    def _create_request_data(self) -> list[Union[int, list[int]]]:
        return [
            m.index if isinstance(m, Resource) else [m[0].index, m[1].index]
            for m in self._resources
        ]

    def __str__(self) -> str:
        return str(
            [
                m.name if isinstance(m, Resource) else [m[0].name, m[1].name]
                for m in self._resources
            ]
        )
...
class Machine:
    ...
    def add_maintenance_time(
        self, start: Union[datetime, time, int], end: Union[datetime, time, int]
    ) -> None:
    ...
    def _create_request_data(
        ...
        if len(self._maintenance_times) != 0:
            d["maintenance_times"] = [
                mt._create_request_data(dtime_unit) for mt in self._maintenance_times
            ]
...
class Machines:
    ...
    def add(self, machine: Union[str, Machine]) -> None:
        ...
        if not machine.name:
            raise ValueError(
                'machine is no name, use `machine["name"] = value` instead'
            )
    ...
    def _create_request_data(
        ...
        return [
            m._create_request_data(dtime_unit)
            for m in sorted(self._machines_set, key=lambda m: m.index)
        ]
...
class TransportationTimes:
    ...
    def __setitem__(
        self, index: int, value: tuple[Union[int, timedelta], Machine, Machine]
    ) -> None:
        self._times[index] = self._check_element(value)
        ...
        return [
            (
                ...
                if isinstance(t, int) or isinstance(t, timedelta)
                    ...
                    _to_int_timedelta(dtime_unit, v) if i == 0 else v.index
                    for (i, v) in enumerate(t)
                ]
            )  # type: ignore
            for t in self._times
...
class DependentJob:
    ...
    def __init__(
        ...
        self._interval_time_lower_bound, self._interval_time_upper_bound = (
            self._check_times(interval_times)
        )
    ...
    def _check_times(
        self, interval_time: tuple[int | None, int | None] | int | None
        ...
        if isinstance(interval_time, int):
            ...
            if (
                interval_time[0] is not None
                and interval_time[1] is not None
                and interval_time[0] > interval_time[1]
            ):
...
class Task:
    ...
    def add_transportation_time(
        ...
        self._transportation_times.append(
            (transportation_time, src_machine, dst_machine)
        )
    ...
    def _create_request_data(
        ...
        if len(self._transportation_times) != 0:
            d["transportation_times"] = self._transportation_times._create_request_data(
                dtime_unit
            )
    ...
    def _asdict(self) -> dict[str, Any]:
        return {
            ...
            "transportation_times": self._transportation_times
            if self._transportation_times
            else None,
...
class Jobs:
    ...
    def _create_request_data(
        ...
        return [
            j._create_request_data(dtime_unit)
            for j in sorted(self._jobs_set, key=lambda j: j.index)
        ]
...
class Model:
    ...
    def _create_request_data(self, timeout: int) -> dict[str, Any]:
        ...
        d["model"]["machines"] = self._machines._create_request_data(
            self._datetime_unit
        )
    ...
    def solve(
        ...
        if self._ret["status"] in ["OPTIMAL", "FEASIBLE"]:
            return Schedule(
                result=self._ret["gantt"], status=self._ret["status"], model=self
            )
