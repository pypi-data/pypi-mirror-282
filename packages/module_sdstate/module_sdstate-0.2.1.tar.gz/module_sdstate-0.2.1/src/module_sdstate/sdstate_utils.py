...
#         To be changed or improved? Current implementation based on Epq
    ...
    def get_cov_cost(self, U = None):
        ...
        for p in range(self.n_qubit):
            ...
            if U is not None:
                n_p_vec = np.zeros(self.n_qubit)
                n_p_vec[p] = 1
                n_p_vec = np.einsum('ak,bl,kl->ab', np.conj(U), U, n_p_vec)
                n_p = sum([of.FermionOperator(f"{i}^ {i}") for i in range(self.n_qubit) if n_p_vec[i] != 0])
            ...
            for q in range(p + 1):
                ...
                if U is not None:
                    n_q_vec = np.zeros(self.n_qubit)
                    n_q_vec[q] = 1
                    n_q_vec = np.einsum('ak,bl,kl->ab', np.conj(U), U, n_q_vec)
                    n_q = sum([of.FermionOperator(f"{i}^ {i}") for i in range(self.n_qubit) if n_q_vec[i] != 0])
