...
from modelbest_sdk.dataset.thrift_wrapper.base_doc import BASE_DOC, MM_DOC, BaseDoc, DetailedDoc, MmDoc, MmDocSeq
...
class SegmentDataset(torch.utils.data.IterableDataset):
    def __init__(
        ...
        self.name = prefetch_chunk_dataset.path.split('/')[-1]
    ...
    def __iter__(self):
        for detailed_doc in self.prefetch_chunk_dataset:
            ...
            elif self.doc_type == MM_DOC:
                detailed_doc.mm_doc_seq = MmDocSeq.deserialize(detailed_doc.raw)
                detailed_doc.mm_doc_seq.doc_seq[0].tag = [self.name] if not detailed_doc.mm_doc_seq.doc_seq[0].tag else detailed_doc.mm_doc_seq.doc_seq[0].tag
            
            detailed_doc.position.length = len(detailed_doc)
