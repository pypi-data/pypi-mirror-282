...
class TaskLineGroupRestView(BaseRestView):
    ...
    def post_load_groups_from_catalog_view(self):
        ...
        sale_product_group_quantities = self.request.json_body.get(
            "sale_product_group_quantities", []
        )

        logger.debug("sale_product_quantities : %s", sale_product_group_quantities)

        # handle the case where sale_product_quantity is not retrieved in the request
        groups = []
        for id_, quantity in sale_product_group_quantities.items():
