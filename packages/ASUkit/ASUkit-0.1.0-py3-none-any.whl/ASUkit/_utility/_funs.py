...
def map_int(peak, x, twotheta):
    ...
    for angle in range(len(twotheta)):
        index = np.where(np.isclose(x, twotheta[angle]))[0]  # Find index for each angle
        if index.size > 0:  # Check if indices are not empty
            y_twotheta[angle] = peak[index]  # Map peak intensity
    
    return y_twotheta

def combined_peak(twotheta, Weight, mu, gamma, sigma2, step=0.01):
    ...
    x = np.arange(np.round(mu - l_gap, 2), np.round(mu + 30, 2), step)
    ...
    combined = combined / combined.max() * Weight  # Normalize peak and apply weight
