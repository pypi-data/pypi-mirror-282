...
class Models:
    ...
    def _get_versions(self, model_id):
        ...
        endpoint = f"models/{model_id}/modelversions"
        return self.api_client._make_request('GET', endpoint)
    ...
    def list(self):
        ...
        * **df** (pd.DataFrame): Pandas Dataframe containing model details.
        ...
        if models:
            ...
            df = df[['_id', 'name', 'developer', 'created']]
            df = df.rename(columns={'_id': 'Model id', 'name': 'Model name', 'developer': 'Author', 'created': 'Created'})
            ...
            df = pd.DataFrame(columns=['Model id', 'Model name', 'Author', 'Created'])
            self.api_client.log.warning(f"No models found.")
    ...
    def upload(self, file_path, model_name, version_name=None):
        """
        ### Description
        ...
        * **version_id** (str): The ID of the uploaded version or None if upload fails.
        ...
        model_id = self._get_id(model_name)
        if not model_id:
            model_id = self._create(model_name)
        if model_id:
            ...
            if result:
                ...
                self.api_client.log.info(f"Model {model_name}:{version_name} uploaded successfully.") 
            else:
                version_id = None
    ...
    def _deploy_new_workload(self, node_id, model_id, model_name, version_id, version_name, grpc_port=9083, rest_port=9084):
        """
        ### Description
        ...
        * **workload_id** (str): The ID of the deployed workload or None if deployment fails.
        ...
        endpoint = f"devices/{node_id}/model/space"
        ...
        json_data = {
            ...
            'environmentVariables': [
                ...
                {
                'name': encode_to_base64('MODEL_NAME'),
                'value': encode_to_base64('model')
                }
            ]
        }
    ...
    def _deploy_update_workload(self, workload_id, node_id, model_id, model_name, version_id, version_name, grpc_port=9083, rest_port=9084):
        """
        ...
        endpoint = f"devices/{node_id}/model/space/{workload_id}"
    ...
    def deploy(self, node_name, model_name, version_name=None, grpc_port=9083, rest_port=9084, timeout=300):
        """
        ### Description
            ...
            if version_name:
                ...
                if version_id:
                    ...
                    if node_id:
                        ...
                        if workload_id:
                            workload_id = self._deploy_update_workload(workload_id, node_id, model_id, model_name, version_id, version_name, grpc_port, rest_port)
                        else:
                            workload_id = self._deploy_new_workload(node_id, model_id, model_name, version_id, version_name, grpc_port, rest_port)
                        ...
                        if workload_id:
                            workload_id = self.api_client.workloads.start(workload_id)
                        else:
