...
class Workloads:
    ...
    def _get(self):
        ...
        endpoint = f"devices/applications/running/"
    ...
    def list(self):
        ...
        * **df** (pd.DataFrame): A Pandas DataFrame containing the list of workloads with columns:
        ...
        if workloads:
            ...
            df = df[['_id', 'typeText', 'applicationName', 'appVersionName', 'status', 'deviceName']]
            
            # Rename the columns for better readability
            df = df.rename(columns={'_id': 'Workload id', 'typeText': 'Type', 'applicationName': 'Name', 'appVersionName': 'Version name', 'status': 'Status', 'deviceName': 'Node name'})
            ...
            df = pd.DataFrame(columns=['Workload id', 'Type', 'Name', 'Version name', 'Status', 'Node name'])
            self.api_client.log.warning(f"No workloads found.")
    ...
    def start(self, workload_id, timeout=120):
        ...
        * **workload_id** (str): The workload ID if started successfully, or None if failed or timed out.
        ...
        if node_id:
            ...
            endpoint = f"devices/{node_id}/docker/space/{workload_id}/start"
    ...
    def stop(self, workload_id, timeout=120):
        ...
        * **workload_id** (str): The workload ID if stopped successfully, or None if failed or timed out.
        ...
        if node_id:
            ...
            endpoint = f"devices/{node_id}/docker/space/{workload_id}/stop"
            result = self.api_client._make_request('POST', endpoint)
            if result and result == 'Done.':
    ...
    def remove(self, workload_id, timeout=120):
        ...
        * **workload_id** (str): The workload ID if removed successfully, or None if failed or timed out.
        ...
        if node_id:
            ...
            endpoint = f"devices/{node_id}/docker/space/{workload_id}"
