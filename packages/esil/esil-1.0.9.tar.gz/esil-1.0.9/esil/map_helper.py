...
import numpy as np
from panelayout_helper import get_layout_col_row,PaneLayout
import math
#for wrf_out data
...
def get_multiple_data(dict_data,dataset_name, variable_name, grid_x, grid_y,grid_concentration,file_name='',is_delta=False):
...
if __name__=="__main__":
    ...
    from rsm_helper import model_attribute  
    import xarray as xr
    import cmaps
    ...
    dict_data={}
    cmap_conc=cmaps.WhBlGrYeRe #cmaps.cmaps_wh_bl_gr_ye_re
    ...
    for i in range(1,26):        
        ...
        if  len(dict_data.keys())==6:
            fig=show_maps(dict_data,unit=unit,cmap=cmap_conc,boundary_file=boundary_file,show_plot=True,show_minmax=True,show_original_grid=True,show_lonlat=True,show_dependenct_colorbar=False,
                          value_range=(0,100),show_domain_mean=True,xy_ticks_digits=1,colorbar_ticks_num=9,colorbar_ticks_value_format='.1f',value_format='.1f',xy_title_fontsize=8,title_fontsize=12,
                          delta_map_settings={'cmap':cmap_delta,'value_range':(-3,3),'colorbar_ticks_num':7,'value_format':'.2f','colorbar_ticks_value_format':'.1e'},subplots_fig_size=None,
                          font_name='SimSun'
                          ) 
