...
class SQLAInterface:
    ...
    """
    The pydantic schema for the model. This is the standard schema, but all fields are optional and set to IgnoredData as default. Useful for POST and PUT requests.
    """
    schema_optional_none: Type[BaseModel]
    """
    The pydantic schema for the model. Same as schema_optional, but all fields are optional and set to None as default.
    """
    ...
    def __init__(self, obj: Model, with_fk: bool = True):
        ...
        self.schema_optional_none = self.generate_schema(
            self.list_properties.keys(), optional=True, only_as_none=True
        )
        self._init_filters()
    ...
    def generate_schema(
        ...
        Args:
            ...
            only_as_none (bool, optional): Whether to set the type as IgnoredData or only as None. Only works when optional is True. Defaults to False.
        ...
        if only_as_none:
            model_name += "-None"
        schema_dict = {
            "__model_name": name or model_name,
        ...
        self._generate_schema_properties(columns, optional, only_as_none, schema_dict)
    ...
    def get_type(
        self, col: str, optional: bool = False, only_as_none: bool = False
    ) -> Type:
        """
        ...
        Args:
            ...
            only_as_none (bool, optional): Whether to set the type as IgnoredData or only as None. Only works when optional is True and for related models. Defaults to False.
        ...
        if self.is_text(col) or self.is_string(col):
            ...
            schema = None
            if optional and only_as_none:
                schema = self.get_related_interface(
                    col, with_fk=False
                ).schema_optional_none
            elif optional:
                schema = self.get_related_interface(col, with_fk=False).schema_optional
            else:
                schema = self.get_related_interface(col, with_fk=False).schema
    ...
    def _generate_schema_properties(
        ...
        Args:
            ...
            only_as_none (bool): Flag indicating whether optional properties should be set to None instead of IgnoredData.
        ...
        for key, column in prop_dict.items():
            ...
            params = {}
            type = self.get_type(key, optional, only_as_none)
            if hasattr(column, "primary_key") and column.primary_key:
                self.id_schema = type
            if self.is_nullable(key) or optional:
                params["default"] = (
                    IgnoredData() if optional and not only_as_none else None
                )
                type = (
                    type | IgnoredData if optional and not only_as_none else type | None
                )
