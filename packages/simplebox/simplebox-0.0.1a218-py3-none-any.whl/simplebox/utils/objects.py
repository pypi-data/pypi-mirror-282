...
{propertycontent}
    '''
        ...
        property_getter_template = '''    @property
    def {attr}(self):
        return self.__{attr}
    '''
...
class ObjectsUtils(metaclass=StaticClass):
    ...
    def generators_setters_getters(class_name=None, attributes=None, need_setter: bool = True,
        ...
        if instance is not None:
            ...
            for attr in instance.__dict__.keys():
                if attr.startswith(prefix):
                    func_name[(attr.replace(prefix, ""))] = attr.replace(attr_prefix, "")
                elif len(attr) >= 2 and attr.startswith("_") and attr[1] != "_":
                    func_name[attr[1:]] = attr
                else:
                    func_name[attr] = attr
        ...
        content = _inner_constants.gen_class.class_template.format(classname=_strings.convert_to_camel(class_name),
