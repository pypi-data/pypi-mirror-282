...
parser.add_argument('-l', '--local', action='store_true', help='Run the script on your local machine. Overwrites --positron-deploy', dest='local')
parser.add_argument('--wrapper', action='store_true', help=argparse.SUPPRESS, dest='wrapper')
...
positron_args = parser.parse_args()
...
def positron_sync(**positron_parameters):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            if positron_args.local:
                func(*args, **kwargs)
            elif positron_args.wrapper:
                func(*args, **kwargs)
            elif positron_args.deploy:
...
def positron_deploy(positron_parameters):
    ...
    SOCKET_IO_PATH = '/backend/api/ws/socket.io'
    ...
    try:
        ...
        auth_token = positron_parameters.get('auth_token', config['DEFAULT']['UserAuthToken'])
...
def create_job():
    ...
    data = {"imageName": image_name, "fundingGroupId": funding_group, "environmentId": environment_id, "entryPoint": entry_point}
    debug(data)
