...
from feasytools import TimeFunc, ConstFunc, makeFunc, FloatLike
...
class Bus:
    ...
    def __init__(self, id: str, p_pu: FloatLike, q_pu: FloatLike):
        self.ID = id
        self.V = None
    ...
    def __str__(self):
        return f"Bus[ID={self.ID}, V={_tfv(self.V)}, Pd={self.Pd}, Qd={self.Qd}]"

    def __repr__(self):
        return self.__str__()
    
    def str_t(self, _t: int):
        return f"Bus[ID={self.ID}, V={_tfv(self.V)}, Pd={self.Pd(_t)}, Qd={self.Qd(_t)}]"
    ...
    def load(fp: TextIO, Sb: float = 1.0, loop: str = "0,0"):
        ...
        def parseline(ln: str,i:int):
            ...
            if loop_period > 0 and loop_times > 0:
                p = p * loop_times
                q = q * loop_times
            return ret[0].strip(), makeFunc(tl, p), makeFunc(tl, q)
...
class Grid:
    ...
    @staticmethod
    def fromFile(zip_name: str, holdShadowPrice: bool = False):
    ...
    def solve(self, _t: int, /, *, max_v_pu:float = 1.1, min_v_pu: float = 0.9) -> 'tuple[bool, float]':
        '''求解_t时刻的最优解, 返回(是否求解成功, 目标函数最优值)'''
        model = Model("model")
        max_v = max_v_pu ** 2
        min_v = min_v_pu ** 2
        max_l = (0.866 / self.Ib) ** 2  # 中国10kV配网限制功率15MVA，对应电流0.866kA
        # ---------------添加变量---------------
        # pg0[k]: 发电机有功
        # qg0[k]: 发电机无功
        # --> pg[j]: 节点所有发电机的有功
