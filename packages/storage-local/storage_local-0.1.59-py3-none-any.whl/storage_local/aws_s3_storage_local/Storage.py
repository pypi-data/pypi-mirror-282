...
class Storage(GenericCRUD, metaclass=MetaLogger, object=LOGGER_CODE_OBJECT):
    ...
    def _get_region_and_folder_by_file_type_id(self, file_type_id: int):
    ...
    def download_by_url_when_no_file(self, url: str) -> int:
        where = f"url = '{url}'"
        file = self.select_one_value_by_where(select_clause_value = "filename",
                                              schema_name = "storage",
                                              view_table_name = "storage_view",
                                              where = where)
        if file is None:
            file_id = self.select_one_value_by_where(select_clause_value = "storage_type_id",
                                              schema_name = "storage",
                                              view_table_name = "storage_view",
                                              where = where)
            remote_filename = self.select_one_value_by_where(select_clause_value = "path",
                                              schema_name = "storage",
                                              view_table_name = "storage_view",
                                              where = where)
            self.download(file_type_id=file_id, 
                          remote_filename=remote_filename,
                          local_path = os.path.dirname(os.path.dirname(__file__)),
                          )
            storage_id = self.save_url_content_in_storage(url = url, file_name = remote_filename, to_upload = False)
            return storage_id
        else:
            return self.select_one_value_by_where(select_clause_value = "storage_id",
                                              schema_name = "storage",
                                              view_table_name = "storage_view",
                                              where = where)