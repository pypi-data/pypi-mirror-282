...
class AwsS3Storage(StorageInterface, GenericCRUD, metaclass=MetaLogger, object=LOGGER_CODE_OBJECT):
    ...
    def download_file_by_storage_id(self, storage_id: int, local_file_path: str):
        remote_path_dict = super().select_one_dict_by_column_and_value(schema_name=self.default_schema_name, 
                                                    view_table_name=self.default_view_table_name,
                                                    column_name=self.storage_id_column_name,
                                                    column_value=storage_id,
                                                    select_clause_value="path,filename")
        remote_path = os.path.join(remote_path_dict["path"], remote_path_dict["filename"])
        self.boto3_client.download_file(self.bucket_name, remote_path, local_file_path)
