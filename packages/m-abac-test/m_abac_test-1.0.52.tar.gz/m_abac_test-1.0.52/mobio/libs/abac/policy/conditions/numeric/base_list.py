...
class NumericListCondition(ConditionBase, metaclass=ABCMeta):
    ...
    def __init__(self, values, what, qualifier=ConditionBase.Qualifier.ForAnyValue, **kwargs):
        values_format = [i for i in values if self.is_number(i)] if isinstance(values, list) else []
        what_format = [i for i in what if self.is_number(i)] if isinstance(what, list) else []
