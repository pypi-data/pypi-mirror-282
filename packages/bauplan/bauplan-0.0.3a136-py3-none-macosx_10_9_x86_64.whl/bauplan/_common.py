...
def _make_api_call(
    ...
    :meta private:
    """
    url = _get_catalog_host() + path
    headers = {'X-Bauplan-Api-Key': _get_api_key(api_key)}
    if pagination_token:
    ...
    out = schema.APIResponse.model_validate(res.json())
...
def _paginate_api(
    ...
    while not stop:
        if pagination_token:
            params['pagination_token'] = pagination_token
        out: schema.APIResponse = _make_api_call(
            api_key,
            method='get',
            path=path,
