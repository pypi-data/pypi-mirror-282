...
class Client:
    ...
    def _make_api_call(
        ...
        headers = {}
        if self._api_key:
            headers = {'X-Bauplan-Api-Key': self._api_key}
        elif self._private_session_user_token:
            headers = {'X-Bauplan-User-Session-Token': self._private_session_user_token}
        if pagination_token:
...
def _set_from_args(self: Client, kwargs: Dict) -> None:
    """
    private ways to handle other things passed to the client
    """
    self._private_user_session_token = kwargs.get('user_session_token', None)
