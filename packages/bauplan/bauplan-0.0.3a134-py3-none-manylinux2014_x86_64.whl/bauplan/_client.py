...
class Client:
    ...
    def query(
        ...
        .. code-block:: python
            ...
            my_table = client.query('SELECT c1 FROM my_table', branch='main')
    ...
    def query_to_generator(
        ...
        .. code-block:: python
            ...
            for row in client.query_to_generator('SELECT c1 FROM my_table', branch='main'):
                # do logic
    ...
    def plan_import(self, search_string: str, args: Optional[Dict] | None = None) -> PlanImportState:
        ...
        """
        try:
            return _plan(search_string=search_string, args=args)
        except grpc._channel._InactiveRpcError as e:
            if e.args:
                raise exceptions.RunError(e.args[0]) from e
            raise exceptions.RunError(e) from e
    ...
    def apply_import(
        self, plan_yaml: str, branch: str, table: str, args: Optional[Dict] | None = None
    ) -> ApplyPlanState:
        """
        ...
        """
        try:
            return _apply(plan_yaml=plan_yaml, branch=branch, table=table, args=args)
        except grpc._channel._InactiveRpcError as e:
            if e.args:
                raise exceptions.RunError(e.args[0]) from e
            raise exceptions.RunError(e) from e
    ...
    def _make_api_call(
        ...
        :meta private:
        """
        url = _common._get_catalog_host() + path
        headers = {'X-Bauplan-Api-Key': _common._get_api_key()}
        if pagination_token:
