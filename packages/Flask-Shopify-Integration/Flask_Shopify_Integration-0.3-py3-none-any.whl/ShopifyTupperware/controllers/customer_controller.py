...
class CustomerController:
    ...
    def update_customer_metafield():
        try:
            ...
            if types == 'delete':
                values.remove("\"Kode Voucher: %s - %s Expired: %s\"" %(data['voucher_code'], data['voucher_amount'], data['voucher_expired']))
            else:
                values.append("\"Kode Voucher: %s - %s Expired: %s\"" %(data['voucher_code'], data['voucher_amount'], data['voucher_expired']))
            repository.createCustomerMeta(sid, customer['id'], values)
            return jsonify(Status = "OK", Code = 200), 200
        except Exception as ex:
            return jsonify(Status = "Internal Server Error", Code = 500, Desc = ex.args), 500
