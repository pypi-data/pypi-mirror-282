...
def spin_wait_for_child(
    ...
    is_read_successful: bool = read_next_child_stdout_chunk(r_child_stdout)
    while (
        ...
        is_child_running_var: bool = not is_child_exited()
        if not is_child_running_var:
            # No point to spin with completed child -
            # break out and pump out data from the pipe:
            break
        is_read_successful: bool = read_next_child_stdout_chunk(r_child_stdout)
