...
class _SpecStore:
    ...
    def __init__(self, network: _StatsigNetwork, options: StatsigOptions, statsig_metadata: dict,
        ...
        self.spec_updater = SpecUpdater(network, options.data_store, options, diagnostics, sdk_key, error_boundary,
                                        statsig_metadata, shutdown_event)
        self.spec_updater.register_process_network_id_lists_listener(
            lambda id_lists: self._process_download_id_lists(id_lists))
        self.spec_updater.register_process_dcs_listener(lambda response, reason: self._process_specs(response, reason))

    def initialize(self):
        self._initialize_specs()
        self.initial_update_time = -1 if self.spec_updater.last_update_time == 0 \
    ...
    def _initialize_specs(self):
        if self._options.data_store is not None:
            ...
            self.spec_updater.load_config_specs_from_storage_adapter()

            if self.spec_updater.last_update_time == 0:
                self._log_process("Retrying with network...")
                self.spec_updater.download_config_specs(for_initialize=True)

        elif self._options.bootstrap_values is not None:
            self.spec_updater.bootstrap_config_specs()

        # If no updates from bootstrap or data_store, try to initialize from network
        if self.init_reason is not EvaluationReason.bootstrap and self.spec_updater.last_update_time == 0:
            self.spec_updater.download_config_specs(for_initialize=True)
    ...
    def _get_current_context(self):
        return Context.INITIALIZE if not self.spec_updater.initialized else Context.CONFIG_SYNC
