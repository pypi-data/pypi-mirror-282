...
def add_add_command(subparsers, parents):
    ...
    # We have to replicate these arguments to all the subparses to keep the command
    # positional order sane
    # parser.add_argument('--label', required=False, help='Name of the provider')
    # parser.add_argument('--skip-edit', required=False,
    #   help='Skip the interactive editor', action='store_true')
...
def add_modify_command(subparsers, parents):
    ...
    # The args are parsed for all command so to allow unkwown commands would required
    # enabling that on the entire tree
    # Instead just add all the possible args and suppress the notice so they are hidden
    # but validated if supplied
    parser.add_argument(
...
def add_provider(args):
    """Actual data processing routing for adding providers.

    All sub command route through this
...
def assign_provider(args):
    ...
    def check_container(args, storage_type, valid_types):
        if args.type not in valid_types:
            log.error(
                f"{storage_type} provider can be set for {' and '.join(valid_types)} "
                "only"
            )
            sys.exit(1)
...
def process_edit(config, function, id_=None):
    ...
    Uses the current api to validate changes so that we can determine when the
    configuration is correct and complete otherwise we look the user in the edit
    routine, unless user exits.

    Args:
        config (dict): The current object config to be injected
        function (func): The callable sdk command to process the data
        id (str): The id if we are going to be editing an existing provider
    """
    yaml = YAML()
    ...
    while not valid:
        ...
        try:
            ...
            log.error(str(exc))
            choice = input(
                "\nInvalid YAML. Enter 'q' to quit. Or press any other key to re-edit "
                "the configuration\n"
            )
            if choice in ("q", "Q"):
        ...
        try:
            ...
            choice = input(
                "\nEnter 'q' to quit. Or press any other key to re-edit the "
                "configuration\n"
            )
            if choice in ("q", "Q"):
                log.info("Provider has not been saved")
...
def get_config_vals(class_, type_):
    """
    Provides a configuration dict that contains the needed fields for the specified type
    and class so that data can be injected into this dic to present to the user.
    """
