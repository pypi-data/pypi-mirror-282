...
class ContainerFactory:
    ...
    def resolve(self, context, create=True):
        ...
        If the node definition is ambiguous (i.e. missing an intermediate level such as
        project), then this function returns None. This is fine if there is nothing to
        upload at the given node.
    ...
    def _validate_container(cid, uid, label, container_type):
        ...
        if not any((cid, uid, label)):
            raise ValueError(
                f"Couldn't create {container_type} container because missing all of "
                "(id, uid, label)"
            )
        ...
        if (
            ...
            raise ValueError(
                f"Couldn't create {container_type} container because it does not have "
                "a label"
            )
