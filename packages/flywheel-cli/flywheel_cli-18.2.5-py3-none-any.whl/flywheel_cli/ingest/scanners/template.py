...
class TemplateScanner(AbstractScanner):
    ...
    def _get_parent_context(context: s.ItemContext) -> s.SourceContainerContext:
        """
        Get parent container context from item context, like if group, project in the
        context this method will return the project context
        """
        for cont_level in ("acquisition", "session", "subject", "project", "group"):
            cont_ctx = getattr(context, cont_level, None)  # type: ignore
            if cont_ctx:
                break
        return cont_ctx  # type: ignore
