...
class SyncQueue(WorkQueue):
    ...
    def write_audit_log(
        ...
        with self._lock:
            self.audit_log_fp.write(
                f"{src_path},{dest_path},{status},{existing},{error_message},"
                f"{action_taken}\n".encode()
            )
...
class StoreTask(Task):
    ...
    def execute(self):
        """Compare src and dst (if it exists) and create/update(prompt)/skip as
        appropriate"""
        # Perform zip storing
        ...
        if self.queue.unpack and self.src_file.is_packed:
            ...
            log.debug(
                f"{this_thread.name} Unpacking {self.src_file.name}, "
                f"exists={out_exists}, size={self.src_file.size}"
            )
            use_local = (
    ...
    def get_bytes_processed(self):
        """Return number of bytes read from the source (equals bytes written to
        destination)"""
        ...
        for name, group in self.groups.items():
            ...
            if unskipped_count > 0:
                if group.completed == group.total_count:
                    ...
                    # TODO in-depth subclassing of WorkQueue for appropriate attribute
                    # name
