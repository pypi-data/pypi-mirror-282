    ...
    def _create_container(self, container, parents):
        ...
        while retries := 3:
            try:
                ...
                retries -= 1
                if not (level == "project" and exc.status == 409 and retries):
                    raise
                label = self.sidecar_project_name
                log.warning(f"Error creating sidecar project {label}: {exc}")
                time.sleep(random.randint(1, 5))
                label_ = f"{label[:-10]}{int(time.time())}"
                src_context = container.src_context
                self.sidecar_project_name = src_context.label = label_
                update = {
                    "id": container.id,
                    "src_context": src_context.model_dump(exclude_none=True),
                }
                self.update_containers.push(update)
