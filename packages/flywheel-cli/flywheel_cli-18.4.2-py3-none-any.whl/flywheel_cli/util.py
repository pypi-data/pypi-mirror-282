...
def load_auth_config():
    ...
    if platform.system() == "Windows":
        ...
        try:
            config["key"] = keyring.get_password("Flywheel Credentials", "fw_api_key")
...
def save_api_key(api_key, root=False):
    ...
    - Windows: data will be stored within Windows credential manager (fallback to plaintext)
    - MacOS, Linux: data will be stores as is with specific permissions
    Args:
        ...
        try:
            if config.get("key", None):
                keyring.set_password(
                    "Flywheel Credentials", "fw_api_key", config.get("key", "")
                )
                config.pop("key", None)
        except keyring.errors.NoKeyringError as e:
            log.debug(f"No Keyring Backend: {e} ")
...
def get_http_config() -> HttpConfig:
    ...
    config = HttpConfig(
        ...
        retry_total=5,
        retry_allowed_methods=["GET", "HEAD", "PUT", "OPTIONS"],
    )
    return config
