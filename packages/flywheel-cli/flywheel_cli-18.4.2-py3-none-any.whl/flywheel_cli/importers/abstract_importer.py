...
class AbstractImporter(ABC):
    ...
    def verify(self):
        """
        Verify the upload plan, returning any messages that should be logged, with
        severity.
        ...
        for _, container in self.container_factory.walk_containers():
            ...
            if container.container_type in util.NO_FILE_CONTAINERS:
                ...
                for path in container.files:
                    ...
                    msg = (
                        f"File {fname} cannot be uploaded to "
                        f"{container.container_type} {cname} - files are not supported "
                        "at this level"
                    )
                ...
                for _ in container.packfiles:
                    msg = (
                        f"pack-file cannot be uploaded to {container.container_type} "
                        f"{cname} - files are not supported at this level"
                    )
                    results.append(("warn", msg))
        ...
        if self.config.check_unique_uids:
            ...
            for container in conflict_containers:
                msg = (
                    ...
                    f'{getattr(container, "label", "UNKNOWN")} has duplicates the UID '
                    "of an existing container (uid={container.uid})"
                )
                results.append(("warn", msg))
