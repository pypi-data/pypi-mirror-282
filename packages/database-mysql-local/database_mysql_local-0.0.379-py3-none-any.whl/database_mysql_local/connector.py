...
class Connector(metaclass=MetaLogger, object=LOGGER_CONNECTOR_CODE_OBJECT):
    @staticmethod
    def connect(schema_name: str, ignore_cache: bool = False) -> Connector:
        if ignore_cache and schema_name in connections_pool:
            connections_pool.pop(schema_name)
        ...
        if (schema_name in connections_pool and
            ...
            if connections_pool[schema_name].connection.is_connected():
                ...
                connector = connections_pool[schema_name]
            else:
                # reconnect
