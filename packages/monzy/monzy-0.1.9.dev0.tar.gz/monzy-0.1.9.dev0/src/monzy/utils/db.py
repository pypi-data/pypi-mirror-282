...
class Db:
    ...
    def delete(self, table: str, schema: str, data_ids: list) -> None:
        """Method to delete data from a table in the database.

        Args:
            table (str): Table name from which data will be deleted.
            data_ids (list): List of IDs or primary keys to delete.
        """
        # Construct SQLAlchemy delete statement
        metadata = sqlalchemy.MetaData()
        table_obj = sqlalchemy.Table(table, schema, metadata, autoload_with=self.conn)
        delete_stmt = sqlalchemy.delete(table_obj).where(table_obj.c.id.in_(data_ids))

        # Execute the delete statement
        self.conn.execute(delete_stmt)
        logger.info(f"Deleted transactions with IDs {len(data_ids)} from {table}")