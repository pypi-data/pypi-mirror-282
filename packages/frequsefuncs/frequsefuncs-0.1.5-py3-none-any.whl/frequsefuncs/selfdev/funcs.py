...
def func_read_sql_from_file(fpath_str_sql, str_split, str_var_head):
    ...
    dict_str_sql = {}                         #准备存放各段sql和变量名
    for str_sql in lst_str_sql:
        try:
            str_sql_var, str_var_val = func_parse_var_n_val(str_sql, str_var_head)
            dict_str_sql[str_sql_var] = str_var_val
        except:
            print(str_sql)
            raise
    return dict_str_sql
