...
class SwsApiClient:
    ...
    def __get_authclient_credentials_from_secret(self, env_name="SWS_USER_CREDENTIALS_SECRET_NAME") -> AuthClientCredentials:
        logger.debug(f"Fetching auth client credentials from secret: {env_name}")
        secret_name = os.getenv(env_name)
        self.session = boto3.Session() if self.session is None else self.session
        self.secret_manager = self.session.client('secretsmanager') if self.secret_manager is None else self.secret_manager
