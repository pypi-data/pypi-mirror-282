...
N = # Blocks
M = Total markers = 2 x dimension(d) x M

- Establish the grid markers in each dimension and map their ordinates to them
  using a binary search. NLog(N)
- Create a cross section for each marker. (N^d)^2 at worse
