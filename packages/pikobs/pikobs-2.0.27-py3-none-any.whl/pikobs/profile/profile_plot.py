  ...
  #  print (  ' graphe1 =',graphe1 )
    ...
    for filen in  files:
     ...
     query=f"""create temporary table  boites1 as select 
                               ...
                               sum(n) AS N  ,
                               sum(SUMBIAS_CORR)/sum(N) AS bcorr from profile where varno="{varno}" and id_stn ='{id_stn}' and nomi group by vcoord;"""
   ...
   #  print (  ' VARNO = -------------------',varno,varno   in graphe_limites,graphe1[filenumb-1] )
    ...
    #==========================================================
     pylab.savefig(f'{pathwork}/profile/{id_stn[0]}_{varno}.png',format='png',dpi=100)
    #==========================================================
