...
def scatter_plot(
   ...
   for fonction  in  fonctions:
    for Proj in proj:
       ...
       if id_stn =='all' and vcoord=='all' :
          crite_id_stn = "    "
          crite ="  "
       if id_stn =='all' and vcoord!='all' :
          crite_id_stn = f"  WHERE vcoord = '{vcoord}'   "
          crite = " vcoord in ({vcoord}) and "

       if id_stn !='all' and vcoord=='all' :
          crite_id_stn = f"  WHERE id_stn= '{id_stn}'    "
          crite = "    "
       if id_stn !='all' and vcoord!='all' :
          crite_id_stn = f"  WHERE vcoord = {vcoord} and id_stn= '{id_stn}'  "
          crite = f" vcoord in ({vcoord}) and "
    ...
    #   ax3 = fig.add_axes()
       ...
       if len(filesin)<1:
           ...
           ax.text(text1_x, y_coord, namesin[0], fontsize=fontsize, color='red', transform=ax.transAxes)
         #  ax.text(text2_x, y_coord, " VS ", fontsize=fontsize, color='black', transform=ax.transAxes)
         #  ax.text(text3_x, y_coord, namesin[1], fontsize=fontsize, color='blue', transform=ax.transAxes)
     ...
     #  ax.add_feature(cartopy.feature.BORDERS, zorder=10)
       ...
       if vcoord == 'all':
          plt.savefig(f'{pathwork}/plot_{family}/{fonction}_{Proj}_{id_stn}_{region}_{vcoord}.png', format='png')

       else:
          plt.savefig(f'{pathwork}/plot_{family}/{fonction}_{Proj}_{id_stn}_{region}_{int(vcoord)}.png', format='png')
       plt.close(fig)
