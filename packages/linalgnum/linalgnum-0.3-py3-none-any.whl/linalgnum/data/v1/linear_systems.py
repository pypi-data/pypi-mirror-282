...
def jacobi(linear_system: LinearSystem, max_iter=10000, tolerance=1e-8, x0=None):
    ...
    if x0 is None:
        ...
        x = x0.copy()
    # Per criterio di terminazione usiamo il rapporto di cambiamento tra il vettore x precedente e quello attuale
    change_ratio = lambda xa, xb: np.linalg.norm(xa - xb) / np.linalg.norm(xa)
    # Eseguo un massimo di max_iter iterazioni
    for iter in range(max_iter):
        # Eseguo una copia del vettore x per calcolare il rapporto di cambiamento
        prev_x = x.copy()
        for i in range(linear_system.variables): # Ciclo per un numero pari al numero di variabili
            ...
            rowsum = sum([ linear_system.A[i, j] * prev_x[j] for j in range(linear_system.variables) if i != j ])
            # Aggiorno il valore della variabile x[i] con la formula di Jacobi
            x[i] = (linear_system.b[i] - rowsum) / linear_system.A[i,i]
        # Eseguo il controllo per il criterio di terminazione
        if change_ratio(prev_x, x) < tolerance:
            break
    return iter, x
