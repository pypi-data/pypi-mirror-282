...
def build_dataset(entity_id, selected_features, view_name, schema_name=tdfs4ds.SCHEMA,
    ...
    entity_null_substitute (dict, optional): Dictionary to handle null substitution for entity IDs. Defaults to an empty dictionary.

    Returns:
    str or DataFrame: A SQL query string if query_only is True, otherwise a DataFrame of the created dataset view.
    """

    # Determine the valid time statement based on the FEATURE_STORE_TIME
    if tdfs4ds.FEATURE_STORE_TIME is None:
        validtime_statement = 'CURRENT VALIDTIME'
    else:
        validtime_statement = f"VALIDTIME AS OF TIMESTAMP '{tdfs4ds.FEATURE_STORE_TIME}'"
    ...
    # Retrieve available entity ID records and the list of features
    ...
    # Determine the time condition based on the presence of a time manager
    if time_manager is None:
        ...
        if tdfs4ds.DEBUG_MODE:
            ...
            print('Time condition', time_condition)
    else:
        validtime_statement = ''
