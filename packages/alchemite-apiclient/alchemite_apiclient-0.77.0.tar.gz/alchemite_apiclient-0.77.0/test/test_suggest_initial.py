...
def test_suggest_initial(set_insecure_transport, api_default, example_dir):
    suggest_initial_args_path = os.path.join(
        example_dir, "suggest_initial_args.json"
    )
    ...
    delete_response = api_default.suggest_initial_job_id_delete(
        suggest_initial_job_id
    )
