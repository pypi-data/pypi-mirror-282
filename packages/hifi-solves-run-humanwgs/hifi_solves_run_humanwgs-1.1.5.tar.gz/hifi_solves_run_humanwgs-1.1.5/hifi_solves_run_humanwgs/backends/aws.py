...
def check_file_exists(bucket, path_prefix, file_path, sample_id, file_type):
    ...
    s3 = boto3.client("s3")
    try:
        s3.head_object(Bucket=bucket, Key=remote_path)
...
def generate_inputs_json(
    ...
    Args:
        ...
        workflow_tabular_outputs_bucket (str): Bucket where tabular workflow outputs will be written
    ...
    """
    samples = sample_info.drop(columns=["family_id"]).to_dict(orient="records")
    ...
    workflow_tabular_outputs_bucket = f"s3://{workflow_tabular_outputs_bucket}"
    ...
    humanwgs_inputs = {
        ...
        "HumanWGS_wrapper.tabular_outputs_bucket": workflow_tabular_outputs_bucket,
    }
