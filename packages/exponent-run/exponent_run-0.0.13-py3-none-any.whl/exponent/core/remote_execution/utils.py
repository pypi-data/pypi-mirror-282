...
from typing import Any, NoReturn, TypeGuard, TypeVar, cast, overload
...
async def safe_deserialize_list_response_data(
    response_data: Response,
    deserialize: Callable[[Any], TModel],
    log_and_reraise: Callable[[Exception, str | None], Awaitable[NoReturn]],
) -> list[TModel]:
    try:
        response_json = response_data.json()
    except json.JSONDecodeError as e:
        await log_and_reraise(e, response_data.text)
    results = []
...
async def safe_deserialize_response_data(
    response_data: Response,
    deserialize: Callable[[Any], TModel],
    log_and_reraise: Callable[[Exception, str | None], Awaitable[NoReturn]],
) -> TModel:
    ...
    try:
        ...
        # The return statement is here to make
        # the linter happy, this just reraises
        return await log_and_reraise(e, response_data.text)
