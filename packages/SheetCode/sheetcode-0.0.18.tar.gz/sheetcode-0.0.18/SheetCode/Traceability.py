...
def EvaluateSheetsCoverage(validationMethod):
    ...
    mergedCoverage = coverage.groupby(["Req ID", "Req Text", "Validation Method", "Test Sheet Name"]).agg({"Test sheet": lambda x: '\r\n'.join(map(str, pd.unique(x)))})
    ...
    # Rename columns
    mergedCoverage = mergedCoverage.rename(columns={'Test Sheet Name': 'Expected Test Sheet', 'Test sheet': 'Actual Test Sheet'})
    
    mergedCoverage.to_excel(os.path.join(os.environ["TRACEABILITY_PATH"], "coverage.xlsx"))
