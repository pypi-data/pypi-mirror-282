...
class CBR__Custom__Deploy_Lambda(Type_Safe):
    ...
    def cbr_custom__create_lambda(self):
        ...
        if deploy_lambda.exists() is False:
            ...
            try:
                from deploy.lambdas.CBR_Deploy_Lambda import CBR_Deploy_Lambda      # todo: refactor since there is quite a lot of repeated code here
                from lambdas.cbr_lambda__dev import run
                cbr_deploy_lambda = CBR_Deploy_Lambda(run)
                cbr_deploy_lambda.deploy_lambda = self.deploy_lambda()
                aws_config.set_aws_session_region_name('eu-west-1')
                kwargs = { 'lambda_name': self.lambda_name(), 'version': version__cbr_website }
                cbr_deploy_lambda.deploy_lambda_function(**kwargs)
                print(" ... waiting for lambda update to complete")
                cbr_deploy_lambda.deploy_lambda.lambda_function().wait_for_function_update_to_complete()
    ...
    def cbr_custom__configure_lambda(self):
        ...
        env_variables = { 'S3_DEV__VERSION'      : self.version_file()                ,
                          ...
                          'CBR__CONFIG_FILE'      : CBR__CONFIG_FILE                    ,
                          'EXECUTION_ENV'        : 'DEV'                              ,
