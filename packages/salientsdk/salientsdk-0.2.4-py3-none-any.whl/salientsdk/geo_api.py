...
def add_elevation(
    ...
    **kwargs,
    ...
    Args:
        loc (Location): The location to query.
        ds (xr.Dataset): The dataset to add `elevation` to
        destination (str): The destination directory for downloaded files.
        force (bool): If False (default), don't download the data if it already exists
        session (requests.Session): The session object to use for the request
        apikey (str | None): The API key to use for the request.
    ...
    Returns:
        xr.Dataset: The dataset with `elevation` added as a data variable
    """
    variables = "elevation"
    args = locals()
    args.pop("ds")
    args["variables"] = "elevation"
    elev = xr.load_dataset(geo(**args))

    if ds.lat.equals(elev.lat) and ds.lon.equals(elev.lon):
        # we should be able to trivially concatenate the two datasets
        pass
    elif len(elev.lat) > 1 and len(elev.lon) > 1:
        elev = elev.interp(lat=ds.lat, lon=ds.lon, method="linear")

    ds = xr.merge([ds, elev], combine_attrs="override", compat="override")
