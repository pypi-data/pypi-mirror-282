...
class AI:
    def __init__(self, personality_message: str = None, main_prompt: str = None, verbose: bool = False, timeout: int = 300, fine_tuned_model=False, fine_tuned_context_window=16000) -> None:
    ...
    def within_context_window(self, text : str = None, model=None):
        if not self.fine_tuned_model:
            return self.get_tokens(text) < self.model_token_limits.get(model, 16000) if model else 16000
        else:
    ...
    def model_check(self, token_count, model):
        if self.fine_tuned_model:
            ...
            print('no', self.fine_tuned_model)
            ...
            if model in suitable_models or self.fine_tuned_model:
                return model
