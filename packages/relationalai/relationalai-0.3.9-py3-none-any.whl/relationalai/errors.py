...
class RAIException(Exception, metaclass=RAIPostInitMeta):
    ...
    def update_exception_stack(self):
        _, _, tb = sys.exc_info()
        # If we're not in an exception context, create a new traceback
        if tb is None:
            try:
                raise self
            except Exception:
        ...
        # Create a new traceback object starting from the current frame
        new_tb = TracebackType(tb.tb_next, tb.tb_frame, tb.tb_lasti, tb.tb_lineno)
        # Update the exception with the new traceback
        self.__traceback__ = new_tb
        return self


    def clone(self):
        return self.update_exception_stack()
