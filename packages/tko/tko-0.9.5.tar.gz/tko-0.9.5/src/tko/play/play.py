...
class Play:
    ...
    def build_bar_header(self, dx:int) -> Sentence:
        ...
        header.addf(self.get_cb(self.flags_on("flags_bar")), f"Flags [f]").addt(" ")
        header.addf(self.get_cb(self.flags_on("help")), "Help [h]").addt(" ")
        header.addf(self.get_cb(self.flags_on("skills_bar")), f"Skill [s]").addt(" ")

        xp = XP(self.game)

        # header.addf(self.get_cb(self.flags_on("cont")), "Count [c]").addt(" ")
        # header.addf(self.get_cb(self.flags_on("perc")), "Percent [p]").addt(" ")
        # header.addf(self.get_cb(self.flags_on("goal")), "Goals [g]").addt(" ")
        # header.addf(self.get_cb(self.flags_on("admin")), "Admin [A]").addt(" ")
    ...
    def show_skill_bar(self, xinit, xp, xp_dx, cols, deeper):
            total_perc = 0
    ...
    def show_flags_bar(self, xinit, xp_dx, deeper):

            frame = Frame(2, xinit).set_inner(deeper, xp_dx - 3).set_border_rounded()
            frame.set_header(Sentence().addt("{").addf("/", "Flags").addt("}"), "^")
            frame.draw()

            def add_flag(_flags, _flag, _text):
                _fmt = self.get_cb(self.flags_on(_flag))
                _flags.append(Sentence().addf(_fmt, _text).center(xp_dx - 5, " ", _fmt))
    ...
    def show_items(self):
        ...
        if self.flags_on("skills_bar"):
            xp_dx = max(20, dx // 3)
            task_dx -= xp_dx - 1
            self.show_skill_bar(cols - xp_dx, xp, xp_dx, cols, dy)
        ...
        frame_tasks = Frame(2, task_x_init).set_inner(dy, task_dx).set_border_rounded()
