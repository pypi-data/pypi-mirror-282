...
class Play:
    ...
    def process_down(self):
        ...
        if isinstance(obj, Task):
            ...
            self.input_layer = Input().timer("Essa não é uma tarefa de código", 2)
    ...
    def show_items(self):
        ...
        # lines, cols = Fmt.scr.getmaxyx()
        self.reload_options()
        if self.input_layer is not None and not self.input_layer.enable:
            Fmt.scr.clear()
        else:
            Fmt.erase()
        dx = cols - 3
        ...
        while True:
            ...
            if self.input_layer and self.input_layer.enable:
                ...
                if value == ord("q"):
                    ...
                    # self.test_color(scr)
                    self.input_layer = Input().timer("Desabilitando flags", 2)
            self.save_to_json()
            time.sleep(0.1)
