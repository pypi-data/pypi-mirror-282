...
class Play:
    ...
    #     skills, _total = self.game.get_skills_resume()
    ...
    def show_flags_bar(self, xinit, dx, dy):
            ...
            add_flag(flags, "folder", "Folder  [l]")
    ...
    def build_help_bar(self, limit: int) -> Sentence:
        ...
        if "help" in self.flags:
            msgs += ["Down[d]", "Lang[l]", "Grade[0-9]", "Expand[>]", "Collapse[<]",  "MassMark[M]"]
        help = Sentence().addt(" ")
        for x in msgs:
            help.addf("", x).addt(" ")
        help.data.pop()
    ...
    def show_help(self):
        ...
        help.addt("Para navegar entre tópicos, missões e tarefas utilize as setas")
        help.adds(Sentence().addt("  ").addf("G", "f").addt(" - Mostrar ou esconder a barra de flags"))
        help.adds(Sentence().addt("  ").addf("G", "s").addt(" - Mostrar ou esconder a barra de skills"))
        help.adds(Sentence().addt("  ").addf("G", "l").addt(" - Mudar a linguagem de download dos rascunhos"))
        help.adds(Sentence().addt("  ").addf("G", ">").addt(" - Expandir tudo"))
        help.adds(Sentence().addt("  ").addf("G", "<").addt(" - Contrair tudo"))
        help.addt("")
        help.adds(Sentence().addt("Estando em um tópico ou missão"))
        help.adds(Sentence().addt("  ").addf("G", "esquerda e direita").addt(" - Expandir ou contrair"))
        help.addt("")
        help.adds(Sentence().addt("Estando em uma tarefa"))
        help.adds(Sentence().addt("  ").addf("G", "enter ou espaço").addt(" - Marcar ou desmarcar"))
        help.adds(Sentence().addt("  ").addf("G", "0 a 9").addt(" - Definir a nota parcial"))
        help.adds(Sentence().addt("  ").addf("G", "d").addt(" - Baixar a tarefa"))
