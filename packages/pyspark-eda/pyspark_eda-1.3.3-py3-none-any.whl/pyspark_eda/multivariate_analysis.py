    ...
    if vif_analysis and numerical_columns:
        for column in numerical_columns:
            try:
                ...
                df_pd = df.select(numerical_columns).dropna().toPandas()

                # Calculate VIF for the current column
                vif = variance_inflation_factor(df_pd.values, df_pd.columns.get_loc(column))
