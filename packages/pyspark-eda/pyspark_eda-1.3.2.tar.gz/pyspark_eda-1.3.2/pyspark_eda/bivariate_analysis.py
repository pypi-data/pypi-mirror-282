        ...
        for i in range(len(categorical_columns)):
            for j in range(i + 1, len(categorical_columns)):
                ...
                try:
                    ...
                    if counts.size > 0:
                        counts = np.nan_to_num(counts)  # Replace NaN values with zero
        ...
        for num_col in numerical_columns:
            for cat_col in categorical_columns:
                try:
                    ...
                    pdf = df.select(cat_col, num_col).dropna().toPandas()
                    # Perform ANOVA
                    category_groups = [group[num_col].values for name, group in pdf.groupby(cat_col)]
                    f_val, p_val = f_oneway(*category_groups)
