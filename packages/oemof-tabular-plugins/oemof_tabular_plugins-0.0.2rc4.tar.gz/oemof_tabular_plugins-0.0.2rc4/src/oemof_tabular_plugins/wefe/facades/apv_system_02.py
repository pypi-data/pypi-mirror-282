...
class APVSystem(Converter, Facade):
    ...
    def build_solph_components(self):
        ...
        def apv_geometry(instance):
            ...
            geometry = {
                float(lat): {
                    k: convert_from_serializable(v) for k, v in lat_results.items()
                }
                for lat, lat_results in geometry.items()
            }
            ...
            def interpolate_values(lat, geometry):
                ...
                for xgap in xgaps:
                    ...
                    fbifacial_interp_funcs.append(
                        interp1d(lats, fbifacials, kind="linear")
                    )
                    fshading_interp_funcs.append(
                        interp1d(lats, fshadings, kind="linear")
                    )

                # Calculating new values for the given latitude
                new_fbifacials = [
                    interp_func(lat).item() for interp_func in fbifacial_interp_funcs
                ]
                new_fshadings = [
                    interp_func(lat).item() for interp_func in fshading_interp_funcs
                ]
            ...
            fareas = [
                (1 + fbifacials[i]) * x / ((1 + fbifacials[0]) * (x + xgaps[i]))
            ...
            opti_model.fshading_pieces = Piecewise(
                opti_model.fshading,
                opti_model.xgap,
                pw_pts=xgaps,
                f_rule=fshadings,
            ...
            opti_model.farea_pieces = Piecewise(
                opti_model.farea,
                opti_model.xgap,
                pw_pts=xgaps,
                f_rule=fareas,
            ...
            opti_model.min_shading_constraint = Constraint(
                expr=opti_model.fshading >= min_fshading
            )
            ...
            fbifacial_optimal = (
                (farea_optimal * (1 + fbifacials[0]) * (xgap_optimal + x)) / x
            ) - 1
        ...
        def apv_production(instance):
            ...
            date_time_index = pd.date_range("1/1/2022", periods=8760, freq="H")
            ...
            df["f_temp"] = df["t_air"].apply(lambda t_air: temp(t_air, t_opt, t_base))
            ...
            def solar(cum_temp, t_sum, i50a, i50b, f_solar_max):
                ...
                if cum_temp < 1:
                    ...
                    f_solar = f_solar_max / (
                        1 + np.exp(0.01 * (cum_temp - (t_sum - i50b)))
                    )
                else:
                    f_solar = 0
                return f_solar
            ...
            df["biomass_efficiency"] = (
                instance.fshading * rue * df["f_solar"] * df["f_temp"] * df["f_heat"]
            )
            ...
            df["pv_efficiency"] = df.apply(
                lambda row: power(
                    p_rated, row["ghi"], rad_ref, row["t_air"], t_ref, noct
                ),
                axis=1,
            )
