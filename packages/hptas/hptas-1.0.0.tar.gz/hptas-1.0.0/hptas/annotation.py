...
from .component import Segment, Isoform, SNP, Task
...
_ISOFORM_DB_FILENAME = 'transcript.db'
_SNP_DB_FILENAME = 'snp.db'
_TASK_DB_FILENAME = 'phasing.db'
...
class AnnotationDB:
    ...
    def _create_task(self, n: int, segment_id: str, phased: bool, ploidy: int, snp_ids: List[str]) -> int:
        ...
        if n != 0 and n % 10000 == 0:
            self._mc.handle_progress('{} phasing genes created...'.format(n))
        if phased:
            task_id = '{}-PHASING'.format(segment_id)
        else:
...
def generate_annotation(gff_path, vcf_path, vcf_sample, add_chrom_prefix, out_dir):
    """Generate annotation."""

    segment_feature_name = 'gene'
    isoform_feature_name = 'transcript'
    gene_name_attr = 'gene_name'
    isoform_name_attr = 'ID'
    ...
    with AnnotationDB(out_dir, initialize=True, mc=mc) as anno_db:
        ...
        del segments
        anno_db.create_tasks(2, HeteroSNPReader(vcf_path, vcf_sample, 2, add_chrom_prefix, mc))
