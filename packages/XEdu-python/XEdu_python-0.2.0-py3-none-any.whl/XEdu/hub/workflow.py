        ...
        if checkpoint is None and self.task in default_task: # 若不指定权重文件，则使用对应任务的默认模型
            ...
            if not os.path.exists(checkpoint): # 本地未检测到模型，云端下载默认模型
                ...
                model_name_map_download = {
                    ...
                    "depth_anything":'https://download.openxlab.org.cn/repos/file/xedu/hub-model/main?filepath=depth_anything.onnx&sign=6f14e5a72db915ac7a767a124c76242a&nonce=1719306041356',
                    "embedding_audio":"https://download.openxlab.org.cn/repos/file/xedu/hub-model/main?filepath=embedding_audio.onnx&sign=f99f65cdcccc6e3bdb61ac4ce0a74120&nonce=1719818647701",
                }
            ...
            if not os.path.exists(checkpoint[0]):
                ...
                if not os.path.exists(path):
                    os.mkdir(path)
                downloader = Downloader("https://download.openxlab.org.cn/repos/file/xedu/hub-model/main?filepath=seg_sam_encoder.onnx&sign=54ce80fbe91914e7f058fe83e35962d1&nonce=1718180611636", checkpoint[0],output_dir=path)
                downloader.start()
