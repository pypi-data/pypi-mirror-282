...
def run(
  *, train: str, val: str, weights: str, base_path: str, epochs: int,
  ...
  val_ds, val_n = mo.records.read_dataset(val, mode='keep_order', batch_size=hparams['batch_size'], char2num=char2num)
  ...
  history = mo.finetuning.loop(
    ...
    n_val_batches=val_n and val_n//hparams['batch_size']
  )
  if logstream:
