    ...
    while True:
        try:
            ...
            if task['update'] == False:
                logger.info(
                    f"Executando a task: {task['task']['nomProcesso']}"
                )
                await perform_task(task['task'])
            else:
                logger.info('Solicitando atualização...')
                sys.exit(1)
...
async def main_process():
    ...
    print(ascii_banner + f' versão: {VERSION}\n')
    logger.info('Worker em execução.')
    print('Worker em execução.')  # Mensagem de inicialização
