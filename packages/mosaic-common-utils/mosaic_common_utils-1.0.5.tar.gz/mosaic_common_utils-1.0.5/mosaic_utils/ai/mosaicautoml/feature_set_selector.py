...
class FeatureSetSelector(BaseEstimator, SelectorMixin):
    ...
    def __init__(self, subset_list, sel_subset):
        ...
        subset_list: string, required
            ...
            There should be 3 columns on the table, including subset names,
            ...
            The feature names in the files must match those in the
    ...
    def fit(self, X, y=None):
        ...
        if isinstance(X, pd.DataFrame):
            ...
            self.feat_list = sorted(
                list(set(sel_uniq_features).intersection(set(self.feature_names)))
            )
            self.feat_list_idx = [
                list(X.columns).index(feat_name) for feat_name in self.feat_list
            ]
    ...
    def transform(self, X):
        ...
        (n_samples, n_features + 1 + n_classes) for classifier with
