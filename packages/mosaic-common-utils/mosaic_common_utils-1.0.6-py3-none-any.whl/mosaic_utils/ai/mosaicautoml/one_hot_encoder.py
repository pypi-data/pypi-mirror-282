...
class OneHotEncoder(BaseEstimator, TransformerMixin):
    ...
    categorical_features: "all" or array of indices or mask
        ...
        - 'auto' (default): Select only features that have less than 10 unique
           values.
    ...
    >>> enc = OneHotEncoder()
    >>> enc.fit([[0, 0, 3], [1, 1, 0], [0, 2, 1], [1, 0, 2]])  # doctest:
        +ELLIPSIS
                ...
                for unique_value in unique:
                    if np.isfinite(unique_value):
                        if sparse.issparse(X):
                            ...
                            indptr_end = X.indptr[column + 1]
                            count = np.nansum(
                                unique_value == X.data[indptr_start:indptr_end]
                            )
                        else:
                        ...
                        if sparse.issparse(X):
                            ...
                            indptr_end = X.indptr[column + 1]
                            count = np.nansum(
                                ~np.isfinite(X.data[indptr_start:indptr_end])
                            )
                        else:
                ...
                for unique_value in unique:
                    if unique_value not in do_not_replace_by_other[-1]:
                        if sparse.issparse(X):
                            ...
                            ] = SPARSE_ENCODINGS["OTHER"]
                        else:
                            X[:, column][
                                X[:, column] == unique_value
                            ] = SPARSE_ENCODINGS["OTHER"]
    ...
    def fit_transform(self, X, y=None):
        ...
        if self.categorical_features == "auto":
            self.categorical_features = auto_select_categorical_features(
                X, threshold=self.threshold
            )

        return _transform_selected(
            X, self._fit_transform, self.categorical_features, copy=True
        )
            ...
            for i, n_value_check in enumerate(n_values_check):
                if (n_value_check - 1) >= self.n_values_[i]:
                    if sparse.issparse(X):
                        ...
                        X.data[indptr_start:indptr_end][
                            X.data[indptr_start:indptr_end] >= self.n_values_[i]
                        ] = 0
                    else:
    ...
    def transform(self, X):
        ...
        """
        return _transform_selected(
            X, self._transform, self.categorical_features, copy=True
        )
