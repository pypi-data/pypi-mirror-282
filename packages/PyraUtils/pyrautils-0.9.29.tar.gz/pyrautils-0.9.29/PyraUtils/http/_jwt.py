...
Modified on 2024-03-18 16:36:32
...
class JWTDecorator:
    ...
    def require_auth(self, auth_header):
        ...
        try:
            ...
            self.return_auth_error(auth_header, {'message': 'Token已过期'})
            return False
    ...
    def jwtauth(self, handler_class):
        ...
        def _execute(self, transforms, *args, **kwargs):
            if not self.require_auth(self, kwargs):
                return  # 认证失败时直接返回，不再执行后面的操作
...
class JWT:
    ...
    def jwt_encode(self, payload, algorithm='HS256', exp_seconds=3600):
        """
        生成JWT token字符串。
    ...
    def jwt_decode(self, jwt_token, audience=None, algorithms=['HS256']):
        ...
        """
        try:
            if jwt_options.get('verify_aud', True) and audience:
                decoded = jwt.decode(
                    jwt_token,
                    self.secret_key,
                    audience=audience,
