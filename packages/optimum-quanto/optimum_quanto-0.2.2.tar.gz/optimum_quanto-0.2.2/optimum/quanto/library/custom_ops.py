...
from torch import _C, Tensor, _library, _ops, autograd, library
...
class CustomOpDef:
    ...
    def register_kernel(self, device_types: device_types_t, fn: Optional[Callable] = None, /) -> Callable:
            ...
            for device_type in dtypes:
                if device_type not in self._backend_fns:
                    ...
                    def backend_impl(*args, **kwargs):
                        ...
                        storages = {id(tensor.untyped_storage()) for tensor in iter_tensors(args, kwargs)}
        ...
        if schema.is_mutable:
            ...
            def adinplaceorview_impl(keyset, *args, **kwargs):
                ...
                with _C._AutoDispatchBelowADInplaceOrView():
                    return self._opoverload.redispatch(keyset & _C._after_ADInplaceOrView_keyset, *args, **kwargs)
...
def iter_tensors(args: Tuple[Any], kwargs: Dict[str, Any], allowed_nesting: int = 1) -> Iterator[Tensor]:
    def check(arg):
...
def _maybe_get_opdef(op: Union[CustomOpDef, _ops.OpOverload, str]) -> Optional[CustomOpDef]:
