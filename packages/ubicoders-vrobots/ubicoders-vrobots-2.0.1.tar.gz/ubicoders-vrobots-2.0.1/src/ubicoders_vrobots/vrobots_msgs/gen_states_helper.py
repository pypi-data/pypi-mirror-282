...
"""
    ...
    for name, field_type in fields:
        ...
        if field_type == "string":
            ...
            else_fields += (
                f"            self.{field_name} = [Collision(c) for c in instance.{field_name}]\n"
            )
            dict_data_fields += (
                f'            "{name}": [c.get_dict_data() for c in self.{field_name}],\n'
            )
        elif field_type.startswith("["):
            init_fields += f"            self.{field_name} = []\n"
            else_fields += f"            self.{field_name} = instance.{field_name}\n"
            dict_data_fields += f'            "{name}": self.{field_name} if self.{field_name} is not None else [],\n'
