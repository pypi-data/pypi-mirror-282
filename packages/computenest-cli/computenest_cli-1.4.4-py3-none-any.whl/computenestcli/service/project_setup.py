    ...
    def _render_templates(self):
        ...
        if artifact_source_type == ArtifactSourceType.DOCKER_COMPOSE.value:
            ...
            if pre_start_command:
                updated_pre_start_command = re.sub(r'\$\{([^}]+)}', r'${!\1}', pre_start_command)
                self.parameters[project_setup_constant.PRE_START_COMMAND_KEY] = updated_pre_start_command
            post_start_command = self.parameters.get(project_setup_constant.POST_START_COMMAND_KEY)
            if post_start_command:
                updated_post_start_command = re.sub(r'\$\{([^}]+)}', r'${!\1}', post_start_command)
                self.parameters[project_setup_constant.POST_START_COMMAND_KEY] = updated_post_start_command
