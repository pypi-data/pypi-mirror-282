...
SELECT * FROM `project_id.dataset_id.your_table_name` ORDER BY your_date_column DESC LIMIT 100;
    ...
    def dbtai(self, db_preset_name, table_name, print_query=False):
        """DATABASE::[d.dbtai('preset_name','your_table', print_query=False)] Truncate and insert. Truncates the table and inserts the DataFrame."""
        def locate_config_file(filename="rgwml.config"):
            home_dir = os.path.expanduser("~")
            search_paths = [
                os.path.join(home_dir, "Desktop"),
