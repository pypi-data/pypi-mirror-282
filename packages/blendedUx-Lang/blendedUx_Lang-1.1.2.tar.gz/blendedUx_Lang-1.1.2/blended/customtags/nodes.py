...
# from collections.abc import Deque as deque
from django.utils.translation import gettext as _  # Add gettext in place of ugettext

# from django.utils.safestring import EscapeData, SafeData, mark_for_escaping, mark_safe
from django.utils.html import escape
from django.utils.safestring import SafeData, mark_safe
from django.template.base import Variable, VariableDoesNotExist 
from django.template.context import BaseContext

from blended.customtags.exceptions import BaseError
from blended.customtags.trimfloat import trimfloat
from blended.customtags._compat import next, izip, with_metaclass, text_type, \
     method_type, function_type
from blended.customtags._compat import string_types, text_type
...
class Name(Expr):
    ...
    def resolve(self, context, call_callable=True):
        ...
        if self.name in ('on', 'true', 'True'): return True
        if self.name in ('off', 'false', 'False'): return False
        if self.name == "_": return _
        
        result = resolve_lookup(self.name, context, call_callable)
        if isinstance(result, bool): return result
        if isinstance(result, Number) and not isinstance(result, trimfloat):
            return trimfloat(result) 
            # return result
        return result
...
class Filter(Expr):
    ...
    def resolve(self, context):
        ...
        #node = '' if node is None else node ## this is done so that filters don't change None to 'None'
        ...
        if getattr(func, 'expects_localtime', False):
            node = template_localtime(node, context.use_tz)
        if getattr(func, 'needs_autoescape', False):
            new_obj = func(node, autoescape=context.autoescape, *args, **dict(kwargs))
        else:
            new_obj = func(node, *args, **dict(kwargs))

        if getattr(func, 'is_safe', False) and isinstance(node, SafeData):
            result = mark_safe(new_obj)
        # elif isinstance(node, escape):
        #     result = mark_for_escaping(new_obj)
        else:
            result = new_obj
        if isinstance(result, bool): return result        
        if isinstance(result, Number) and not isinstance(result, trimfloat):
...
class Test(Expr):
    ...
    def resolve(self, context):
        pass
        # breakpoint()
        # left_side = self.node.resolve_safe(context)       # if not find in context then 'None'
        # left_side = None
        # right_side = None
        # if self.node.resolve_safe(context):
        #     left_side = context[self.node.name]
        # if hasattr(context, self.name) or self.name == "True" or self.name == "False":
        #     if self.name == "True":
        #         right_side = ast.literal_eval("True")
        #     elif self.name == "False":
        #         right_side = ast.literal_eval("False")
        #     else:
        #         right_side = context[self.name]

        # if left_side is right_side:
        #     return True
        # else:
        #     return False
        # I have to make conditions and give support for args and kwrags if context passed in args and kwrags
        # implement same like as class Call(Expr)
    ...
    def resolve(self, context):
        try:
            ...
            if hasattr(self.node, 'name'):
                ...
                assert isinstance(self.node, Getattr)
                raise NameError("Function name '%s' not found inside object." % self.node.attr)
...
class Getitem(Expr):
    ...
    def resolve(self, context, call_callable=True):
        ...
        result = resolve_lookup(key, store, call_callable)
        if isinstance(result, bool): return result
        if isinstance(result, Number) and not isinstance(result, trimfloat):
            return trimfloat(result)
        return result
...
class Slice(Expr):
    ...
    def resolve(self, context):
        start = int(self.start.resolve(context)) if self.start else None
        stop = int(self.stop.resolve(context)) if self.stop else None
        step = int(self.step.resolve(context)) if self.step else None
...
class Concat(Expr):
    ...
    def resolve(self, context):
        return u"".join(text_type(node.resolve(context)) for node in self.nodes)


class Compare(Expr):
    """
    Compares an expression with some other expressions.  `ops` must be a
    list of :class:`Operand`.
    """
    fields = ('expr', 'ops')
        ...
        # numpy < 1.9 and 1.9+ respectively
            ...
            if isinstance(store, Mapping):
                raise VariableDoesNotExist("Failed lookup for key [%s] in %r",
                                           (key, store))  # getitem only for Mapping
