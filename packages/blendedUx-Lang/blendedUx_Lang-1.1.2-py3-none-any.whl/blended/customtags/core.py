...
from blended.customtags.arguments import NodeList, BlockTag, TagName, Optional
from blended.customtags.parser import structure_arguments
from blended.customtags.utils import get_default_name, Container
from blended.customtags.lexer import Lexer
from blended.customtags._compat import string_types, with_metaclass, iteritems
...
class Options(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.lexer = Lexer()
        self.error_msg_contents = ""               # added error message initializers
        blocks = []
    ...
    def parse(self, parser, tokens, container):
        ...
        if self.tagname == 'for':
            temp = tokens.split_contents()
            if "'" in temp[1] or '"' in temp[1]:
                raise BreakpointExpected(self.tagname,breakpoints=[], got=self, msg=tokens.contents)
        
        stream = self.lexer.tokenize(tokens.contents)
        self.original_string = tokens.contents
        self.parser.parse(parser, stream, container, tokens.contents)
...
class Tag(with_metaclass(TagMeta,Node)):
    """
    Tag class.
    """
    options = Options()
    ...
    def as_tag(cls, **initkwargs):
        ...
        def tag(parser, tokens):
            ...
            self.options.error_msg_contents = tokens.contents
            return self
    ...
    def render(self, context):
        ...
        # kwargs1 = {k: v.resolve(context) for k, v in self.container.tag_kwargs.items()}
        
        # for k,v in self.container.tag_kwargs.items():
        #     a = k
        #     b = v.resolve(context)
        #     print(a,b)
