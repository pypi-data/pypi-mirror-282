...
from blended.customtags import nodes
from blended.customtags.tokens import describe_token, describe_token_expr
from blended.customtags._compat import next, imap
...
class ExprParser(object):
    ...
    def parse_compare(self):
        ...
        while 1:
            ...
            if token_type == 'assign':
                ...
                raise TemplateSyntaxError()            # raised error if we used "=" for comparison instead of "=="
            ...
            # Add the below elif to support "is" and "is not" operator
            elif self.stream.skip_if('name:is'):
                if self.stream.current.test('name:not'):
                    self.stream.skip(1)
                    ops.append(nodes.Operand('is_not', self.parse_add()))
                else:
                    ops.append(nodes.Operand('is', self.parse_add()))
            else:
                break
        if not ops:
            return expr
    ...
    def parse_filter_expr(self, node):
        ...
        while 1:
            ...
            # comment the below elif because previously "is" used as test now implement as operator
    ...
    def parse_test(self, node):
        ...
        if self.stream.current.test('name:not'):
            ...
            negated = False
        name = self.stream.expect('name').value   # name changed for "self.stream.current.type"
