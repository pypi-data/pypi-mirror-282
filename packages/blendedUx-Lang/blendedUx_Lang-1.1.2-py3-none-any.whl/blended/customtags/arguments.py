...
from blended.customtags.values import *
from blended.customtags.utils import FakeParser, NULL, Container
from blended.customtags.lexer import Lexer
from blended.customtags.expr_parser import ExprParser
from blended.customtags._compat import string_types
# from blended.customtags.core import Options
...
class Constant(BaseArgument):
    ...
    def clean_token(self, parser, stream):
        content_str = BaseArgument.content_string              # added contents
        if stream.eos:
            raise TooFewArguments(str(self), self.tagname)
        if self.value == stream.current.value:
            return stream.current.value
        
        elif (self.tagname == 'for') and (len(content_str.split()) < 4):                # for error raising --- "for x items"
            raise TooFewArgumentsFor(str(self), self.tagname, msg=content_str)
        elif (self.tagname == 'for') and (stream.current.value == 'from'):
            raise FormatError(self.tagname, content_str)
        else:
            if stream.current.test('name:with') and stream.look().type == 'string': # raise error if with next token type is string
...
class Argument(BaseArgument):
    ...
    def __init__(self, name=None, default=NULL, required=True, 
    ...
    def clean_token(self, parser, stream):
        ...
        self.line_no = stream.current.lineno
        if stream.eos:                         #  modified messages wrt its tag
            error_msg = ""
            if (self.tagname == 'include'):
                error_msg = "'include' tag takes at least one argument: the name of the template to be included."
            elif (self.tagname == 'extends'):
                error_msg = "'extends' takes one argument"
            elif (self.tagname == 'autoescape'):
                error_msg = "'autoescape' tag requires exactly one argument."
            elif len(error_msg.split()) < 1:
                ArgumentRequiredError(self, self.tagname, msg = error_msg)
            raise ArgumentRequiredErrorGeneral(self, self.tagname, msg = error_msg)
...
class KeywordArgument(Argument):
    ...
    def clean_token(self, parser, stream):
        ...
        if stream.eos:
            raise ArgumentRequiredError(self, self.tagname, msg = BaseArgument.content_string)
...
class MultiArgument(BaseArgument):
    ...
    def _do_parse(self, parser, stream, container, nextargs=None):
        ...
        #     current_arg = arguments.popleft()
        #     # if len(arguments) == 0:
        #     #     nextargs_arguments = nextargs
        #     # else:
        #     #     nextargs_arguments = list(arguments)
    ...
    def parse(self, parser, stream, container, nextargs=None):
        ...
        while arguments:
            ...
            if hasattr(parser, 'tokens'):
                parser_copy = DjangoParser(list(reversed(parser.tokens)))
...
class BlockTag(MultiArgument):
    ...
    # add method to access child error messages
    ...
    def parse(self, parser, stream, container,original_contents, nextargs=None):
        # self.err_msg = original_contents
        BaseArgument.content_string = original_contents
        # if "for key value" in original_contents:
        #     breakpoint()
        if stream.eos:
            block_found = False
...
class Optional(MultiArgument):
    ...
    def parse(self, parser, stream, container, nextargs=None):
        ...
        try:
            ...
            self._do_parse(parser, stream, container, nextargs)   # pars
            return
                ...
                if hasattr(option, 'get_default'):
                    ...
                    if default is not NULL:
                        if option.name is None:
                            container.tag_args.append(default)
                        else:
                            container.tag_kwargs[str(option.name)] = default 
...
class EndTag(BaseArgument):
    ...
    def parse(self, parser, tokens, container, nextargs=None):
        ...
        tokens = deque(parser_token.split_contents())
        # print(tokens,self.tagname)
        # if tokens.popleft() != self.tagname:
        #     tokens.appendleft(self.tagname)
        # tokens.popleft()
        if tokens.popleft() != self.tagname:                    # endif/endifblock/endfor     error came from this line

            messages = " ".join(parser_token.split_contents())
            sp_tokens = parser_token.split_contents()
            if sp_tokens[0] == 'else' and sp_tokens[1] == 'if':     # raise error "else if" used instead of "elif"
                # breakpoint()
                raise TemplateSyntaxError(
                    'Malformed template tag at line {}: "{}"'.format(
                        Argument.line_no, messages
                    )
                )
            raise ArgumentRequiredError(self, self.tagname, msg = BaseArgument.content_string)
