...
try:
    dbstring = ('DRIVER={SQL Server};SERVER=SIN2T\\SQLEXPRESS;DATABASE=DemoKino;Trusted_Connection=yes;')
except Exception:
...
class MainWindow:
    ...
    def create_widgets(self):
        ...
        if self.role_id == 3:  # Admin
            ...
            # Добавляем новую вкладку "Сортировка"
            ...
            # Добавляем меню сортировки для пользователей и менеджеров
            self.sort_menu = tk.Menu(self.menu, tearoff=0)
            self.menu.add_cascade(label="Сортировка", menu=self.sort_menu)
    ...
    def sort_orders_by_date(self, order):
        ...
        if self.role_id == 1:  # User
            cursor.execute(
                f"SELECT id, title, description, first_name, last_name, phone_number, status, created_by_id, created_at, completed_by_id, completed_at "
    ...
    def sort_orders_by_date_desc(self):
        ...
        if self.role_id == 1:  # User
            cursor.execute(
                "SELECT id, title, description, first_name, last_name, phone_number, status, created_by_id, created_at, completed_by_id, completed_at "
                "FROM Orders WHERE created_by_id=? AND status='создан' ORDER BY created_at DESC", (self.user_id,))
        elif self.role_id == 2:  # Manager
            cursor.execute(
                "SELECT id, title, description, first_name, last_name, phone_number, status, created_by_id, created_at, completed_by_id, completed_at "
                "FROM Orders WHERE status NOT IN ('выполнен', 'отменен') ORDER BY created_at DESC")
        else:  # Admin
            cursor.execute(
                "SELECT id, title, description, first_name, last_name, phone_number, status, created_by_id, created_at, completed_by_id, completed_at "
    ...
    def initialize_order_tree(self):
        columns = ("id", "title", "description", "first_name", "last_name", "phone_number", "status", "created_by_id", "created_at", "completed_by_id", "completed_at")
        ...
        self.order_tree.heading("created_by_id", text="Создано ID", anchor='center')
        self.order_tree.heading("created_at", text="Дата создания", anchor='center')
        self.order_tree.heading("completed_by_id", text="Выполнено ID", anchor='center')
        ...
        self.order_tree.column("title", anchor='center')
        self.order_tree.column("description", anchor='center')
        self.order_tree.column("first_name", anchor='center')
        self.order_tree.column("last_name", anchor='center')
        self.order_tree.column("phone_number", anchor='center')
        self.order_tree.column("status", anchor='center')
        self.order_tree.column("created_by_id", width=100, anchor='center')
        self.order_tree.column("created_at", anchor='center')
        self.order_tree.column("completed_by_id", width=100, anchor='center')
    ...
    def complete_order(self):
        ...
        if selected_order:
            ...
            execute_query(
                "UPDATE Orders SET status='выполнен', completed_by_id=?, completed_at=GETDATE() WHERE id=?", 
                (self.user_id, order_id)
            )
            self.load_orders()  # Refresh the table after completing an order
            messagebox.showinfo("Успех", "Заказ успешно выполнен")
        else:
    ...
    def cancel_order(self):
        ...
        if selected_order:
            order_id = selected_order[0]
            execute_query(
                "UPDATE Orders SET status='отменен', completed_by_id=?, completed_at=GETDATE() WHERE id=?", 
                (self.user_id, order_id)
            )
            self.load_orders()  # Refresh the table after canceling an order
            messagebox.showinfo("Успех", "Заказ успешно отменен")
        else:
...
class CreateOrderWindow:
    ...
    def create_order(self, user_id):
        ...
        if user:
            ...
            execute_query(
                "INSERT INTO Orders (title, description, first_name, last_name, phone_number, status, created_by_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (title, description, first_name, last_name, phone_number, 'создан', user_id)
            )
