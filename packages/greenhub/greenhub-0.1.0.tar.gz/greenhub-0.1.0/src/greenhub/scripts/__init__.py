...
def print_cli_description():
    ...
    sections.append(
        ...
        "\nFor more information, please visit wiki.greenhub.ai."
    )
    sections.append(
        "Usage:"
        "\n  greenhub <command>"
...
def run_implementation_test():
    ...
    # expected signatures
    IN_SEASON_MODEL_EXPECTED_SIGNATURE = {
        'names': ['year', 'month'],
        'types': [int][int],
        'signature': 'run(year: int, month: int)'
    }
    END_OF_SEASON_MODEL_EXPECTED_SIGNATURE = {
        'names': ['year'],
        'types': [int],
        'signature': 'run(year: int)'
    }
    # check
    if param_names not in [IN_SEASON_MODEL_EXPECTED_SIGNATURE['names'], END_OF_SEASON_MODEL_EXPECTED_SIGNATURE['names']]:
        print(f"FAILURE: 'run' function implementation does not expect the right parameters. "
              f"The 'run' function signature should look like: "
              f" - for an 'in-season model': '{IN_SEASON_MODEL_EXPECTED_SIGNATURE['signature']}' "
              f" - or for an 'end-of-season model': '{END_OF_SEASON_MODEL_EXPECTED_SIGNATURE['signature']}' "
              f"For more information please checkout the official greenhub.ai documentation.")
        return
    if param_types not in [IN_SEASON_MODEL_EXPECTED_SIGNATURE['types'], END_OF_SEASON_MODEL_EXPECTED_SIGNATURE['types']]:
