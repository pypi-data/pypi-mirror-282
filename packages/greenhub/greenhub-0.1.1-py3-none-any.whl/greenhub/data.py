...
def _filter_for_valid_feature_data_paths(file_paths: [str]) -> [str]:
    """
    The greenhub REST API endpoint `filterForValidFeatureDataPaths` (Google Cloud Function) is queried with a list of
    `file_paths`. Before the actual download request is made, the function filters out all paths that do not exist in
    the Firebase Storage.

    :param file_paths: list of queried paths to feature data.
    :returns: list containing those feature data paths that actually exist in the Firebase storage.
    """

    # make the API request
    user_api_key = get_user_api_key()
    params = {
        'featureDataPaths': ",".join(file_paths),
        'apiKey': user_api_key
    }
    res = requests.get(GREENHUB_REST_API_URL + '/filterForValidFeatureDataPaths', params=params)

    # parse and return the requested list of valid feature data paths
    if res.status_code == 200:
        return res.json().get('validFeatureDataPaths', [])
    else:
        raise HTTPError(f'Filtering for valid feature data paths from API endpoint failed with status code: {res.status_code}')
...
def _fetch_data(file_path: str) -> Optional[pd.DataFrame]:
    ...
    res = requests.get(GREENHUB_REST_API_URL + '/getFeatureData', params=params)
...
def _load_data(feature_files_paths: [str], data_name: str) -> Optional[pd.DataFrame]:
    ...
    # get all valid feature data paths (those that exist in the Firebase storage)
    valid_feature_file_paths = _filter_for_valid_feature_data_paths(feature_files_paths)

    # fetch data and cache it, if already cached load it
    fdc = get_fdc()
    data = []
    for f in tqdm(valid_feature_file_paths, desc=f"Loading {data_name}", unit="items"):
...
def get_vi_data(
    ...
    if start_year < 2000 and end_year < 2000:
        raise ValueError(f'There is no VI data available before 2000. '
...
def get_climate_data(
    ...
    if start_year < 2000 and end_year < 2000:
        raise ValueError(f'There is no climate data available before 2000. '
                         f'For more information please checkout the official greenhub.ai documentation.')
    if start_year < 2000:
