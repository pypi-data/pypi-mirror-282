...
# for post in get_posts('mon111012', pages=1):
#     print(post['text'][:50])
...
class FaceApiWrapper:
        ...
        while cursor is not None:
            try:
                ...
                if session:
                    ...
                    break
                if response:
                    res = paser_node_posts_group(data=response.text)
                    cursor = res['cursor']
                    count_post += len(res['node_posts'])
                    yield res['node_posts']
                    if post_limit != -1:
                        if count_post >= post_limit:
                            print(f"Get sussces {count_post} post")
                            break
                    time.sleep(sleep_time)
                else:
                    break
            except Exception as e:
                print(f"Error func get_node_posts_group: {e}")
                break
    @staticmethod
        ...
        while cursor is not None:
            try:
                ...
                if session:
                    ...
                    break
                if response:
                    res = paser_node_posts_page(data=response.text)
                    cursor = res['cursor']
                    count_post += len(res['node_posts'])
                    yield res['node_posts']
                    if post_limit != -1:
                ...
                print(f"Error func get_node_posts_page: {e}")
                break
    ...
    def get_all_comment_post_permalink(postID: Union[str, int], session: Optional[Session] = None , **kwargs):
        try:
            ...
            print(f"Error func get_all_comment_post_permalink: {e}")
            return []
    @staticmethod
    ...
    def get_reply_depth1_comment(expansionToken: Optional[str], id_comment: Optional[str] = "", feedback_id: Optional[str] = "", session: Optional[Session] = None, **kwargs):
        try:
            ...
            print(f"Error func get_reply_depth1_comment: {e}")
            return []
    @staticmethod
