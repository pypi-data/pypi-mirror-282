...
class BC(VeryBasicControl):
    ...
    def sym_f(self, x, u, t=None):
        return [2*sym.sin(0 + x[0] + u[0] )  ]

def line(f, xs, us, xb, ub):
    sub = dict(zip(xs + us, xb + ub))
    return sym.Matrix(f).jacobian(xs).subs(sub) @ (sym.Matrix(xs) - sym.Matrix(xb)) + sym.Matrix(f).jacobian(us).subs(sub) @ (sym.Matrix(us) - sym.Matrix(ub)) + sym.Matrix(f).subs(sub)


class Discretization03(Question):
    ...
    def generate(self):
        ...
        n = bmodel.n
        xs = sym.symbols(f'x0:{n}') #symv('x',         bmodel.n)
        us = (sym.symbols('u'),) #symv('u',         bmodel.d)

        f = bmodel.sym_f(xs, us)
        x.f_tex = sym.latex(f[0].subs({xs[0]: sym.Symbol('x')}))
        ...
        f_euler =   [ self.fixf(dt * f_ + x) for f_, x in zip(f, xs)]

        f_euler_k = [f_.subs( zip(xs+us, xks + uks) ) for f_ in f_euler]

        x.sol.f_euler_k = f_euler_k[0]
        # sym.latex(f_euler_k)
