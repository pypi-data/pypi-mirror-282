...
# TODO: check
def specificity_2(y2, y, threshold=0.5): # TNR = TN/N
    y2 = (y2 > threshold).byte()
    y = y.byte()
    y2 = torch.max(y2, dim=1)[1]
    tn = torch.mul((y2==y), y.byte()^1).sum().item()
...
def sensitivity_2(y2, y, threshold=0.5):
    return recall2(y2, y, threshold=threshold)
...
def mae(y2, y):
    """Mean Absolute Error"""
    return 100*torch.mean(torch.abs(y-y2))

def mae_inv(y2, y):
    """Mean Absolute Error"""
    return 1-torch.mean(torch.abs(y-y2))
