...
def get_preds(model, x, bs=1024):
    ...
    with torch.no_grad():
        for i in range(0, len(x), bs):
            ...
            xb = xb.cuda()
            y2b = model(xb)
            y2_list.append(y2b.detach().cpu().numpy())
    return np.concatenate(y2_list, axis=0)
...
def save_model(model, p, stats=None):
    p = Path(p)
    p = p.with_suffix('.pth.tar')
    state = {'state_dict':model.state_dict(), 'stats':stats}
    torch.save(state, p)

def load_model(model, p, cuda=False, inference=False, show=False):
    p = Path(p).with_suffix('').with_suffix('.pth.tar')
