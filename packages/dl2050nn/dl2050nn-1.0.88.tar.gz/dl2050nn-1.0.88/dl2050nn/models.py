...
class SignalNet(nn.Module):
    def __init__(self, ch, n, c, p=0., show=False):
        """
        SignalNet: fully CNN model with Head.
        Head is either a Classification head with c classes if c is not zero, or a Regression head otherwise.
        """
        super().__init__()
        n1,sz = ch,n
        n2 = min((n1+1)*8,64)
        ...
        head = [Linear(n1, 1024, p=p),LinearFinal(1024, c, p=None)] if c>0 else\
               [Linear(n1, 1024, p=p), Linear(1024, 1, bn=False, act=None, p=None), nn.Sigmoid()]
