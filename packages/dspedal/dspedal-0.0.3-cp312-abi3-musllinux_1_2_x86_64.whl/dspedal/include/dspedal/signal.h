...
namespace dspedal
{
    ...
    template <typename T>
    void connect(Signal<T> &s, T &port, const std::string &direction)
    {
        s.bind(&port, direction);
    }

    template <typename T, size_t N>
    void connect(std::array<Signal<T> *, N> &arr, T (&ports)[N], const std::string &direction)
    {
        for (size_t i = 0; i < N; i++)
        {
            arr[i]->bind(&ports[i], direction);
        }
    }
} // namespace dspedal
