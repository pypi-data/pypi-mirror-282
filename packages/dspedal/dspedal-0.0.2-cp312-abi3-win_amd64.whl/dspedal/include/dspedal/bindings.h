...
namespace dspedal
{
    ...
    static inline auto bind_tracer(nanobind::handle &scope, const char *name)
    {
        return nanobind::class_<Tracer<TraceType>>(scope, name)
            .def(nanobind::init<const std::string &, int, int>(), nanobind::arg("trace_name"), nanobind::arg("levels"), nanobind::arg("options") = 0)
            .def("close", &Tracer<TraceType>::close);
    }
    ...
    {
        return nanobind::class_<M>(scope, name)
    ...
    static inline auto bind_signal(nanobind::handle &scope, const char *name)
    {
        return nanobind::class_<Signal<T>>(scope, name)
            ...
            .def_prop_rw(
                ...
                { return s->_read_d(); },
                [](Signal<T> *s, int val)
                { s->writei(val); }, nanobind::arg("value"))
