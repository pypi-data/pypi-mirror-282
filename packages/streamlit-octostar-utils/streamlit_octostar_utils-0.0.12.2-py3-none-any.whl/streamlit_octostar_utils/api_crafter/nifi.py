...
class NifiContextManager(object):
    ...
    def receive_input(self, json_data, processor_name, producer_name=None) -> List["NifiEntity"]:
        ...
        entities = sorted(entities, key=lambda x: string_to_unix(x.record['os_last_updated_at']))
    ...
    def send_output(self, entity_batches, processor_name, producer_name):
        ...
        for entity in entities:
            if entity.sync_params.get(NifiContextManager.SyncFlag.WRITE_ENTITY_ANNOTATIONS):
                ...
                for producer_name, params in annotations_per_producer:
                    ...
                    producer_annotations = sorted(producer_annotations, key=lambda x: string_to_unix(x.record['os_last_updated_at']))
    ...
    f" FROM `dtimbr`.`{entity_type}` WHERE `os_entity_uid` IN ({ids})")
            ...
            rels = query_ontology.sync(sql_query)
            rels = sorted(rels, key=lambda x: x['os_entity_uid_from'])
            for os_entity_uid_from, group in itertools.groupby(rels, key=lambda x: x['os_entity_uid_from']):
                if os_entity_uid_from not in entities_by_id:
                    continue
                for rel in group:
                    if rel['os_entity_uid']:
                        rel_request = {
                            'jwt': entities_by_id[os_entity_uid_from].request['jwt'],
                            'ontology_name': entities_by_id[os_entity_uid_from].request['ontology_name'],
                            'ontology_info': {
                            ...
                            'sync_params': {},
                            'nifi_attributes': {},
                            'config': deepcopy(entities_by_id[os_entity_uid_from].request['config']),
                            'fallback_os_workspace': entities_by_id[os_entity_uid_from].request['fallback_os_workspace'],
                            'is_temporary': False
                        }
                        rel = {**rel, 'entity_id': rel['os_entity_uid'], 'os_concept': rel['entity_type']}
                        entities_by_id[os_entity_uid_from].children.append(
                            NifiEntityProxy(
                                entities_by_id[os_entity_uid_from].context,
                                rel['os_entity_uid'],
                                rel['os_entity_type'],
...
class NifiEntity(object):
    def __init__(self, context, request, record, all_independent_uids, children=[], contents=None):
        ...
        assert(self.record.get('os_last_updated_at'))
        assert(self.record.get('os_last_updated_by'))
        assert(self.record.get('os_last_created_at'))
        assert(self.record.get('os_last_created_by'))
    ...
    def children_entities(self):
        return list(filter(lambda x: isinstance(x, NifiEntityProxy)))
    @property
    def relationships(self):
        return list(filter(lambda x: isinstance(x, NifiOTMRelationshipProxy) or x.is_child_concept(RELATIONSHIP_ENTITY_NAME), self.children))
    '''
    @property
    ...
    def to_json(self):
        ...
        proxy_otm_children = list({c.record['os_entity_uid_from'] + "|" + c.record['os_relationship_name'] + "|" + c.record['os_entity_uid_to']: c for c in proxy_entity_children}.values())
        proxy_otm_children = [c.record for c in proxy_otm_children]
        full_entity_children = sorted(full_entity_children, key=lambda x: string_to_unix(x.record['os_last_updated_at']))
    ...
    def get_annotation(self, producer_names=None):
        ...
        annotations = sorted(annotations, lambda x: string_to_unix(x.record['os_last_updated_at']))
