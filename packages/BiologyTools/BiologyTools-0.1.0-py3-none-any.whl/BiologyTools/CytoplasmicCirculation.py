...
class Measure:
    def __init__(
            self, path: str, cache: str, interval: int = 1, model: str = os.path.join(base_path, 'model/不错.pt'),
    ...
    def chloroplast(self):
        ...
        for i in self.stream:  #
            for id_, post in i.items():
                try:
                    chloroplasts[id_].append(post[:2])
                except KeyError:
                    chloroplasts[id_] = [post[:2]]
        for k, v in chloroplasts.items():
            all_ += len(v)
            chloroplasts[k] = numpy.array(v, dtype=numpy.float64)
        return chloroplasts


class Exec:
    @classmethod
    def exec(cls, operations):
        if '' in operations:
            with Measure() as posts:
                pass
