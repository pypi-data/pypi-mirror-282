...
class api_rest: # Classe para interação com APIs Rest (especialmente Sienge)
    ...
    def endpoint_json(self, endpoint, payload, metodo): # Interage com End Point
        ...
        try:            
            if self.Metodo == 'POST':
                self.RetEndPoint = self.Sessao.post(self.EndPoint, json=self.Payload)  
            elif self.Metodo == 'GET':
                self.RetEndPoint = self.Sessao.get(self.EndPoint, json=self.Payload)  
            elif self.Metodo == 'PATCH':
                self.RetEndPoint = self.Sessao.patch(self.EndPoint, json=self.Payload)  
            elif self.Metodo == 'PUT':
                self.RetEndPoint = self.Sessao.put(self.EndPoint, json=self.Payload)  
            else:
                print(f'Método não previsto: {metodo}. Use [post|get|patch|put]')
                os._exit(1)
