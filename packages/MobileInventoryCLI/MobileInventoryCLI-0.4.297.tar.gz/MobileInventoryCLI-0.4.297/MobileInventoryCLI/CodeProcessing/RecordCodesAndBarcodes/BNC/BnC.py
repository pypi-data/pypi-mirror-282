...
{self.billTotal()}
{'.'*20}
{Fore.light_green}P -> $ Paid
{Fore.light_yellow}U -> $ Unpaid
{Fore.cyan}TTL -> Total Bill
{'.'*20}{Style.reset}
				'''
				fieldname='Menu'
				mode='BnC'
	...
	def billTotal(self):
		with Session(ENGINE) as session:
			...
			for i in query.all():
				total+=(i.Value*i.Qty)
			msg=f'{Fore.light_steel_blue}Bill={Fore.light_green}P:${paid_total}|{Fore.light_yellow}U:${unpaid_total}/{Fore.cyan}TTL:${total}{Style.reset} '
			return msg
