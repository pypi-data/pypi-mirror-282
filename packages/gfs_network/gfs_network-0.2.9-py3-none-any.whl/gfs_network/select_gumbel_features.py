...
def select_gumbel_features(X_train, y_train, device="cpu", verbose=False, temperature_decay=0.997, epochs=300, batch_size=1, fs_balance=1.0):
    ...
    for epoch in tqdm.trange(epochs, disable=not verbose):
        ...
        for data in trainloader:
            ...
            loss = criterion(output, y_.argmax(axis=1)) + selected_no * fs_balance
            loss.backward()
            optimizer.step()
