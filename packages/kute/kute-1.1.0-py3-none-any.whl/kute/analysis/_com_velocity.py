...
class COMVelocity(AnalysisBase):
    ...
    """
    def __init__(self, universe: Universe, filename:str="com_velocity.h5"):
    ...
    def _prepare(self):
        ...
        self._com_vel = np.zeros((self._total_steps, self.u.residues.n_residues, 3))

    def _single_frame(self):

        self._com_vel[self._frame_index, :, :] = self._matrix @ self.u.atoms.velocities
    ...
    def write_h5_file(self):
        ...
        ## Metadata group
            ...
            file['timeseries'].create_dataset('time', data=self.times)
            file['timeseries'].create_dataset('com_velocities', data=self._com_vel)