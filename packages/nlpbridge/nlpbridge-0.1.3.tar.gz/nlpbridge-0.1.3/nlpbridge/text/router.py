...
class Node():
    ...
    def __call__(self, *args, **kwargs) -> Output:
        ...
        print("current agent generate:", response)
...
class Condition(Callable[[dict], TypedDict]):
    ...
    @classmethod
    def init_with_params(cls, params: dict):
        return cls(cur_node=params["cur_node"], target_nodes=params["target_nodes"])
...
class RouterManager:
    ...
    def get_graph(self, chat_id: str, llm, router_id: int, condition_cls: Condition = Condition) -> Tuple[CompiledGraph, dict]:
        ...
        runnable_nodes = self.convert_to_runnable_node(nodes, edges, llm, chat_id)
    ...
    def _create_conditional_edges(self, graph: StateGraph, nodes: List[Node], edges: List[Edge],
        ...
        for edge in edges:
            ...
            params_dict = {"cur_node": cur_node, "target_nodes": target_nodes}

            graph.add_conditional_edges(
                cur_node_name,
                condition_cls.init_with_params(params_dict),
                path_map
            )
        return graph
    ...
    @staticmethod
    def convert_to_runnable_node(nodes: List[NodeModel], edges: List[Edge], llm, chat_id: str) -> List[Node]:
        runnable_nodes = []
        for node in nodes:
