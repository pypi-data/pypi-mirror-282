...
from ..inferences.base import ASRInference
...
def get_grpc_result(
    ...
    return_spent_time: bool = False,
    print_info: bool = True,
) -> Union[Dict, Tuple[Dict, float]]:
    """调用wpai test环境服务"""
    ## 传递参数，例如此处容器中通过kwargs["meta"]["tags"]获取到dict类型的参数{"length":6}
    if print_info:
        print("audio url:", url)
    start = time.perf_counter()
    ## 构建请求对象
    request = construct_client_request(url)
    ## 请求的host地址
    host = f"test-lbg-{business}.wpai.58dns.org:8866"
    ## 设置请求的任务taskid
    ...
    if print_info:
        print("spent time:", spent)
        print("response:", result)
    if not return_spent_time:
        return result
    return result, spent


def test_grpc_concurrency_v1(
    audios: List[str], task_id: str, bussiness: str = "huangye", num_workers: int = 4
):
    ## 创建线程池
    pool = threadpool.ThreadPool(num_workers=num_workers)
    ## 创建任务列表
    requests = threadpool.makeRequests(
        get_grpc_result, [([audio, task_id, bussiness], None) for audio in audios]
    )
    ## 将任务添加到线程池中
...
def test_grpc_concurrency(
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for i in range(num_workers):
            if audios:
                first_audios.append(audios.pop())
        futures = {
            executor.submit(get_grpc_result, audio, task_id, bussiness)
            ...
            for future in done:
                ...
                if audios:
                    future = executor.submit(
                        get_grpc_result, audios.pop(), task_id, bussiness
                    )
                    futures.add(future)


def test_concurrency(infer: ASRInference, input: str, n_workers: int = 5):
    ## 创建线程池
    pool = threadpool.ThreadPool(num_workers=n_workers)
    ## 创建任务列表
    requests = threadpool.makeRequests(infer.predict, [input] * n_workers)
    ## 将任务添加到线程池中
    [pool.putRequest(req) for req in requests]
    ## 等待所有任务完成
    pool.wait()
