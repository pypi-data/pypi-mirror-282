...
class Reaction:
    def __init__(self, input_ion, target_atom, output_ions, cross_section_object, cross_section_type='curve', name=None):
        ...
        self.name = name if name is not None else '|'.join((target_atom.name, input_ion.name, *[oi[0].name for oi in output_ions]))
...
class Spectrum:
    ...
    def save_csv(self, name, bins=400):
        """
...
def reset(range_tables=True, perf_timings=True):
    ...
    global INITIAL_SPECTRUMS_LIST, LAYERS_LIST, MATERIALS_LIST, PERF_TIMINGS, PROPAGATING_IONS_SPECIES_LIST, \
           RANGE_TABLES, REACTIONS_LIST, SLICES_LIST, SLICES_BACK_POSITIONS, SLICES_FRONT_POSITIONS
    if range_tables:
        RANGE_TABLES = {}
    if perf_timings:
        PERF_TIMINGS = {}
...
def set_energy_bins(number=None, distribution=None, resize_trigger=None):
    ...
    distribution : function, optional
        The energy bins distribution. The default is None.
