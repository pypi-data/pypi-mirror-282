...
class Session:
    ...
    async def set_content_status(self, url: AnyUrl | str, status: ContentStatus, storage_id: Optional[str] = None):
        ...
        state.status = status
        state.storage_id = storage_id if status == ContentStatus.DOWNLOADED else state.storage_id
    ...
    async def all_content_processed(self) -> bool:
        ...
        while True:
            ...
            for __url, raw_state in res[1].items():
                ...
                if content_state.status == ContentStatus.DOWNLOADED:
                    return False
            cursor = res[0]
            if cursor == 0:
                break
