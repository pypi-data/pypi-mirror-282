...
@pytest.mark.anyio
async def test_session_content(session):
    assert await session.has_content("CrawlerContent.url") is False
    await session.add_content("CrawlerContent.url")
    assert await session.has_content("CrawlerContent.url") is True
