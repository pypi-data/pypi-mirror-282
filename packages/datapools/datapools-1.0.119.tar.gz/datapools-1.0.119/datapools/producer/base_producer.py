...
class BaseProducer(Stoppable):
            ...
            while not await self.is_stopped():
                ...
                if message:
                    ...
                    try:
                        ...
                        await self._report_evaluation(session.id, task, EvaluationStatus.Failure)
    ...
    async def process_content(self, session: Session, raw_data, task: ProducerTask):
        ...
        if await session.is_alive():  # session may have been stopped/deleted while processing content
            await self._report_evaluation(session.id, task, EvaluationStatus.Success)

    async def _report_evaluation(self, session_id, task: ProducerTask, status: EvaluationStatus):
        report = SchedulerEvaluationReport(status=status)
        await self.reports_queue.push(QueueMessage(session_id, QueueMessageType.ReportEvaluation, report))
