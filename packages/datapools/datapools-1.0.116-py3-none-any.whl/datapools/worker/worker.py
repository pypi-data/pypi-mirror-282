...
from ..common.session_manager import SessionManager, Session, SessionStatus
...
class CrawlerWorker(Stoppable):
        ...
        try:
            ...
            if qm.type == QueueMessageType.Task:
                ...
                if message.redelivered is True:
                    ...
                    if url_state.status != CrawlerHintURLStatus.Unprocessed:
                ...
                if loop_result is not None or plugin is None or plugin == MAX_INSTANCES_ERROR:
                    ...
                    await session.set_url_worker(task.url, "")
                    return
                ...
                # await session.set_url_worker(task.url, None)
    ...
    async def _report_task_status(self, session: Session, task: WorkerTask, status: CrawlerHintURLStatus):
        ...
        task.status = status
        await self.reports_queue.push(QueueMessage(session.id, QueueMessageType.Report, task.to_dict()))
    ...
    async def _add_delayed_task(self, session_id, task: WorkerTask, message):
        ...
        await self.delayed_todo_queue_w.push(
            QueueMessage(session_id, QueueMessageType.DelayedTask, delayed_task.to_dict())
        )
        await self.todo_queue_r.mark_done(message)
