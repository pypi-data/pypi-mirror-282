...
class Plot:
    ...
    def make_tt_summary_plots(self, **kwargs):  
        ...
        offset = [-0.5,0.5]
        [axs[0].axvline(x=np.argmax([x==p for x in d.index])+offset[i],linestyle='--',color=[0.6,0.6,0.6],linewidth=2) for p,i in zip(test_period,offset)]
...
##### MAIN CLASS
class PyCausalImpact(Params,Match,Test,Plot):
    """
