...
class AzureBatch(BackendBase):
    ...
    def __init__(self, compute, compute_def, core, config, username=None, arg_dict=None):
        ...
        if expire_days > 30:
            errors.user_error(("'general.storage-cert-days' in config file must be <= 30"))
...
class BatchLogReader():
    ...
    def get_task(self):
        ...
        # should be done only once (for certificate lifetime reasons)
    ...
    def read(self):
        ...
        if self.log_name is None:
            self.log_name = "stdout.txt"
        file_path = os.path.basename(self.log_name)
        ...
        if not found_file:
            ...
            if self.task is None:
                ...
                self.task = self.batch_backend.get_node_task(self.service_node_info)
            ...
            try:
                ...
                stream = self.batch_client.file.get_from_task(self.batch_job_id, self.task.id, file_path=live_log_name, 
        ...
        return {"new_text": new_text, "simple_status": simple_status, "log_name": self.log_name, 
            "service_status": batch_status, "file_path": file_path,  "found_file": found_file, 
