...
class BranchSelectNode extends ControlNode {
  constructor(editor, name) {
    ...
    super(editor, name, 'Branch Select', inputs, [commonOutputs.dep]);

    this.branchStartIndex = inputs.length;
  ...
  async updateBranches() {
    ...
    if (numBranches > this.numPrevBranches) {
      ...
      for (let i = this.numPrevBranches; i > numBranches; i--) {
        ...
        const connection = outgoingConnections.find((c) => c.sourceOutput === key);
