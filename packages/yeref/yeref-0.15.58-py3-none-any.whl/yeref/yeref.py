...
async def create_invoice_link(BOT_TID, BOT_LC, msg_text, msg_btns, POST_LNK, BASE_D):
    ...
    try:
        ...
        print(f"{POST_LNK=}")
        if POST_LNK:
            photo_url = ast.literal_eval(POST_LNK)[0] if '[' in POST_LNK else POST_LNK
            if not str(photo_url).endswith(('.png', '.jpg', '.jpeg')):
                photo_url = None
        else:
            photo_url = None
    ...
    try:
        ...
        for btn in msg_btns:
            try:
                ...
                if btn['knd'] == 'payment':
                    invoice_link = await create_invoice_link(BOT_TID, BOT_LC, msg_text, msg_btns, POSY_LNK, BASE_D)
