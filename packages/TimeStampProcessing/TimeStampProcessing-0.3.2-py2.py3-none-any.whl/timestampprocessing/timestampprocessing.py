...
def compare_ts_column_sums(df, start_time, end_time):
    """
    Compare the sums of columns within a specified time range in a pandas DataFrame and return the column indices for max and min sums.
    
    Args:
    df (pd.DataFrame): Input DataFrame with a datetime index and numeric columns.
    start_time (str or pd.Timestamp): Start of the time range.
    end_time (str or pd.Timestamp): End of the time range.
    
    Returns:
    tuple: Indices of the columns with the maximum and minimum sums within the specified time range.
    """
    # Ensure time inputs are in pd.Timestamp format
    if isinstance(start_time, str):
        start_time = pd.to_datetime(start_time)
    if isinstance(end_time, str):
        end_time = pd.to_datetime(end_time)
    
    # Slice the DataFrame to only include data within the specified time range
    df_filtered = df.loc[start_time:end_time]
    
    # Compute the sum of each column while skipping NaN values
    col_sums = df_filtered.sum(skipna=True)
    
    # Find the indices of the max and min sums
    max_index = col_sums.idxmax()
    min_index = col_sums.idxmin()
    
    # Order the indices to ensure the smaller index is returned first
...
def find_time_ranges_with_condition(df, col_indices=[1, 2], operator='>'):
    """
    Find time ranges where a specified condition between two columns holds in a pandas DataFrame.
    
    Args:
    df (pd.DataFrame): Input DataFrame with a datetime index and numeric columns.
    col_indices (list): List of two integers denoting the column indices to compare; defaults to [1, 2].
    operator (str): Comparison operator as a string ('>', '>=', '<', '<=', '==', '!='); defaults to '>'.
    
    Returns:
    list of tuples: Each tuple contains the start and end times of ranges where the specified condition holds.
    """
    # Get the column names from indices
    col1, col2 = df.columns[col_indices[0]], df.columns[col_indices[1]]
    
    # Compare columns based on the specified operator
    if operator == '>':
        ...
        raise ValueError("Unsupported operator. Please choose from '>', '>=', '<', '<=', '==', '!='.")

    # Identify changes in the mask
    change_points = mask.astype(int).diff().fillna(0) != 0
    
    # Identify start and end times
    starts = df.index[change_points & mask].tolist()
    ends = df.index[change_points & ~mask].tolist()
    
    # Ensure closure of intervals
    if mask.iloc[0]:
