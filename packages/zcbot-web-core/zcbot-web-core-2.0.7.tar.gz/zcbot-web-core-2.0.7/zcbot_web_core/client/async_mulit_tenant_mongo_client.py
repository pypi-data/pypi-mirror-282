...
class AsyncMultiTenantMongoHolder(object):
    ...
    def get_tenant_mongo(tenant_code: str) -> Optional[AsyncMultiTenantMongo]:
        ...
        # 无实例则加锁创建
        AsyncMultiTenantMongoHolder._instance_async_lock.acquire()
        try:
            # 双重锁校验
            ...
            if AsyncMultiTenantMongoHolder._instance_async_lock.locked():
                AsyncMultiTenantMongoHolder._instance_async_lock.release()
            return AsyncMultiTenantMongoHolder._tenant_instance_dict.get(tenant_code)
        finally:
            try:
                if AsyncMultiTenantMongoHolder._instance_async_lock.locked():
