...
def genDate():
    ...
    end_date = start_date.replace(day=18) 
...
words = read_words(d + "/opagen/words.txt")
@app.command()
def main(accounts: Annotated[int, typer.Argument()] = "1"):
        for x in track(range(accounts),description="Processing..."):
            ...
            if os.path.isfile('opa_accs.csv') == False : 
                with open('opa_accs.csv', 'w', newline='') as file:
                    writer = csv.writer(file)
                    field = ["username", "password","birth month"]
                    writer.writerow(field)
                    writer.writerow([emailGen, passwordGen,birthdayGen.strftime('%B')])
            else:
                with open('opa_accs.csv', 'a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([emailGen, passwordGen,birthdayGen.strftime('%B')])
            time.sleep(5)
        else:
            console = Console()
            text = Text.assemble("Done! your account info is stored in ", ("opa_accs.csv", "bold magenta"),  "! Hope you enjoy free gyros!")
            console.print(text)
