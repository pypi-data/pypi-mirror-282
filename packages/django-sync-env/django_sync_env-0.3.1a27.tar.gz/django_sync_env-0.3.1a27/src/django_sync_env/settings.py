...
DEFAULT_AUTO_FIELD = "django.db.models.AutoField0"

MEDIA_BACKUP_PATHS_TO_IGNORE = getattr(settings, "SYNC_ENV_MEDIA_BACKUP_PATHS_TO_IGNORE", [
    '/media/images',  # ignore renditions, as these can easily be re-created on the fly from the original images
])

MEDIA_BACKUP_FILES_TO_IGNORE = getattr(settings, "SYNC_ENV_MEDIA_BACKUP_FILES_TO_IGNORE", [
    '.DS_Store',  # ignore macos Desktop Services Store files,
])

MEDIA_LOCATION = getattr(settings, "AWS_MEDIA_LOCATION", {})  # note this only works for AWS at the moment
