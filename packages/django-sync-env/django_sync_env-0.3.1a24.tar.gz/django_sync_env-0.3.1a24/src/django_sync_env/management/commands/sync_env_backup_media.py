...
class Command(BaseSyncBackupCommand):
    ...
    def _explore_storage(self, media_storage, paths_to_ignore=[]):
        ...
        paths_to_ignore = set(os.path.normpath(p) for p in paths_to_ignore)
        self.logger.info(f"paths_to_ignore: {paths_to_ignore}")
        path = ""
        dirs = [path]
        while dirs:
            path = dirs.pop()
            try:
                ...
                self.logger.info(f"Looking at subdirs: {subdirs}; files: {files}; Path: {path}")
                for media_filename in files:
                    file_path = os.path.join(path, media_filename)
                    if not any(os.path.normpath(file_path).startswith(ignored) for ignored in paths_to_ignore):
                        yield file_path
                    else:
                        self.logger.info(f"Skipping: {file_path} due to match in SYNC_ENV_MEDIA_BACKUP_PATHS_TO_IGNORE")

                for subdir in subdirs:
                    dir_path = os.path.join(path, subdir)
                    if not any(os.path.normpath(dir_path).startswith(ignored) for ignored in paths_to_ignore):
                        dirs.append(dir_path)
                    else:
                        self.logger.info(f"Skipping: {dir_path} due to match in SYNC_ENV_MEDIA_BACKUP_PATHS_TO_IGNORE")
