...
class Command(BaseSyncBackupCommand):
    ...
    def handle(self, **options):
        ...
        try:
            ...
            paths_to_ignore = sync_env_settings.MEDIA_BACKUP_PATHS_TO_IGNORE

            if storage:
                self.backup_mediafiles(media_storage, storage, paths_to_ignore)
    ...
    def _explore_storage(self, media_storage, paths_to_ignore=[]):
        ...
        while dirs:
            ...
            for media_filename in files:
                if media_filename not in sync_env_settings.MEDIA_BACKUP_FILES_TO_IGNORE:
                    yield os.path.join(path, media_filename)
                else:
                    self.logger.info(f'Skipping: {media_filename} due to match in SYNC_ENV_MEDIA_BACKUP_FILES_TO_IGNORE')

            for subdir in subdirs:
                full_path = os.path.join(sync_env_settings.MEDIA_LOCATION, path, subdir)
                # Prefix with a '/' to match the format of MEDIA_BACKUP_PATHS_TO_IGNORE
                full_path_with_prefix = '/' + full_path
                for media_path_to_ignore in paths_to_ignore:
                    if re.match(full_path_with_prefix, media_path_to_ignore):
                        self.logger.info(f"Skipping: {full_path_with_prefix} due to match in SYNC_ENV_MEDIA_BACKUP_PATHS_TO_IGNORE")
                        continue
                    else:
                        self.logger.info(f"Looking in: {full_path_with_prefix} for files")
                        dirs.extend(os.path.join(path, subdir))


    def _create_tar(self, media_storage, name, paths_to_ignore):
        """Create TAR file."""
        ...
        for media_filename in self._explore_storage(media_storage, paths_to_ignore):
    ...
    def backup_mediafiles(self, media_storage, storage, paths_to_ignore):
        """
        ...
        tarball = self._create_tar(media_storage, filename, paths_to_ignore)
