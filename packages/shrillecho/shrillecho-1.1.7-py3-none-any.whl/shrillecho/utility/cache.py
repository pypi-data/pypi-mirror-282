...
class Mongo(Cache):
    ...
    async def cache_query(
        self, cache_key, cache_update_function, expiry, class_type, **kwargs
    ):
        ...
        data_dict["expireAt"] = datetime.datetime.now() + datetime.timedelta(
            seconds=expiry
        )
...
class Redis(Cache):
    ...
    async def cache_query(
        self, cache_key: str, cache_update_function, expiry, class_type, **kwargs
    ):
        ...
        if cached_value:
            ...
            if isinstance(retrieved_value, list):
                return [
                    class_type.from_json(json.dumps(item)) for item in retrieved_value
                ]
    ...
    def invalidate_cache_by_prefix(self, prefix: str):
        pattern = f"{prefix}*"
...
redis_client: Redis = Redis("192.168.56.101", "6379")


def set_redis_host(host):
    print(f"updating redis to {host}")
    redis_client = Redis(host, "6379")


# Cache invalidation method
async def invalidate_cache(prefix: str):
    redis_client.invalidate_cache_by_prefix(prefix)


def cached(cache_key_prefix, class_type, expiry=3600):
    def decorator(func):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            ...
            return await redis_client.cache_query(
                full_cache_key, update_function, expiry, class_type
            )
