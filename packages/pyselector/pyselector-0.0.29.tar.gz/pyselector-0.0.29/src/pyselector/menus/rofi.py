...
SUPPORTED_ARGS: dict[str, str] = {
    'name': 'rofi',
    'url': constants.HOMEPAGE_ROFI,
    'theme': 'theme name (str)',
    'lines': 'number of lines (int)',
    'prompt': 'prompt message (str)',
    'markup': 'enable markup (bool)',
    'mesg': 'message to display (str)',
    'filter': 'filter the list by setting text in input bar to filter (str)',
    'cycle': 'cycle through the result list. (bool)',
    'location': """Specify where the window should be located. The numbers map to the
          following locations on screen:
                1 2 3
                8 0 4
                7 6 5
          Default: 0""",
    'width': 'width of the selection window (str). e.g. 60%',
    'height': 'height of the selection window (str). e.g. 50%',
    # FIX:
    # -case-sensitive
    # multi-select
}
...
class Rofi:
    ...
    def new_build_args(self, **kwargs) -> list[str]:  # noqa: C901, PLR0912
        messages: list[str] = []
        dimensions_args: list[str] = []
        ...
        args.extend(['-p', kwargs.get('prompt', constants.PROMPT)])
    ...
    def select(self, items: Sequence[T], hide_keys: bool = False, **kwargs) -> T | None:
        restore = False
        if hide_keys:
            self.keybind.toggle_hidden()
            restore = True

        args = self.new_build_args(**kwargs)
        selected, keycode = helpers.run(args, items)

        if restore:
            self.keybind.toggle_hidden(restore=True)

        if not selected or keycode == UserCancel(1):
            return None

        result = extract.item(items, selected)
    ...
    def input(self, prompt: str = 'PySelector> ', validation: Callable[..., T] | None = None) -> str:
        raise NotImplementedError

    def confirm(self, question: str, options: Sequence[str], default: str) -> bool:
        raise NotImplementedError

    def supported_args(self) -> str:
        """Shows a list of supported arguments for the menu"""
        formatted_lines = [f'{key:^9}: {value:<30}' for key, value in SUPPORTED_ARGS.items()]
