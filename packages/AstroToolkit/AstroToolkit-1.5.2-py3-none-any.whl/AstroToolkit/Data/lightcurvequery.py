class LightcurveStruct(object):
    ...
    def plot(self, kind="lightcurve", colours=None, bands=None, freq=None, bins=None):
        from ..Plotting.plotmap import map_to_plot

        return map_to_plot(
            self, kind=kind, colours=colours, bands=bands, freq=freq, bins=bins
        )
    ...
    def bin(self, bins=None, binsize=None):
        ...
        if self.data:
            self.data = [
                binning(band, bins, binsize) if band is not None else None
                for band in self.data
            ]
        return self
