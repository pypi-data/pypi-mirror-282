...
class TestAttribute:
    ...
    def test_get_levels(self, s1):
        ...
        attributeStateHiearchy = s1.get_xobject("Patients").get_dimension("Gender").get_attribute("Gender").get_levels()

        print(attributeStateHiearchy)

        assert False #str(attributeStateHiearchy).startswith("Gender")

    # BTS-144
    def test_get_root_state(self, s1):

        attributeStateHiearchy = s1.get_xobject("Patients").get_dimension("Gender").get_attribute(
            "Gender").get_root_state()

        print(attributeStateHiearchy)

        assert True  # str(attributeStateHiearchy).startswith("Gender")

    # BTS-145
    def test_get_state_hierarchy(self, s1):

        attributeStateHiearchy2 = xplain.Attribute("Patients", "Gender", "Gender", s1).get_state_hierarchy()

        print(attributeStateHiearchy2)

        attributeStateHiearchy = s1.get_xobject("Patients").get_dimension("Gender").get_attribute(
            "Gender").get_state_hierarchy()
