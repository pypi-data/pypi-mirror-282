...
class TestXsession:
    ...
    def test_get_queries(self, s1):
        ...
        print(queryIds, openedQuery)

        assert False  # len(jsondiff.diff(dimensionInfo, dimensionInfoRef)) == 0
    ...
    def test_get_xobject(self, s1):
        ...
        object2 = s1.get_object("Patients")

        assert str(object_name).startswith("<xplain.xobject.XObject object at ")

        assert str(object2).startswith("<xplain.xobject.XObject object at ")
    ...
    def test_get_child_objects(self, s1):
        childObject = (s1.get_root_object().get_child_objects()[1]).get_name()

        object_name = (s1.get_xobject("Patients").get_child_objects()[1]).get_name()

        print(childObject, object_name)
    ...
    def test_get_dimensions(self, s1):
        childDimension = ((s1.get_root_object().get_child_objects()[1]).get_dimensions()[0]).get_name()

        dimension = ((s1.get_xobject("Patients").get_child_objects()[1]).get_dimensions()[0]).get_name()
