...
def init_download_dir():
    ...
    if not os.path.isdir(history_download_dir):
        try:
            ...
            logger.info(f"Created candlestix cache_dir {candlestix_conf['cache_dir']}")
        except OSError as e:
            logger.warning(f"Error while creating candlestix cache_dir {candlestix_conf['cache_dir']}: {e}. "
...
def reset():
    ...
    try:
        ...
        logger.info(f"Deleted candlestix cache_dir {candlestix_conf['cache_dir']}")
    except Exception as e:
        logger.warning(f"Error while deleting candlestix cache_dir {candlestix_conf['cache_dir']}: {e}",
                       stack_info=True)
    init_download_dir()
    # 2. download and process the instruments file
    f = download_and_unzip_gzip_file()
