...
class BollingerBandsIndicator:
    ...
    def bollinger_std(self):
        bollinger_std = np.full(len(self.close), np.nan)
        for i in range(self.window - 1, len(self.close)):
            bollinger_std[i] = np.std(self.close[i-(self.window-1): i+1]) # 0 -> 19, 1 -> 20, 2 -> 21,...
        return bollinger_std
