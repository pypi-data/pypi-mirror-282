...
# TODO config view could display the config file
...
# TODO decide on an ASCII art font
...
# TODO this needs to be an env var
...
def config(name: Annotated[str, typer.Option("--name", "-N",
    ...
    """
    # TODO should allow custom config file locations
    config_path = os.path.expanduser(DEFAULT_CONTEXT_PATH)
    # TODO handle more cases if we allow custom config file locations
...
def init(project_name: Annotated[
    str, typer.Option("--project-name", "-n", help="Name for the VAIP App (No spaces or special chars besides '_')", prompt=True)],
    ...
    if any(char in invalid_characters for char in project_name):
        print("Invalid project name. The only special character allowed is '_'.")
        typer.Exit(code=1)
...
def build(confirm_build: Annotated[bool, typer.Option("--yes", "-y",
    ...
    print("Built VAIP App, check your /dist directory.")


# TODO we may want args here to specify uploading outside of the current directory
# TODO handle 404 when CLI API is turned off
@app.command()
