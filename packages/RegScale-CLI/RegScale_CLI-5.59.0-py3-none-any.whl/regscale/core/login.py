...
def get_regscale_token(
    ...
    if response.status_code == 200:
        ...
        raise ValueError("Unable to authenticate with RegScale. Please check your credentials.")
    elif response.status_code == 400 and mfa_token == "":
        raise ValueError("Unable to authenticate with RegScale. Please try again with an MFA Token.")
    else:
        raise ValueError(f"Unable to authenticate with RegScale.\n{response.status_code}: {response.text}")
