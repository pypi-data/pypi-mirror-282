...
class ContainerScan(ABC):
    ...
    def validate_headers(self, header: list) -> None:
        ...
        if ignore_validation and not all(
            item in header for item in self.mapping.expected_field_names
        ):
            raise AssertionError(
    ...
    def handle_extra_headers(self, header: list) -> None:
        ...
        """
        extra_headers = [
            column for column in header if column not in self.attributes.headers
        ]
        required_headers = [
            column for column in header if column in self.attributes.headers
        ]
    ...
    def create_assets(self, func: Callable) -> None:
        ...
        :rtype: None
        """
        existing_assets: List[Asset] = Asset.get_all_by_parent(
        ...
        if update_assets := [asset for asset in self.data["assets"] if asset and asset in existing_assets]:
    ...
    def lookup_kev(self, cve: str) -> str:
        ...
        if kev_data:
            ...
            kev_date = convert_datetime_to_regscale_string(
                datetime.strptime(kev_data["dueDate"], DT_FORMAT)
            )
        return kev_date

    def update_due_dt(
        self, iss: Issue, kev_due_date: Optional[str], scanner: str, severity: str
    ) -> Issue:
        """
    ...
    def create_issues(self, func: Callable) -> None:
        ...
        with create_progress_object() as issue_progress:
            issue_task = issue_progress.add_task(
                "Processing issues...", total=len(self.file_data)
            )
            for dat in self.file_data:
        ...
        self.attributes.logger.info(
            "Found %i issues for processing in RegScale.", len(self.data["issues"])
        )
        if insert_issues := [
            ...
            for cve, issues in existing_cves.items():
                if cve not in parsed_cves:
                    for issue in issues:
                        ...
                        issue.status = "Closed"
                        issue.dateCompleted = self.scan_date.strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                        if issue.save():
    ...
    def log_and_save_closed_issues(self, closed_issues: list[Issue]) -> None:
        ...
        if len(closed_issues) > 0:
            self.attributes.logger.info(
                "Closed %i issue(s) in RegScale.", len(closed_issues)
            )
            ReportGenerator(
    ...
    def handle_link_creation(
        self, new_issues: list[Issue], parsed_issues: list[Issue]
    ) -> None:
        """
            ...
            if cve in parsed_issues:
                ...
                if vuln_link := parsed_issues[cve].extra_data.get("link"):
                    ...
                    self.attributes.logger.debug(
                        "Creating link for issue #%s, url: %s", issue.id, vuln_link
                    )
                    new_link = Link(
                        url=vuln_link,
                        parentID=issue.id,
    ...
    def create_vulns(self, func: Callable) -> None:
        ...
        with create_progress_object() as vuln_progress:
            vuln_task = vuln_progress.add_task(
                "Processing vulnerabilities...", total=len(self.file_data)
            )
            for dat in self.file_data:
                vuln = func(dat)
    ...
    def clean_up(self) -> None:
        ...
        try:
            if self.attributes.parent_id:
                ...
                self.attributes.logger.info(
                    "File uploaded to RegScale and moved to %s", processed_dir
                )
        except shutil.Error:
            self.attributes.logger.debug(
    ...
    def gen_issue(self, **kwargs: dict) -> Issue:
        ...
        iss.dateFirstDetected = (
            first_detected_dt.strftime(self.dt_format)
            if isinstance(first_detected_dt, datetime)
            else ""
        )
