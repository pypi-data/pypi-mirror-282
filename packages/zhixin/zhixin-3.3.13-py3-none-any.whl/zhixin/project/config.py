...
class ProjectConfig(ProjectConfigBase, ProjectConfigLintMixin, ProjectConfigDirsMixin):
    ...
    def as_tuple(self):
        click.echo("self: %s" % self)
        tuples = [(s, self.items(s)) for s in self.sections()]
        click.echo("contents tuple: %s" % tuples)
    ...
    def save(self, path=None):
        ...
        with open(path or self.path, mode="w+", encoding="utf8") as fp:
            ...
            fp.seek(0)
            contents = fp.read()
            click.echo("contents: %s" % contents)
            tuples = [(s, self.items(s)) for s in self.sections()]
            click.echo("contents tuple: %s" % tuples)
            tuples = [{s: self.items(s, None, True)} for s in self.sections()]
            contents = json.dumps(tuples[0], ensure_ascii=False, indent=2)
            click.echo("contents json: %s" % contents)
            fp.seek(0)
            fp.truncate()
