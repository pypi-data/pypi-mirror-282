...
from .base import rattail_typer, Subcommand, date_argument, dict_argument
...
class BatchHandlerCommand(Subcommand):
    ...
    def run(self, args):
        ...
        do_run(self.config, args.batch_type, self.do_stuff, user, params,
               progress=self.progress)

    def do_stuff(self, params, handler, session, user, progress=None):
...
class MakeBatch(BatchHandlerCommand):
    ...
    def add_args(self):
        ...
        parser.add_argument('--kwargs', dest='create_kwargs', type=dict_argument, default={},
                            help="Optional JSON-encoded string containing extra "
                            "keyword arguments to be passed to the handler's batch "
                            "creation logic.")
    ...
    def do_stuff(self, params, handler, session, user, progress=None):
        ...
        """
        return do_make_batch(params, handler, session, user, progress=progress)
...
class BatchAction(BatchHandlerCommand):
    ...
    def do_stuff(self, params, handler, session, user, progress=None):
        ...
        if not batch:
            raise RuntimeError("Batch of type '{}' not found: {}".format(params['batch_type'], params['batch_uuid']))

        return self.action(params, handler, batch, user, progress=progress)
...
class AutoReceiveBatch(BatchAction):
    ...
    def action(self, params, handler, batch, user, progress=None):
...
class ExecuteBatch(BatchAction):
    ...
    def add_args(self):
        ...
        parser.add_argument('--kwargs', dest='execute_kwargs', type=dict_argument, default={},
                            help="Optional JSON-encoded string containing extra "
                            "keyword arguments to be passed to the handler's batch "
                            "execution function.")
...
class PurgeBatches(BatchHandlerCommand):
    ...
    def run(self, args):
        ...
        handler = self.app.get_batch_handler(args.batch_type)
