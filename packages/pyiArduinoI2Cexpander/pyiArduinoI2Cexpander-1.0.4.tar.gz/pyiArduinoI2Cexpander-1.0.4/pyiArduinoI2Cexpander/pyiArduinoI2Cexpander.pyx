...
cdef class pyiArduinoI2Cexpander:
    cdef iarduino_I2C_Expander c_module

    def __cinit__(self, address=None, auto=None, bus=None):

        if address is not None:

            self.c_module = iarduino_I2C_Expander(address)
            ...
            self.c_module = iarduino_I2C_Expander()
    ...
    def levelWrite(self, unsigned short level):
        self.c_module.levelWrite(level)

    def levelRead(self, unsigned char pin):
        return self.c_module.levelRead(pin)

    def levelHyst(self, unsigned short hysteresis):
        self.c_module.levelHyst(hysteresis)
    ...
    def servoAttach(self, unsigned char pin, unsigned short width_min, unsigned short width_max, short angle_min, short angle_max):
        self.c_module.servoAttach(pin, width_min, width_max, angle_min, angle_max)

    def servoWrite(self, unsigned char pin, short angle):
        self.c_module.servoWrite(pin, angle)

    def servoWriteMicroseconds(self, unsigned char pin, unsigned short width):
        self.c_module.servoWriteMicroseconds(pin, width)

    def changeBus(self, char* bus):
        self.c_module.changeBus(bus)

    def mymap(self, long x, long in_min, long in_max, long out_min, long out_max):
        return self.c_module.map(x, in_min, in_max, out_min, out_max)
