...
# @memoize_cache
def legacy_ask_gpt_for_response(prompt, max_response_tokens=5, model="gpt-3.5-turbo-instruct"):
...
def ask_gpt_for_response(messages, max_response_tokens=5, model="gpt-3.5-turbo"):
    ...
    # trim string
    trimmed_response = response_message.content.strip()
    return trimmed_response


$ZIG update. probly in my top 3 biggest bags..i gotta check again but defo top 5

if it breaks out bull flag then im expecting 0.207 next https://t.co/koWKQntl5q
