        ...
        try:
            ...
            for i in range(0, len(chat_info_list), 2):
                ...
                if chat_info_list[i]['text'] is not None and chat_info_list[i + 1]['text'] is not None:
                    ...
                    chat = chat.replace("\\n", "\n")
                    chat_memory.update({"question": chat})
                    files = []
                    if len(chat_info_list[i]["resourceList"]) > 0:
                        for resource in chat_info_list[i]["resourceList"]:
                    ...
                    plugin = chat_info_list[i + 1]["pluginCode"]
