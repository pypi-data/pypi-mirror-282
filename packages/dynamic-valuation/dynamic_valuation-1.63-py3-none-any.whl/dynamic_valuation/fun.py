...
#####
def find_fixed_points(cl):
  global sdotvec0
  global signvec
  global sign_change_indices
  global fig
  global ax
  sdotvec0=np.array([cl.sdot(i,cl.x(i)) for i in cl.s_grid])
  signvec=np.sign(sdotvec0).astype(int)

  sign_diffs = np.diff(signvec)
  sign_change_indices = np.unique(np.r_[0,np.where(sign_diffs!=0)[0],[len(signvec)-1]])  # Skip first index

  fig, ax = plt.subplots()
  ax.plot(cl.s_grid,sdotvec0)
  ax.plot(cl.s_grid[sign_change_indices],sdotvec0[sign_change_indices],'ko',label='Unstable Roots')
...
def find_stable_points(cl):
  ...
  roots = []
  for i,s in enumerate(sign_change_indices[:-1]):  # Include 0th index and all sign changes

      a, b = max(0,cl.s_grid[sign_change_indices[i]]-cl.ds), cl.s_grid[sign_change_indices[i]]+cl.ds
  ...
  for r in roots:
    if np.isin(r, cl.s_grid):
        sdotvec0[np.where(cl.s_grid==r)]=0
        signvec[np.where(cl.s_grid==r)]=0
        print(f"stable root {r} is in the grid already")
    else:
        cl.s_grid=np.insert(cl.s_grid,0,r)
        cl.s_grid=np.unique(np.sort(cl.s_grid))
        sdotvec0=np.insert(sdotvec0,np.where(cl.s_grid==r)[0],0)
        signvec=np.insert(signvec,np.where(cl.s_grid==r)[0],0)
        print(f"stable root {r} added to the grid")


  ax.plot(roots,[cl.sdot(i,cl.x(i)) for i in roots],'ro',label='Stable Roots')
...
def buffer_indices(indices, buffer_size, set_length):
  """
  Creates a buffer around the given indices.

  Args:
      indices: An array of indices.
      buffer_size: The size of the buffer.
      set_length: The length of the set being indexed.

  Returns:
      A list of lists containing the buffered indices.
  """
  ...
  for index in indices:
    ...
    # Create a list of buffered indices for this index.
...
def vf_plot(cl):
  ...
  global dtvec
  global v
  indices = np.where(np.isin(cl.s_grid, roots))[0]

  buffered_indices = buffer_indices(indices,9 ,len(cl.s_grid))


  wvec0=np.array([cl.W(s,cl.x(s)) for s in cl.s_grid])
  delta_s=np.r_[np.gradient(cl.s_grid)]#np.r_[np.diff(cl.s_grid),0]

  nextvec=range(len(cl.s_grid))+signvec.astype(int)

  sdotvec1=np.array([cl.sdot(cl.s_grid[i],cl.x(cl.s_grid[i])) for i in nextvec])
  wvec1=np.array([cl.W(cl.s_grid[i],cl.x(cl.s_grid[i])) for i in nextvec])
  wvec=.5*(wvec0+wvec1)
  sdotvec=.5*(sdotvec0+sdotvec1)
  #absdot=[np.abs(i),0) for i in sdotvec]
  dtvec=abs(delta_s/sdotvec)
  ...
  #ax.plot(cl.s_grid, v, color=plt.cm.jet(0),
        # lw=2, alpha=0.6, label='Initial guess')

  for i in range(len(buffered_indices)):
    rudy=roots[i]
    ...
    for j in range(len(buffered_indices[i])):
      v[buffered_indices[i][j]]=val+w_s/(cl.δ-sdot_s)*(cl.s_grid[buffered_indices[i][j]]-rudy)
      value_set[buffered_indices[i][j]]=1
    ...
    for i in range(1,len(cl.s_grid)):
      ...
      if(sdotvec[i]<=0) and value_set[i]==0:

        v[i]=np.exp(-cl.δ*dtvec[i])*v[next]+wvec[i]*dtvec[i]*np.exp(-cl.δ*dtvec[i]/2)

    for i in reversed(range(len(cl.s_grid))):

      next=nextvec[i]
      if(sdotvec[i]>=0) and value_set[i]==0:# and (errorvec[i]>.01):

        v[i]=np.exp(-cl.δ*dtvec[i])*v[next]+wvec[i]*dtvec[i]*np.exp(-cl.δ*dtvec[i]/2)


  ax.plot(cl.s_grid, v, color='red',label='Approximate Solution',lw=4)
  rhs=(np.gradient(v)/np.gradient(cl.s_grid)*sdotvec0+wvec0)/cl.δ

  ax.plot(cl.s_grid, rhs,color='blue',label='RHS')
  ...
  ax.set_xlabel('Capital stock: $s$', fontsize=12)
