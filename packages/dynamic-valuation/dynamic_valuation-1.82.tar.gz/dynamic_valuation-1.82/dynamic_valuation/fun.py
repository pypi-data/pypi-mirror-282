...
def find_stable_points(cl):
  ...
  cl.sax.plot(cl.s_grid[cl.sign_change_indices],cl.sdotvec0[cl.sign_change_indices],'ko',label='Unstable Fixed Points or Endpoints')
...
def vf_plot(cl):
  ...
  for i in range(len(cl.buffered_indices)):
    ...
    for j in range(len(cl.buffered_indices[i])):
      ...
      cl.value_set[cl.buffered_indices[i][j]]=1

  cl.vfig0,cl.vax0=plt.subplots()
  cl.vax0.plot(cl.s_grid[cl.value_set==1], v[cl.value_set==1], color='b',label='Approximate Solution',lw=4)
  ...
  cl.vax.plot(cl.s_grid, v, color='red',label='Approximate Solution',lw=4)
  cl.vax.plot(cl.s_grid, rhs,color='blue',label='RHS')
  cl.vax.legend()
