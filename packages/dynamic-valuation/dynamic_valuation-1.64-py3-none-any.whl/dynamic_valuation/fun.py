...
def find_fixed_points(cl):
  ...
  fig, ax = plt.subplots()
  ax.plot(cl.s_grid,cl.sdotvec0)
  ax.plot(cl.s_grid[cl.sign_change_indices],cl.sdotvec0[cl.sign_change_indices],'ko',label='Unstable Roots')
...
def find_stable_points(cl):
  ...
  ax.plot(cl.roots,[cl.sdot(i,cl.x(i)) for i in cl.roots],'ro',label='Stable Roots')
...
def vf_plot(cl):
  ...
  for i in range(len(cl.buffered_indices)):
    ...
    for j in range(len(cl.buffered_indices[i])):
      ...
      value_set[cl.buffered_indices[i][j]]=1
