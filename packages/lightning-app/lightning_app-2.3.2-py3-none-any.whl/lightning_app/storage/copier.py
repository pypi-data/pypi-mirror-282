...
class _Copier(Thread):
    ...
    def run_once(self):
        ...
        obj: Optional[lightning_app.storage.path.Path] = _find_matching_path(self._work, request)
        if obj is None:
...
def _find_matching_path(work, request: _GetRequest) -> Optional["lightning_app.storage.path.Path"]:
    for name in work._paths:
        candidate: lightning_app.storage.path.Path = getattr(work, name)
