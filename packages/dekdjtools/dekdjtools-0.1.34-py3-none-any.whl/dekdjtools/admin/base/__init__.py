...
def calc_list_display(model_cls, rewrite_set=None, disable_set=None, rewrite_suffix='_'):
    ...
    for name in mfs.sort_fields(mfs.auto.keys(), mfs.common.keys(), mfs.ct.keys(), mfs.o2o.keys(), mfs.o2m.keys()):
...
def calc_search_fields(model_cls, list_display=None, disable_set=None):
    array = []
    mfs = ModelFields(model_cls)
    for name in mfs.sort_fields(mfs.auto.keys(), mfs.common.keys(), mfs.ct.keys(), mfs.o2o.keys(), mfs.o2m.keys()):
        if disable_set and name in disable_set:
...
def admin_register(model_cls, list_display=None):
    list_display = list_display or calc_list_display(model_cls)
    type(
        'admin',
        (ModelAdminBase,),
        {
            ...
            'search_fields': calc_search_fields(model_cls, list_display)
        }
    )
