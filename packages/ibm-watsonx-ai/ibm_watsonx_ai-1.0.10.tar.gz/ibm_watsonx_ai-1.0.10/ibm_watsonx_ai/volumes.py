        ...
        **Example**
        ...
        try:
            ...
            if creation_response.status_code == 200:
                ...
                print(
                    "Failed to start the volume. Make sure volume is in running with status RUNNING or UNKNOW and then re-try"
                )
                return "FAILED"
            else:
                print(creation_response.status_code, creation_response.text)
    ...
    def upload_file(self, name: str, file_path: str) -> Literal["SUCCESS", "FAILED"]:
        ...
        **Example**
        ...
        if upload_response.returncode == 0:
            ...
            print(
                upload_response.returncode,
                upload_response.stdout,
                upload_response.stderr,
            )
            print("Failed to upload the file to  volume. Try again.")
            return "FAILED"
