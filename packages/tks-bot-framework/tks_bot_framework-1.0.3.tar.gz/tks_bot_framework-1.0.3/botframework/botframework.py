...
class Botframework:
    ...
    async def start_framework(self, kafka_initial_tasks, other_initial_tasks, asyncio_tasks, ksqldb_query_url, view_name):
        ...
        for pos in pfm.get_directional_position_capacities(Direction.SHORT):
            logger.info(pos)    
        # Execute the Kafka initial tasks.
        kafka_async_tasks = []
        for initial_task in kafka_initial_tasks:
            kafka_async_tasks.append(self.execute_with_retries(initial_task))
        await asyncio.gather(*kafka_async_tasks)
        logger.info("All Kafka initial tasks completed. Waiting for 120 seconds...")
        # Wait for 120 seconds to make sure the kafka materialized views are ready for querying.
        await asyncio.sleep(120)
          # Execute the other initial tasks sequentially.
        for initial_task in other_initial_tasks:
            await initial_task()
        logger.info("All other initial tasks completed.")
    ...
    async def execute_sql(self, sql):
        await database.execute_sql(sql)
