...
class NameDataset:
    ...
    def __init__(self, load_first_names=True, load_last_names=True):
        ...
        first_names_filename = Path(os.path.dirname(__file__)) / "v4/first_names.pkl"
        last_names_filename = Path(os.path.dirname(__file__)) / "v4/last_names.pkl"
        self.first_names = self._load_pickle(first_names_filename) if load_first_names else None
        self.last_names = self._load_pickle(last_names_filename) if load_last_names else None
        end_time = time.time()  # End timing here
        print(f"time to load the names_dataset pickles: {end_time - start_time:.2f} seconds")
    ...
    def get_top_names(self, n: int = 100, use_first_names: bool = True, country_alpha2: Optional[str] = None, gender: Optional[str] = None):
        if n <= 0:
        ...
        for country_, ranks in ranks_per_country.items():
            for gender_id in ["F", "M", "N/A"]:
                if gender_id in ranks:
                    ranks[gender_id] = [a[0] for a in sorted(ranks[gender_id], key=operator.itemgetter(1), reverse=False)][0:n]
    ...
    def _post_process(result):
        ...
        return {
            "country": {pycountry.countries.get(alpha_2=a).name: b for a, b in result["country"].items()},  # type: ignore
            # 'popularity_score': {pycountry.countries.get(alpha_2=a).name: b for a, b in result['popularity'].items()},
            "gender": {gender[a]: b for a, b in result["gender"].items()},  # type: ignore
            "rank": {pycountry.countries.get(alpha_2=a).name: b for a, b in result["rank"].items()},  # type: ignore
        }
