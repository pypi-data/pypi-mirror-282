...
class Sdf:
    ...
    def process_sdf(self, input_file, database, batch_size, affinity_cutoff):
        ...
        for i, line in enumerate(lines):
            ...
            if line.strip() == '$$$$':
                if current_affinity is not None and current_affinity <= affinity_cutoff:
...
class AdmetSpreadsheet:
    ...
    def process_data(self, df, weights):
        ...
        new_cols = ['ID_Molecula', 'Afinidade', 'smiles'] + self.new_cols_to_move + [col for col in analysis_df.columns if col not in self.new_cols_to_move and col not in ['ID_Molecula', 'Afinidade', 'smiles']]
...
class Spreadsheet:
    ...
    def spreadsheet_output(self, df, top_n):
        ...
        updated_df = updated_df.head(top_n)
