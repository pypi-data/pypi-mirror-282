...
def tourney(request, session, entities):
    ...
    if tourney.system == 'knockout':
        ...
        for p, c in enumerate(
            sorted(tourney.competitors, key=lambda c: c.position or -c.rate), 1
        ):
...
def _get_match(request) -> Match | None:
    tid, bnum = _parse_signed_arg(request, 'board', _split_two_ints)
    q = request.dbsession.query(Match).join(Tourney)
    q = q.filter(
        Tourney.idtourney == tid, Match.turn == Tourney.currentturn, Match.board == bnum
    )
...
def match_form(request):
    m = _get_match(request)
    if m is None:  # pragma: no cover
        raise HTTPNotFound()
    elif m.tourney.championship.trainingboards:  # pragma: no cover
    ...
    return {
        ...
        'match': m,
        'currentturn': m.tourney.currentturn,
        'elapsed': m.tourney.countdownElapsed,
        'request': request,
        'session': request.dbsession,
        'today': date.today(),
        'version': version,
    ...
    if 'end_match' in request.params:
        ...
        return HTTPSeeOther(
            location=request.route_path(
                'lit_tourney',
                guid=m.tourney.guid,
                _query={'turn': m.tourney.currentturn, 'board': m.board},
            )
        )
    else:
        changes_logger.info('%r is being self-compiled', m)
        return {'success': True, 'message': 'Ok', 'elapsed': m.tourney.countdownElapsed}
...
def get_countdown_elapsed(request):
    ...
    return {
        'success': True,
        'message': 'Ok',
        'elapsed': row and Tourney.computeElapsedTime(row[0], row[1]),
    }
