...
from teams_ai_azml.state import ConversationState


class TestConversationState(IsolatedAsyncioTestCase):
    ...
    async def test_should_load(self):
        ...
        state = await ConversationState.load(context, storage)
        self.assertEqual(state.__key__, "channel1/bot1/conversations/conversation1")
    ...
    async def test_should_not_load_when_channel_missing(self):
        ...
        with self.assertRaises(ValueError, msg="missing activity.channel_id"):
            await ConversationState.load(context, storage)

    async def test_should_not_load_when_conversation_missing(self):
        context = MagicMock()
        ...
        with self.assertRaises(ValueError, msg="missing activity.conversation"):
            await ConversationState.load(context, storage)

    async def test_should_not_load_when_recipient_missing(self):
        context = MagicMock()
        context.activity.channel_id = "channel1"
        context.activity.conversation = MagicMock(id="conversation1")
