...
def build_ssh_tunnel(ip_chain, pnodes):
    ...
    def build_tunnel(c, addr, hosts, counter):
        ...
        try:
            ...
            #client.connect(next_node, username=node.username, password=node.passwd, key_filename=node.ssh_keyfile, sock=c_channel)
    ...
    try:
        if node.ssh_keyfile:
            client.connect(ip_chain[counter], username=node.username, password=node.passwd, key_filename=node.ssh_keyfile)
        else:
