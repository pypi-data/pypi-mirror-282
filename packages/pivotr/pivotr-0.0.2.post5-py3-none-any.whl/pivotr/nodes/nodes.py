...
class Node:
	def __init__(self, node_id, parent_node, addr, username='', passwd='', ssh_keyfile=''):
		...
		self.addr 			= addr
		self.username 		= username 			# Default username from creds{}
		self.passwd 		= passwd 			# Corresponding default password from creds{}
		self.ssh_keyfile	= ssh_keyfile		# Private key location (if any)
		self.creds 			= {username:[passwd, ssh_keyfile]}
		self.childNodes		= []
		#self.creds 			= {username:passwd}
	...
	def view_node(self):
		...
		if len(self.creds) == 0:
			...
			print('      │    ├─Password: 	' + '**********')
			print('      │    ╰─Keyfile:	' + self.ssh_keyfile)
		print('      ├─Child nodes:     ')
			...
			match choice:
				...
				case 'c' | 'C':
					while True:
						...
						# Get credentials to edit
						choice = input('Selection: ')

						# Check our 'char' inputs first
						...
						match choice:
							...
							case 'a' | 'A':
								#key_location = input("Private key location: ")
								#self.creds = [item.replace(str(self.creds[name_to_edit][1]), key_location) for item in self.creds]
				...
				case _:
					...
					continue
		print('[+] Node n' + str(self.node_id) + ' successfully modified.')
	...
	def node_owned(self):
		if len(self.creds.keys()) > 0:
			for username, passwd in self.creds.items():
				if len(username) > 0:
					if len(passwd[0]) > 0 or len(passwd[1]) > 0:
						return True
		return False