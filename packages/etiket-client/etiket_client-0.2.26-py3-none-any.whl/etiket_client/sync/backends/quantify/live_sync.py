import time, logging, tempfile, h5py
...
def check_live_dataset(file_location: Path) -> bool:
    ...
    try:
        dataset = xr.open_dataset(file_location)
...
class XArrayReplicator:
    ...
    '''
    def __init__(self, dataset_location: Path, qdrive_dataset:dataset):
        self.dataset_location = dataset_location
        self.qdrive_dataset = qdrive_dataset
        self.dataset_followers = {}
        
        self.last_mod_sync = dataset_location.stat().st_mtime
        xr_dataset = xr.open_dataset(dataset_location)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.qdrive_dataset.add_new_file("measurement", destination=m_file,
                                   file_type=FileType.HDF5_CACHE, generator="etiket_client", status = FileStatusLocal.writing)
            
        self.hdf5_file = h5py.File(self.qdrive_dataset["measurement"].path, 'a', locking=False, libver='v112')
        print("opened file from ", self.qdrive_dataset["measurement"].path)
        ...
        while keep_syncing:
            ...
            if self.__has_update():
                try: # since the writing process in quantify is a little sketchy, we need to catch exceptions
                    ...
                    xr_dataset = xr.open_dataset(self.dataset_location, engine='h5netcdf')
                    for name in xr_dataset.variables:
            ...
            try:
                ...
                for i in range(len(data.shape)):
                    if old_cursor[i] == new_cursor[i]:
                        ...
                        break
                print(old_cursor, new_cursor, slices)
                print("writing data", data[*slices])
