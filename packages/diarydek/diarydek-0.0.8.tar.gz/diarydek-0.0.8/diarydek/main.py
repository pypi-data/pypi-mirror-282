...
def diarydek():
    ...
    diarydek = Diarydek(debug=args.debug, db=args.database)

    if args.renameTag:
        if args.words:
            diarydek.error("extra words after '--rename-tags old new'")

        diarydek.rename_tag(args.renameTag[0], args.renameTag[1])
        exit(0)
    ...
    if args.version:
        (major, minor, subminor) = diarydek.appversion
        print("diary version %d.%d.%d" % (major, minor, subminor))
        sys.exit(0)
    if args.showTags:
        print("Tags in database, with counts:")
        for row in diarydek.get_tags_with_counts():
    ...
    if args.time:
        ...
        if len(tmp) == 10:
            ...
            diarydek.error("must give time as \"yyyy-mm-dd\" or \"yyyy-mm-dd HH:MM:SS\"")
            sys.exit(1)
    since = None
    if args.since:
        tmp = args.since[0]
    ...
    if args.readCSV:
        with open(args.readCSV) as csv:
            ...
            for row in rows:
                ...
                # print(tags)
                diarydek.add_entry(time, entry, tags)
        sys.exit(0)  # handle --readCSV

    # Write whole database to CSV
    if args.writeCSV:
        tags = diarydek.get_table("tags")
        entries = diarydek.get_table("entries")
        entry_tags = diarydek.get_table("entry_tags")
        if args.debug:
            print("tags: ", end="")
            print(tags)
            print("entries: ", end="")
