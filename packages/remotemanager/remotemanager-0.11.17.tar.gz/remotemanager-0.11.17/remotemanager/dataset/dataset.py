...
class Dataset(SendableMixin):
    ...
    def run(
        ...
        **run_args,
        ...
        for runner in self.runners:
            ...
            runline = [
                f"echo $(date +'%d/%m/%Y %H:%M:%S') {runner.short_uuid} "
                f"submitted >> {self.manifest_log.name} &&",
    ...
    def update_runners(
        ...
        if log_collection.succeeded:
            ...
            logger.debug("extracted runner info: %s", format_iterable(runner_data))
            ...
            for runner in runners:
                ...
                if runner.short_uuid in runner_data:
                    for logline in runner_data[runner.short_uuid]:
                        runner.insert_history(logline[0], logline[1].strip())
